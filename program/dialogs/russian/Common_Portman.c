...
void ProcessDialogEvent()
{
    ...
    // доп повека
    if (!CheckAttribute(NPChar, "Portman"))
        NPChar.Portman = 0;
    ...
    case "node_2":
        ...
        /* if (rand(3) <= 2)
              {        
                ...
                link.l1 = "Да уж.. Высоко пойдешь. Смотри, как бы не упасть!";
    ...
    case "Fraht_choise":
        ...
        if (sti(iFrahtGoodsQty1) > 10000)
            iFrahtGoodsQty1 = 10000 + rand(500);
        if (sti(iFrahtGoodsQty2) > 10000)
            iFrahtGoodsQty2 = 10000 + rand(500);
        if (sti(iFrahtGoodsQty3) > 10000)
            iFrahtGoodsQty3 = 10000 + rand(500);
    ...
    case "Fraht_Forcemajor":
        ...
        if (pchar.questTemp.WPU.Fraht.Chance > 3)
            TraderHunterOnMap(); //запуск ДУ на глобалке
        if (pchar.questTemp.WPU.Fraht.Chance == 2)
            FrahtHunterOnSea(); //создание ДУ в акватории
        DialogExit();
        break;
    ...
    case "Postcureer":
        ...
        if (sti(pchar.questTemp.WPU.Postcureer.DaysQty) == 1)
            pchar.questTemp.WPU.Postcureer.DaysQty = 2;
    ...
    case "Postcureer_1":
        ...
        if (pchar.questTemp.WPU.Postcureer.Chance > 3)
            EnemyNationHunterOnMap(false); //запуск перехватчиков на глобалке
        if (pchar.questTemp.WPU.Postcureer.Chance == 2)
            FrahtHunterOnSea(); //создание перехватчиков в акватории
    ...
    case "escort_1":
        ...
        if (pchar.questTemp.WPU.Escort.Chance > 3)
            EnemyNationHunterOnMap(false); //запуск перехватчиков на глобалке
        if (pchar.questTemp.WPU.Escort.Chance == 3)
            TraderHunterOnMap(); //запуск ДУ на глобалке
        if (pchar.questTemp.WPU.Escort.Chance < 2)
            FrahtHunterOnSea(); //создание перехватчиков в акватории
    ...
    case "escort_add_ships":
        ...
        for (i = 1; i <= 2; i++)
        {
            ...
            if (i == 1)
                sTemp = pchar.questTemp.WPU.Escort.ShipName1;
            if (i == 2)
                sTemp = pchar.questTemp.WPU.Escort.ShipName2;
            if (i == 2)
                ShipType = SHIP_FLEUT; //один всегда флейт
            ...
            if (i == 1)
                SetCharacterGoods(sld, iGoods1, iSpace1);
            if (i == 2)
                SetCharacterGoods(sld, iGoods2, iSpace2);
    ...
    case "Escort_complete_fail":
        ...
        if (GetCharacterIndex("EscortCaptain_1") == -1)
            RemoveCharacterCompanion(pchar, characterFromID("EscortCaptain_2"));
        else
    ...
    case "EscortBonus_complete_fail":
        if (GetCharacterIndex("EscortCaptain_1") == -1)
            ...
            RemoveCharacterCompanion(pchar, characterFromID("EscortCaptain_1")); //удалим компаньона
        pchar.quest.Escort_fail.over = "yes";                                    //снять прерывание
        amount = sti(pchar.questTemp.WPU.Escort.GoodsQty) - GetSquadronGoods(pchar, sti(pchar.questTemp.WPU.Escort.Goods));
        ...
        if (pchar.questTemp.WPU.Escort == "begin")
        {
            ...
            if (sti(pchar.questTemp.WPU.Escort.MinusMoney) < 0)
                amount = 101;
    ...
    case "Postcureer_LevelUp":
        ...
        case 0:
            ...
            if (pchar.questTemp.WPU.Postcureer.TargetPortmanID == "Villemstad_Mayor")
                pchar.questTemp.WPU.Postcureer.TargetPortmanID = "hol_guber"; // фикс Матиаса Бека #717
    ...
    case "Escort_LevelUp":
        ...
        case 0:                                                       //приключения на необитаемых островах
            ...
            while (!isLocationFreeForQuests(pchar.questTemp.WPU.Current.TargetIslandID))
    ...
    case "Escort_LUGo_01": //добавляем компаньона
        ...
        if (sti(pchar.questTemp.WPU.Escort.Chance) == 0)
            EnemyNationHunterOnMap(true); //скоростной перехватчик
        else
            FrahtHunterOnSea();
        break;
    ...
    case "Escort_LU1S_complete":
        dialog.text = "Да, он мне уже всё рассказал - как вы героически сражались с краснокожими язычниками, а затем с пиратским кораблем. Отличная работа, " + GetAddress_Form(NPChar) + "! Вы уже в который раз доказываете, что на вас можно положиться\nПолучите ваше вознаграждение - 40 000 песо. Не забывайте периодически заглядывать ко мне - такие люди как вы, по праву могут считаться незаменимыми, и у меня обязательно найдется для вас работа.";
        link.l1 = "Приятно это слышать, " + GetAddress_FormToNPC(NPChar) + "! Безусловно, мы будем сотрудничать и дальше. Всего доброго!";
    ...
    case "ShipStockManBack":
        ...
        {
            dialog.text = "Посмотрим... Этот корабль ушел из порта " + chref.quest.Mutiny.date + " года.";
            link.l1 = "То есть как это - ушел? Он должен был быть здесь, в доках!";
    ...
    case "ShipStockManMutiny":
        ...
        link.l1 = "Проклятье! Я не отдавал такого приказа! Нельзя было доверять корабль " + NPCharSexPhrase(chref, "этому прохвосту", "этой плутовке") + "! Ну и черт с " + NPCharSexPhrase(chref, "ним", "ней") + " и с моим кораблем, сейчас уже ничего не поделать.";
...
{
    int bResult = 0;
    if (!CheckAttribute(npchar, "quest.capitainsList"))
        return bResult;
