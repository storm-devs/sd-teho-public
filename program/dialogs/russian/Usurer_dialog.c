...
int findChestMan(ref NPChar)
{
    ...
    for (n = 2; n < MAX_CHARACTERS; n++)
    {
        ...
        sTemp = ch.id;
        if (CheckAttribute(ch, "City") && ch.id != "Jackman")
            sCity = ch.City;
        else
            continue;
        ...
        if (sTemp == "Mayor")
        {
            ...
            if (sti(ch.nation) == PIRATE)
                continue; // пираты не имеют реплик
            if (ch.location == "none")
                continue;
            if (ch.location != ch.Default)
                continue; //захвачанных мэров не надо
            storeArray[howStore] = n;
            howStore++;
        }
        // верфисты
...
bool CheckUseDublons(ref NPChar)
{
    ...
    if (sti(Pchar.Ship.Type) != SHIP_NOTUSED && bOk1 && bOk2)
        return true;
