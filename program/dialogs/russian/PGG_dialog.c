...
void ProcessDialogEvent()
{
    ...
    case "officer":
        ...
        Dialog.Text = NPCharRepPhrase(NPChar, RandSwear() + RandPhraseSimple("Это - то, что нужно! Наши клинки напьются вражеской крови! Одна проблемка, мне нужно " + NPChar.Quest.Officer.Price + " песо, чтобы расплатиться с долгами.", "Ты выглядишь настоящ" + GetSexPhrase("им морским волком", "ей морской волчицей") + ". И если ты готов" + GetSexPhrase("", "а") + " заплатить " + NPChar.Quest.Officer.Price + " песо, то, пожалуй, мы сработаемся."),
        ...
        MakeRandomLinkOrderTwo(link, PCharRepPhrase(RandSwear() + RandPhraseSimple("Меня это устраивает. Но, надеюсь, ты не каждый день будешь требовать столько.", "Это чертовски дорого, но, думаю, ты того стоишь."), LinkRandPhrase("Я соглас" + GetSexPhrase("ен", "на") + ". Считай себя зачисленным в команду.", "Хм... Пожалуй, я готов" + GetSexPhrase("", "а") + " расстаться с этой суммой.", "Похоже, ты стоишь этих денег. Держи!")), "officer_hired",
                               PCharRepPhrase(RandSwear() + RandPhraseSimple("Ты не стоишь столько. Бывай...", "Это чертовски дорого, я не готов" + GetSexPhrase("", "а") + " выкидывать столько денег."),
                                              LinkRandPhrase("Да... жадность не порок...", "Да ты смеешься надо мной? За такие деньги я могу купить десяток таких, как ты.", "Это грабеж... где я возьму тебе такую сумму?")),
                               "exit");
    ...
    case "companion":
        ...
        {
            Dialog.Text = NPCharRepPhrase(NPChar, RandSwear() + RandPhraseSimple("Ты мне нравишься! По рукам! Я уже слышу звон песо в наших карманах!", "А ты " + GetSexPhrase("парень", "девка") + " не промах... Думаю, мы сработаемся!"),
    ...
    case "companion_leave":
        Dialog.Text = RandSwear() + " " + NPCharRepPhrase(NPChar, RandPhraseSimple("Ты мне надоел" + GetSexPhrase("", "а") + "!!! Я выхожу из состава эскадры!", "Мое терпение лопнуло! Я отчаливаю."), RandPhraseSimple("Я устал от ваших выходок, 'адмирал'... Разрешите откланяться.", "Хватит с меня вашего командования. Я выхожу из состава эскадры."));
    ...
    case "companion_stay":
        ...
        Dialog.Text = RandSwear() + " " + NPCharRepPhrase(NPChar, RandPhraseSimple("Премия? Отлично! Сумма в " + NPChar.Quest.Companion.Price + " песо, меня устроит.", "Пожалуй, если ты выплатишь моей команде " + NPChar.Quest.Companion.Price + " песо, я уговорю их остаться."), RandPhraseSimple("Хорошо! Я готов остаться за скромную премию в размере " + NPChar.Quest.Companion.Price + " песо!", NPChar.Quest.Companion.Price + " песо устроят на некоторое время меня и мою команду."));
    ...
    case "quest_onStay": // ПГГ сам подходит
        ...
        {
            link.l1 = PCharRepPhrase(RandSwear() + RandPhraseSimple("Я капитан " + GetFullName(pchar) + ".", "Я капитан посудины '" + PChar.Ship.Name + "' и пары сотен настоящих чертей! Хе-хе! Мое имя " + GetFullName(pchar) + ".") + " А тебя я что-то не помню.",
                                     RandPhraseSimple("Позвольте представиться, джентльмены. Я капитан " + GetFullName(pchar) + ".",
                                                      "Не имею чести быть вам представленн" + GetSexPhrase("ым", "ой") + ". Я капитан корабля '" + PChar.Ship.Name + "' под флагом " + NationNameGenitive(sti(PChar.nation)) + ". Мое имя " + GetFullName(pchar) + ".") +
