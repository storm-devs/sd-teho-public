...
void ProcessDuelDialog(ref NPChar, aref Link, aref NextDiag)
{
    ...
    {
    case "outraged":
        Dialog.Text = NPCharRepPhrase(NPChar, RandSwear() + RandPhraseSimple("Как ты смеешь меня оскорблять, мерзав" + GetSexPhrase("ец", "ка") + "?!", "Ты дорого заплатишь за свои слова!"),
        ...
        MakeRandomLinkOrderTwo(link,
                               NPCharRepPhrase(pchar,
                                               ...
                                               RandPhraseSimple("Я всегда к вашим услугам!", "Я не намерен" + GetSexPhrase("", "а") + " выбирать выражения, разговаривая с отребьем!")),
                               "outraged_1",
                               NPCharRepPhrase(pchar, RandSwear() + RandPhraseSimple("Я не это имел" + GetSexPhrase("", "а") + " в виду...", "Я погорячил" + GetSexPhrase("ся", "ась") + "..."),
                                               RandPhraseSimple("Это была ошибка. Прошу прощения.", "Я обознал" + GetSexPhrase("ся", "ась") + ", " + GetAddress_FormToNPC(NPChar) + ". Прошу меня простить.")),
                               "change_mind");
        break;
    ...
    case "outraged_1":
        ...
        MakeRandomLinkOrderTwo(link,
                               NPCharRepPhrase(pchar, RandSwear() + RandPhraseSimple("Надеюсь, твоя шпага так же быстра, как твой язык!", "За меня будет говорить мой клинок."),
                                               RandPhraseSimple("Я вызываю вас на дуэль!", "Это дело чести!")),
                               "let_s_duel",
                               NPCharRepPhrase(pchar,
                                               ...
                                               RandPhraseSimple("Я тут вспомнил" + GetSexPhrase("", "а") + ", меня ждут. Всех благ...", "Ах! Кажется, мой корабль уже отплывает.")),
                               "change_mind");
        break;
    ...
    case "change_mind":
        ...
        Dialog.Text = NPCharRepPhrase(NPChar, RandSwear() + RandPhraseSimple("Тогда проваливай отсюда!", "Скройся тогда. А то я могу передумать."),
    ...
    case "after_peace":
        Dialog.Text = NPCharRepPhrase(NPChar, RandSwear() + RandPhraseSimple("Проваливай отсюда!", "Скройся. А то я могу передумать."),
