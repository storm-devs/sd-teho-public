...
void ProcessDialogEvent()
{
        ...
        if (CheckAttribute(pchar, "questTemp.Patria") && pchar.questTemp.Patria != "fail" && pchar.questTemp.Patria != "end")
        {
            ...
            if (pchar.questTemp.Patria == "epizode_6_return" && pchar.model == "Sharle_6")
            {
                ...
                if (pchar.questTemp.HWIC.Detector == "holl_win" || pchar.questTemp.HWIC.Detector == "self_win")
                    link.l1 = "Докладываю дословно: 'передайте шевалье, что он больше не в деле на Синт-Маартене. Хватит с него. А касательно инспекции из метрополии - пусть придумает что-нибудь'. Озвучив это послание, минхер директор меня выставил из резиденции. На этом переговоры окончились.";
                else
                    link.l1 = "Передаю высказывание Стайвесанта дословно: 'ваш шеф чрезмерно зажрался на Синт-Маартене, поэтому настало время умерить его ненасытные аппетиты.' После чего меня попытались арестовать. Пришлось клинком прокладывать дорогу к кораблю. Вот так выглядит голландская дипломатия.";
            ...
            {
                if (GetQuestPastDayParam("questTemp.Patria.Curacao.Date") < 30)
                    sTemp = "скоро будет готова"; // в зависимости от времени сколько добирался
                else
                    sTemp = "уже готова";
    ...
    case "patria_8_1":
        if (sti(pchar.money) >= 300000)
            ...
            pchar.questTemp.Patria.NoMoney = "true";
    ...
    case "patria_24":
        ...
        if (ChangeCharacterHunterScore(pchar, "holhunter", 0) > 10)
            link.l1.go = "patria_25_1";
        else
            link.l1.go = "patria_25_2";
        break;
    ...
    case "patria_75":
        DialogExit();
        sTemp = "";
        if (CheckAttribute(pchar, "questTemp.Patria.Ecliaton_Bastion"))
            sTemp = "Также в моём распоряжении будет  и линейный корабль 'Эклятон'.";
    ...
    case "patria_110":
        ...
        if (pchar.questTemp.Patria == "epizode_12_baronwin")
            link.l1.go = "patria_119";
        else
            link.l1.go = "patria_111";
        break;
