...
void ProcessDialogEvent()
{
    ...
    case "Svenson_23":
        if (CheckAttribute(pchar, "questTemp.LongHappy.SvensonsShipSink"))
            dialog.text = "Ну, не сказать, чтобы совсем всё закончилось - наша то жизнь продолжается! Хотя я чувствую себя совершенной развалиной. Не в мои годы прыгать в море с охваченного огнём полуюта и битый час грести к берегу, высматривая акул. Надеюсь, у нас ещё осталась выпивка...";
        else
            dialog.text = "Ну, не сказать, чтобы совсем всё закончилось - наша то жизнь продолжается! Проклятье, да я словно помолодел лет на десять! Давненько так саблей не махал! Надеюсь, твои запасы рома и вина не растащили во время свалки - это надо хорошенько запить!";
    ...
    case "Svenson_34":
        if (CheckAttribute(pchar, "questTemp.LongHappy.Vensan_alive"))
            dialog.text = "Бернар вообще отделался парой царапин - он собирал оборону на берегу и руководил добиванием десанта из катакомб. Сейчас в гавани, организует госпитали для раненых и похороны для тех, кому не повезло.";
        else
            dialog.text = "Бернару не повезло - зачищал город после нападения из катакомб и нарвался на кучку этих поганцев на магазинных складах. Словил шальную пулю. Жаль - хороший был капитан... Тиракс будет в ярости, когда узнает.";
        link.l1 = "";
        link.l1.go = "Svenson_36"; // на завершение
        break;
    ...
    case "Svenson_48":
        if (CheckAttribute(pchar, "questTemp.LongHappy.Terrax.Prisoner") && !CheckAttribute(pchar, "questTemp.LongHappy.Terrax.Free"))
            dialog.text = "Маркуса до сих пор не нашли, но мы справимся, не волнуйся. Ты и так много сделал для всех нас, не забивай себе голову. Тиракс вернётся рано или поздно - мы его найдём.";
        else
            dialog.text = "Маркус закончил тут и умчался на чьём-то люггере на Эспаньолу. Надо решить кое-какие вопросы, вставшие после смерти Барбазона. Передавал тебе привет и наилучшие пожелания вам с женой.";
        link.l1 = "";
    ...
    case "Tichingitu":
        ...
        if (CheckAttribute(pchar, "questTemp.LongHappy.MarrySP"))
            link.l1 = "Ну, в каком-то смысле так и было, а это просто формальность. Спасибо, верный друг! Мы ещё поболтаем после - наши ребята кутят в порту с самого утра, надо показать им виновницу торжества, пока они ещё что-то видят.";
        else
            link.l1 = "Ну, в каком-то смысле так и было, а это просто формальность. Спасибо, верный друг! Мы с тобой ещё наговоримся в Шарптауне - там будет куда меньше официоза и больше душевного веселья, обещаю!";
    ...
    case "Tichingitu_1":
        ...
        if (CheckAttribute(pchar, "questTemp.LongHappy.MarrySP"))
            link.l1 = "Надеюсь, до танцев у костров сегодня не дойдёт, а без абордажа я предпочёл бы и вовсе обойтись. Ладно, держись - осталось недолго, и двинемся в таверну к нашим.";
        else
            link.l1 = "Боюсь, до танцев у костров дойдёт задолго до конца, а без абордажа я предпочел бы обойтись. Ладно, держись - осталось недолго, и сразу отплываем в Шарптаун.";
        link.l1.go = "Guests_block";
        break;
