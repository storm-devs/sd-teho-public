...
void ProcessDialogEvent()
{
            ...
            if (CheckAttribute(pchar, "questTemp.Saga"))
            {
                ...
                if (pchar.questTemp.Saga == "removebakaut") // с бакаутом разобрались
                {
                    ...
                    if (CheckAttribute(pchar, "questTemp.Saga.Late"))
                    {
                        ...
                        dialog.text = "Сударь, ну что же вы так долго! Теперь всё для вашей доверительницы потеряно. Срок действия завещания истёк, и Исла Тесоро теперь по закону принадлежит английской короне. Теперь там будет военная база." link.l1 = "Вот это да!.. Получается, все старания пошли прахом...";
                        link.l1.go = "saga_l3";
                    }
                    else
                    {
    ...
    case "saga":
        ...
        {
            RemoveItems(pchar, "map_sharp_full", 1);
            dialog.text = "Сударь, вы, наверное, давно не были на суше и не узнавали новостей? Исла Тесоро теперь - военная база английского флота! Да, завещание Шарпа существовало, однако срок, указанный в нём для предъявления прав на остров, истёк, и Исла Тесоро отошёл английской короне. Увы!" link.l1 = "Вот это да!.. Получается, все старания пошли прахом...";
            link.l1.go = "saga_l3";
        }
        else
        {
    ...
    case "saga_23":
        ...
        if (CheckAttribute(pchar, "questTemp.Saga.Late"))
        {
            dialog.text = "Сударь, вы меня поразили! Вы сделали почти всё, остров был у нас практически в руках, но из-за вашей проволочки с оплатой теперь всё для вашей доверительницы потеряно! Срок действия завещания истёк, и Исла Тесоро теперь по закону принадлежит английской короне. Теперь там будет военная база." link.l1 = "Вот это да!.. Получается, все старания пошли прахом...";
            link.l1.go = "saga_l3";
        }
        else
        {
    ...
    case "RelationAny_Done":
        ...
        if (iRate <= 10)
            sTemp = "Ну, серьёзными неприятностями это никак не назовёшь. Незначительный пустяк. Я без труда улажу ваши разногласия";
        if (iRate > 10 && iRate <= 30)
            sTemp = "Да, ваша репутация подпорчена, но ничего критичного я не вижу. Я без труда улажу ваши разногласия";
        if (iRate > 30 && iRate <= 60)
            sTemp = "Да, насолили вы властям изрядно. Придётся постараться, но я уверен, что смогу без сучка и задоринки уладить ваши разногласия";
        if (iRate > 60 && iRate <= 90)
            sTemp = "И как же это вы так умудрились, сэр? Неприятности не просто серьёзные - они очень серьёзные. Власти так и жаждут заполучить вас в свои лапы. Мне придётся очень постараться, чтобы уладить ваши разногласия";
        if (iRate > 90)
            sTemp = "Да уж... Ситуация катастрофическая - вы объявлены врагом нации номер один. Это будет крайне сложно, но я всё-таки лучший адвокат на Карибах - я улажу ваши разногласия";
