...
//--------------------------------------------поисковые квесты дворян---------------------------------------------
		...
		case "NM_peaceCap_1":
			...
			link.l1 = "Да. Вас разыскивает человек по имени "+GetFullName(sld)+". Он сказал, что "+sld.quest.text1+". Он очень хочет, чтобы вы как можно быстрее нанесли ему визит. Он живёт в "+XI_ConvertString("Colony"+npchar.quest.cribCity+"Dat")+".";
...
//<-- поисковые квесты дворян
		...
		case "AffairOfHonor_Cavalier_5":
			dialog.text = "Ах, я всё знаю, сударь! Вы даже не представляете, как я вам благодарна! Да хранит вас Господь! ";
...
string SelectNB_battleText()
{
	...
	switch (drand(5))
	{
		...
		case 3: sText = "Один негодяй пытался опорочить моё имя, оклеветав меня перед губернатором. Однако обман был расрыт, и мерзавец сбежал в море на своём корабле" break;
...
void SelectNB_prisonerText(ref npchar)
{
	switch (drand(5))
	{
		...
		break;
		case 3:
			npchar.quest.text = "Один негодяй пытался опорочить моё имя, оклеветав меня перед губернатором. Однако обман был расрыт, и мерзавец сбежал в море на одном корабле, устроившись на должность судового врача";
