void ProcessDialogEvent()
{
    ...
    case "Tuttuat_39":
        ...
        if (CheckAttribute(pchar, "questTemp.Caleuche.Skul"))
            sTemp = "Начну с посещения Кюрасао - именно там я продал нефритовый череп Жоакиму Мерриману.";
        else
            sTemp = "Туттуатхапак сообщил, что нефритовым черепом интересовался какой-то 'белый колдун' и посоветовал искать на голландских островах. Кто бы это мог быть?";
    ...
    case "Tuttuat_49":
        ...
        if (CheckAttribute(pchar, "questTemp.NatanStory"))
            link.l1 = "Ха! Кажется, я знаю, кто этот бледнолицый... Уж больно истории похожие. Калеуче был потоплен?";
        else
            link.l1 = "Хм. Так Калеуче был потоплен?";
    ...
    case "Tuttuat_61":
        ...
        if (CheckAttribute(pchar, "questTemp.NatanStory"))
        {
            ...
            AddQuestUserData("Caleuche", "sText", "Просто огромный кусок морского пространства, но делать нечего: надо прочесать указанный район.");
        break;
    ...
    case "Tuttuat_75":
        if (CheckAttribute(pchar, "questTemp.Caleuche.Potion"))
        {
            ...
            if (sti(pchar.questTemp.Caleuche.Mangarosa) == 1)
                sTemp = " Он сделать твоя движение быстрей, и твоя суметь сражаться лучше тонкий изящный оружие.";
            if (sti(pchar.questTemp.Caleuche.Mangarosa) == 2)
                sTemp = " Он сделать твоя выносливей в долгий поход, и твоя суметь сражаться лучше острый сабля.";
    ...
    case "monk_caleuche_7":
        ...
        if (CheckAttribute(pchar, "questTemp.Caleuche.Skul"))
            sTemp = "посетить дом моего старого знакомца Жоакима Мерримана, в котором, по его словам, по ночам творятся странные вещи. Вспоминая тот особенный блеск в глазах Мерримана, когда тот смотрел на нефритовый череп, мне становится жутковато. Как-то мне совсем не улыбается идти ночью в его бывшее обиталище.";
        else
            sTemp = "посетить дом напротив резиденции, в котором снимал комнату некий Жоаким Мерриман. Судя по всему, это и есть тот самый белый колдун, о котором сообщил Туттуатхапак. По словам монаха, по ночам в доме творятся странные вещи. Как-то мне совсем не улыбается идти ночью в бывшее обиталище колдуна.";
        AddQuestUserData("Caleuche", "sText", sTemp);
