...
void ProcessDialogEvent()
{
    ...
    case "parol_pay":
        ...
        if (pchar.questTemp.LSC.parol == "ривадос")
    ...
    case "whiskey_5":
        ...
        link.l1 = "Что?! Говори немедленно, каким образом он планирует это сделать? К ней в каюту на 'Церес Смити' не попасть, у нее заперты все двери! Как он собирается убить её?!";
    ...
    case "narval":
        ...
        if (sti(pchar.GenQuest.NarvalConflict) < 3)
            dialog.text = "Ну, это ещё не конфликт - так, небольшое недоразумение, хе-хе. " + sti(npchar.quest.price) + " дублонов - и ваша проблема будет решена через сутки.";
        else
        {
            if (sti(pchar.GenQuest.NarvalConflict) >= 3 && sti(pchar.GenQuest.NarvalConflict) < 10)
                dialog.text = "Как же, как же, наслышан. Драку вы учинили знатную, мой дорогой. Но выход есть. " + sti(npchar.quest.price) + " дублонов - и ваша проблема будет решена через сутки.";
            else
                dialog.text = "Уже весь Остров знает о резне, которую вы учинили. Сложно будет вам помочь, но ситуация не безвыходная. " + sti(npchar.quest.price) + " дублонов - и я попробую решить вашу проблему.";
        }
