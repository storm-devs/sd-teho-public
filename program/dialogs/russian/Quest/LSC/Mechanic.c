...
void ProcessDialogEvent()
{
    ...
    case "mechanic_10":
        ...
        link.l1 = "Тогда покажите мне кусок этого металла, чтобы я мог понять, о чём идет речь.";
        link.l1.go = "mechanic_11";
        break;

    case "mechanic_11":
        dialog.text = "Так вы беретесь за розыски? Превосходно. Вот, возьмите этот кусок как образец для сравнения. Ищите среди серебряной руды и самородков. Отличить его на первый взгляд непросто, но вы быстро разберётесь, что к чему. На серебре практически всегда есть хотя бы небольшие потемнения. А этот металл чист.";
    ...
    case "mechanic_12":
        ...
        dialog.text = "Увы, нет. Если бы я знал - то ваша помощь мне бы не потребовалась. Но совет все же дам: поищите на кораблях клана ривадос - я на них не бываю, соответственно, шанс нахождения там больше, ибо корабли нарвалов я уже облазил вдоль и поперёк.";
    ...
    case "mechanic_24":
        ...
        if (!CheckAttribute(pchar, "questTemp.LSC.NatanTalk"))
            string sText = "Но прежде чем осуществить погружение, я должен обязательно найти Натаниэля Хоука и поговорить с ним."; //не было разговора с Натаном
        else
            sText = "";
