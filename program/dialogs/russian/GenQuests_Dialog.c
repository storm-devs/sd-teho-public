void ProcessDialogEvent()
{
    ...
    case "ShipWreck_38":
        ...
        if (!CheckAttribute(pchar, "GenQuest.ShipWreckingRec")) // лесник
        {
            pchar.GenQuest.ShipWreckingRec = true; // лесник - вместо CaptainComission.CrazyRec  = pchar.GenQuest.ShipWreckingRec
            AddQuestRecord("ShipWrecked", "7");    // иначе генер поручение капитана не брался.
        }
        DialogExit();
        break;
    ...
    case "ShipWreck_40":
        pchar.quest.ShipWreck_DeliveToCity.over = "yes";
        if (CheckAttribute(pchar, "GenQuest.ShipWreckingRec"))
            DeleteAttribute(pchar, "GenQuest.ShipWreckingRec"); // лесник.
    ...
    case "ShipWreck_43":
        ...
        AddSimpleRumourCity(LinkRandPhrase(RandPhraseSimple("Говорят, что какой-то псих на острове объявился. Рассказывает, что потерял документы и пытается наняться капитаном на корабль до Дюнкерка...",
                                           ...
                                           "Вас недавно какой-то псих искал, говорил, что якобы Вы ему денег должны..."),
                            pchar.location, 5, 3, "");
        DialogExit();
        AddDialogExitQuestFunction("ShipWreck_GoOut");
        break;
    ...
    case "EncGirl_Lover6":
        AddSimpleRumourCityTip(RandPhraseSimple("Вы слышали, " + pchar.GenQuest.EncGirl.name + " сбежала с любовником из-под венца. Не захотела свою жизнь с инвалидом связывать. А во всём виноваты её родители - прельстились на деньги жениха, незачем было такую красавицу за урода сватать.",
                                                "Слыхали? " + pchar.GenQuest.EncGirl.name + " оставила с носом и своего папашу и новоиспечённого жениха. Говорят, что один благородный капитан помог ей с любовником бежать на другой остров, где живут родители её возлюбленного... Эх, есть ведь люди, которые ещё не забыли что такое страсть!"),
                               pchar.GenQuest.EncGirl.LoverCity, 3, 2, "citizen,habitue", "");
    ...
    case "CaptainComission_73":
        ...
        for (i = 0; i < 3; i++)
        {
            if (GetCharacterIndex("SlaveGuard_" + i) == -1)
                continue;
    ...
    case "CaptainComission_387":
        ...
        if (sti(pchar.GenQuest.CaptainComission.MaxGoodsQty) > sti(pchar.GenQuest.CaptainComission.GoodsQty))
            pchar.GenQuest.CaptainComission.MaxGoodsQty = pchar.GenQuest.CaptainComission.GoodsQty;
    ...
    case "CaptainComission_391":
        ...
        if (sti(pchar.GenQuest.CaptainComission.CapGoodsQty) > sti(pchar.GenQuest.CaptainComission.MaxGoodsQty))
            pchar.GenQuest.CaptainComission.CapGoodsQty = pchar.GenQuest.CaptainComission.MaxGoodsQty;
        if (CheckAttribute(pchar, "GenQuest.CaptainComission.MayorTalkGood"))
            AddQuestRecord("CaptainComission2", "53"); // лесник. выбор записи в СЖ
        else
            AddQuestRecord("CaptainComission2", "26"); // смотря по какому пути пошел
    ...
    case "CaptainComission_404":
        ...
        if (NPChar.alignment == "good" && sti(pchar.reputation.nobility) > 50)
            bTemp = true;
        if (NPChar.alignment == "bad" && sti(pchar.reputation.nobility) <= 50)
            bTemp = true;
    ...
    case "ContraDeliverQuest_Sold1":
        ...
        //{
        link.l1 = "Офицер, поверьте, вы совершаете ужастную ошибку и пытаетесь арестовать ни в чем не повинных людей. Поверьте, я совсем не знаю этого человека и, более того, не желаю его знать." + " Я думаю, ваш комендант будет очень недоволен, если вы арестуете двух законопослушных граждан...";
...
int PiratesOnUninhabited_GenerateShipType()
{
    ...
    if (rank < 5)
    {
        ...
        {
        case 0:
            retShipType = SHIP_LUGGER;
            break;
        case 1:
            retShipType = SHIP_SLOOP;
            break;
        }
    }
    ...
    if (rank >= 5 && rank < 20)
    {
        ...
        case 0:
            ...
            break;
        case 1:
            retShipType = SHIP_BRIGANTINE;
            break;
        case 2:
            retShipType = SHIP_SCHOONER_W;
            break;
        case 3:
            retShipType = SHIP_BRIG;
            break;
        }
    }
    ...
    if (rank >= 20)
    {
        ...
        {
        case 0:
            retShipType = SHIP_CORVETTE;
            break;
        case 1:
            retShipType = SHIP_GALEON_H;
            break;
        case 2:
            retShipType = SHIP_FRIGATE;
            break;
        }
    }
