    ...
    if (CheckAttribute(loc, "type"))
    {
        ...
        case "town":
            ...
            if (iColony != -1)
            {
                ...
                {
                    if (Whr_IsDay())
                        SetMusicAlarm(NationShortName(sti(Colonies[iColony].nation)) + "_music_day");
                    else
                        SetMusicAlarm(NationShortName(sti(Colonies[iColony].nation)) + "_music_night");
                }
            }
            else
            {
        ...
        case "sailing_cabine":
            ...
            if (Whr_IsDay())
                SetMusicAlarm("music_sea_day");
            else
                SetMusicAlarm("music_sea_night");
            break;
        ...
        case "Judgement_hell_dungeon":
            ...
            if (loc.id == "Judgement_dungeon_10")
                SetMusicAlarm("music_retribution_1");
            else
                SetMusicAlarm("music_retribution");
            break;
        }
    }
    SetStaticSounds(loc);
...
{
    if (IsEntity(loc) != true)
        return;
    string locatorGroupName = "locators.sound";
    if (CheckAttribute(loadedLocation, locatorGroupName) == 0)
        return;
    aref locatorGroup;
    ...
    for (int i = 0; i < locatorCount; i++)
    {
        ...
        {
            if (Whr_IsDay() && loc.type != "Dungeon" && loc.type != "cave" && loc.type != "fort_attack")
                continue;
        }
...
void SetMusic(string name)
{
    if (pchar.location == "UnderWater")
        return; //не играть музон под водой
    InitSound();
...
void SetMusicAlarm(string name)
{
    ...
    {
        if (loadedLocation.type == "x_jungle" || loadedLocation.type == "x_seashore" || loadedLocation.id.label == "Maze")
            SetMusic("music_teno"); // калеуче
        else
        {
            if (loadedLocation.id.label == "Alcove")
            {
                if (loadedLocation.type == "Alcove")
            ...
            {
                if (loadedLocation.id == "shore67" || loadedLocation.id == "IslaDeVieques_HouseEntrance" || loadedLocation.id == "Shore_mask")
