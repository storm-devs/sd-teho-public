    ...
    if (CheckAttribute(loc, "type"))
    {
        if (loc.type == "jungle")
        {
            ...
            if (rand(100) < 80)
            {
                ...
                if (!IsDay())
                    crtAnimals = false;
                if (Whr_GetWindSpeed() > 10.0)
                    crtAnimals = false;
                if (Whr_IsRain())
                    crtAnimals = false;
                if (Whr_IsStorm())
                    crtAnimals = false;
                if (Whr_GetFogDensity() > 0.008)
                    crtAnimals = false;
                if (crtAnimals)
                {
...
bool LocLoadModel(aref loc, string sat, string addition)
{
    ...
    if (CheckAttribute(loc, attr))
        tech = loc.(attr);
    if (tech == "LocationModelBlend" && dynamicLightsOn)
        tech = "DLightModel";
    ...
    attr = sat + ".lights";
    if (CheckAttribute(loc, attr))
        dynamicLightsOn = MakeInt(loc.(attr));
    //Грузим модельку
    ...
    if (CheckAttribute(loc, attr) != 0)
    {
        ...
        attr1 = attr + ".x";
        if (CheckAttribute(loc, attr1) != 0)
            x = MakeFloat(loc.(attr1));
        attr1 = attr + ".y";
        if (CheckAttribute(loc, attr1) != 0)
            y = MakeFloat(loc.(attr1));
        attr1 = attr + ".z";
        if (CheckAttribute(loc, attr1) != 0)
            z = MakeFloat(loc.(attr1));
        res = SendMessage(loc, "lfff", MSG_LOCATION_MODEL_SET_ROT, x, y, z);
        if (res == 0)
            Trace("Can't set rotate modifier to model: " + loc.(sat));
    }
    //uvslide
    attr = sat + ".uvslide";
    if (CheckAttribute(loc, attr) != 0)
    {
        ...
        attr1 = attr + ".u0";
        if (CheckAttribute(loc, attr1) != 0)
            u0 = MakeFloat(loc.(attr1));
        attr1 = attr + ".v0";
        if (CheckAttribute(loc, attr1) != 0)
            v0 = MakeFloat(loc.(attr1));
        attr1 = attr + ".u1";
        if (CheckAttribute(loc, attr1) != 0)
            u1 = MakeFloat(loc.(attr1));
        attr1 = attr + ".v1";
        if (CheckAttribute(loc, attr1) != 0)
            v1 = MakeFloat(loc.(attr1));
        res = SendMessage(loc, "lffff", MSG_LOCATION_MODEL_SET_UVS, u0, v0, u1, v1);
        if (res == 0)
            Trace("Can't set uvslide modifier to model: " + loc.(sat));
    }
    //rotate
    attr = sat + ".lamps";
    if (CheckAttribute(loc, attr) != 0)
...
void LocLoadShips(ref Location)
{
    ...
    if (bMainCharacterHere)
    {
        ...
        {
            if (locNumShips >= MAX_SHIPS_IN_LOCATION)
                continue;
    ...
    for (int n = 0; n < locNumShips; n++)
    {
        ...
        if (CreateEntity(&locShips[n], "ship") == 0)
            break;
    ...
    if (CheckAttribute(Location, "locators.reload.boat"))
    {
        if (bMainCharacterHere && iMainCharacterShipType != SHIP_NOTUSED)
        {
            if (CreateEntity(&locShips[n], "ship"))
            {
                ...
                if (!CheckAttribute(rPlayer, "Flags.Pirate"))
                    rCharacter.Flags.Pirate = rand(2);
                else
                    rCharacter.Flags.Pirate = rPlayer.Flags.Pirate;
                Ship_SetLightsAndFlares(rCharacter);
                Ship_PrepareShipForLocation(rCharacter);
                SendMessage(&locShips[n], "laa", MSG_SHIP_CREATE, &rCharacter, &rShip);
...
bool Character_LocIsEntryLocation(ref rCharacter, ref rLocation)
{
    ...
    {
        if (rCharacter.location.from_sea == rLocation.id)
            return true;
    }
    return false;
}
...
bool LocIsEntryLocation(ref Location)
{
    ...
    if (rPlayer.location.from_sea != Location.id)
        return false;
    return true;
}
