...
*/
    ...
    if (CheckAttribute(Location, "locators.burn")) // Jason
    {
        ...
        for (n = 0; n < num; n++)
        {
            ...
            if (CheckAttribute(Location, "hidden_sound"))
                SendMessage(Sound, "lsllllllfff", MSG_SOUND_PLAY, "fortfire", SOUND_WAV_3D, VOLUME_FX, 0, 1, 0, 0, stf(locator.x), stf(locator.y), stf(locator.z));
        }
    }
