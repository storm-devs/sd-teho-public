...
int FindEncounter(int type, int nat)
{
    ...
    for (i = 0; i < MAX_ENCOUNTER_TYPES * 10; i++)
        iTypes[i] = -1;
    int iNumTypes = 0;
    ...
    for (i = 0; i < MAX_ENCOUNTER_TYPES; i++)
    {
        ...
        {
            if (sti(EncountersTypes[i].Skip) || !Encounter_CanNation(i, nat))
            {
                continue;
            }
            // check chance
            if (iChance > sti(EncountersTypes[i].Chance))
            {
                continue;
            }
    ...
    while (findnum >= 0 && iTypes[findnum] == -1)
        findnum--;
...
bool Encounter_CanNation(int iEncounter, int iNation)
{
    ...
    if (!CheckAttribute(&EncountersTypes[iEncounter], sNation))
        return true;
