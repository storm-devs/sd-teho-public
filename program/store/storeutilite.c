...
void AddStoreGoods(ref _refStore, int _Goods, int _Quantity)
{
    ...
    int q = 0;
    if (CheckAttribute(refGoods, "Quantity"))
    {
        q = sti(refGoods.Quantity);
    }
...
int GetStoreGoodsPrice(ref _refStore, int _Goods, int _PriceType, ref chref, int _qty)
{
    ...
    if (_PriceType == PRICE_TYPE_BUY) // цена покупки товара игроком
    {
        ...
        if (tradeType == T_TYPE_CANNONS)
            cModify = 3.0 + MOD_SKILL_ENEMY_RATE / 5.0;
        if (CheckCharacterPerk(chref, "HT2"))
        {
            if (CheckOfficersPerk(chref, "ProfessionalCommerce"))
            {
                skillModify -= 0.20;
            }
            else
    ...
    // boal 23.01.2004 -->
    if (MakeInt(basePrice * tradeModify * skillModify * costCoeff + 0.5) < 1)
        return 1;
    // boal 23.01.2004 <--
...
void UpdateStore(ref pStore)
{
    ...
    for (int i = 0; i < GOODS_QUANTITY; i++)
    {
        ...
        {
            if (Aim > 0.70)
                Aim = -1.0; // товар увезли
            else
                Aim = 0.0; // увоза-привоза нет
        }
        ...
        {
            Manufacture = 0.2;
        } // производим в любой день, когда товар не вывозили
        if (oldQty > makeint(0.25 * Norm))
        {
            Consumption = 0.1;
        } // потребляем
        ...
        {
            oldQty = 0;
        } // ибо нехер портить нам наполнение магазина!
        ...
        else
        {
            ...
            crunch = frnd();
            if (crunch < 0.15)
                curref.Quantity = makeint(Norm * frnd()); // самосвал или "к нам едет ревизор!!"
            else
                curref.Quantity = makeint(Norm * (1.75 + frnd() * 0.25));
        }
        if (sti(curref.Quantity) < 0)
            curref.Quantity = 0; // может быть и меньше 0
        // <<<--  новая система затаривания товаром в магазинах

        if (sti(Goods[i].type) == T_TYPE_UNIQUE || sti(Goods[i].type) == T_TYPE_CROWN)
            curref.Quantity = 0;
...
void StoreDayUpdate()
{
    if (storeDayUpdateCnt >= 0)
    {
        ...
        storeDayUpdateCnt++;
        if (storeDayUpdateCnt >= STORE_QUANTITY)
            storeDayUpdateCnt = -1;
        PostEvent("EvStoreDayUpdate", 200);
    }
}
