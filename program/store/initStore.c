        ...
        if (islandIdx != -1)
        {
            ...
            else
            {
                ...
                if (sti(rColony.ismaincolony) == 1 ||
                    sti(rColony.nation) == PIRATE)
                    RndPriceModifySign = 1.0;
            ...
            for (i = 0; i < nq; i++)
            {
                ...
                case "Export":
                    tt = T_TYPE_EXPORT;
                    break; // экспортные товары
                case "Import":
                    tt = T_TYPE_IMPORT;
                    break; // импортные товары
                case "Aggressive":
                    tt = T_TYPE_AGGRESSIVE;
                    break; // товары агрессивного спроса
                case "Contraband":
                    tt = T_TYPE_CONTRABAND;
                    break; // контрабанда
                }
