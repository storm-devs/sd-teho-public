...
bool LoadMainCharacterInFirstLocation(string location_id, string emerge_locator, string ship_location)
{
    ...
    if (emerge_locator != "")
    {
        for (int i = 0; i < num; i++)
        {
            ...
            if (emerge_locator == locator_ref.name)
                break;
        }
        if (i == num)
        {
...
int Reload(aref reload_group, string locator_name, string current_location)
{
    ...
    for (int i = 0; i < num; i++)
    {
        ...
        if (locator_name == reload_locator_ref.name)
            break;
    }
    if (i == num)
    {
    ...
    else
    {
        ...
        if (loc_pict_index < 0)
            loc_pict_index = reload_cur_location_index;
        if (loc_pict_index >= 0)
        {
    ...
    if (CheckAttribute(&locations[location_index], "changeAnimation"))
    {
        if (locations[location_index].changeAnimation == "man_A" && CheckAttribute(mc, "defaultanimation") && mc.defaultanimation != "")
        {
            ...
            if (CheckAttribute(mc, "IsMushketer"))
                mc.model.animation = "mushketer"; // patch-3
            else
...
bool TeleportCharacterFromCurLocationToLocation(string locatorExit, string group, string locator)
{
    ...
    if (Reload(reload_group, locatorExit, loadedLocation.id) == false)
        return false;
