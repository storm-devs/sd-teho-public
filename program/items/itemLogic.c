...
void Item_OnUseFrame()
{
    ...
    else
    {
        if (MOD_BETTATESTMODE == "On")
            Log_Info("Error: Item_OnUseFrame нет mch.itemLocationIndex ");
    }
    if (!usedOnFrame)
...
void RandItems_SpawnRandItem(int _index, aref _location, aref al)
{
    ...
    if (GetCharacterSkillToOld(GetMainCharacter(), SKILL_FORTUNE) < rand(15))
        return; // boal fix
...
void Box_EnterToLocator(aref loc, string locName)
{
    ...
    if (HasSubStr(locName, "private"))
    {
        ...
        if (!CheckAttribute(loc, locName + ".opened"))
        {
            ...
            {
                bool bOk = (CheckAttribute(loc, locName + ".skel")) && (CheckCharacterItem(pchar, "keys_skel")) if (CheckCharacterItem(pchar, loc.(locName).key) || bOk) //проверяем ключ и отмычки
                {
                    loc.(locName).opened = true;
                    PlaySound("interface\key.wav");
                    if (!CheckCharacterItem(pchar, loc.(locName).key))
                        Log_Info("Замок открыт отмычкой!");
                ...
                else
                {
                    if (CheckCharacterItem(pchar, "keys_skel"))
                        Log_Info("Замок отмычками не открывается");
...
void OpenBoxProcedure()
{
    ...
    {
        if (CheckAttribute(chr, "questTemp.Guardoftruth.ArchyBoom") && chr.location == "IslaDeVieques_House" && atrName == "box1")
            GuardOT_ArchyChestBoom(); // Jason 070712
        else
        {
            if (CheckAttribute(chr, "questTemp.Ksochitam.ShoreShip") && chr.location == "Shore_ship4")
                Ksochitam_ShoreshipOpenBox(); // Jason 220712
            else
            {
                // сюда можно ставить квестовые реакции при открытии сундука с выводом интерфейса содержимого
                if (CheckAttribute(chr, "questTemp.Mtraxx.Grotbox") && chr.location == "PortoBello_Cave" && atrName == "box1")
                    Mtraxx_WolfreekGrotOpenBox(); // Addon 2016-1 Jason Пиратская линейка
                if (CheckAttribute(chr, "questTemp.Mtraxx.Cavebox") && chr.location == "Panama_Cave" && atrName == "box1")
                    Mtraxx_WolfreekCaveOpenBox();
                if (CheckAttribute(chr, "questTemp.Mtraxx.WolfreekBox") && chr.location == "IslaMona_WaterCave" && atrName == "private1")
                    Mtraxx_WolfreekFindTreasure();
                if (CheckAttribute(chr, "questTemp.Mtraxx.Pasquale.Grabbing") && chr.location == "LaVega_TwoFloorHouse" && atrName == "private1")
                    Mtraxx_PasqualeCheckChest();
                LaunchItemsBox(&ar);
            }
        }
    }
