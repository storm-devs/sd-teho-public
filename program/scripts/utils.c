    ...
    if (ChangeCharacterNationReputation(pchar, iNation, 0) < 0)
    {
        if (nowrate <= 20)
        {
            ...
            if (rate <= nowrate)
                ...
                ChangeCharacterNationReputation(pchar, iNation, nowrate);
        }
        else
            ChangeCharacterNationReputation(pchar, iNation, 20);
    }
    else
        SetNationRelation2MainCharacter(iNation, RELATION_NEUTRAL);

    ref sld = characterFromId("Fadey");
    if (CheckAttribute(sld, "quest.relation"))
        DeleteAttribute(sld, "quest.relation");
    if (CheckAttribute(pchar, "GenQuest.FadeyNation"))
        DeleteAttribute(pchar, "GenQuest.FadeyNation");
...
void ChangeNationRelationFromBenuaComplete(string sQuest) // 141012
{
    ...
    if (CheckAttribute(sld, "quest.relation"))
        DeleteAttribute(sld, "quest.relation");
    if (CheckAttribute(pchar, "GenQuest.BenuaNation"))
        DeleteAttribute(pchar, "GenQuest.BenuaNation");
...
{
    string sNation = "RelationAgentRate" + GetNationNameByType(iNation);
    Pchar.GenQuest.(sNation) = abs(ChangeCharacterNationReputation(pchar, iNation, 0)) int iSumm = sti(Pchar.GenQuest.(sNation)) * 1500 + makeint(stf(Pchar.rank) / stf(Pchar.reputation.nobility) * 100000);
...
void GenerateMaps(aref ch, int iProbability1, int iProbability2)
{
    ...
    if (rand(4) == 1)
        AddItems(ch, "map_bad", 1);
    if (rand(6) == 1 && rank >= 10)
        AddItems(ch, "map_normal", 1); // patch-5
    if (rand(iProbability1) == 1 && !CheckMainHeroMap("map_jam"))
        AddItems(ch, "map_jam", 1);
    if (rand(iProbability2) == 1 && !CheckMainHeroMap("map_cayman"))
        AddItems(ch, "map_cayman", 1);
    if (rand(iProbability1) == 1 && !CheckMainHeroMap("map_barbados"))
        AddItems(ch, "map_barbados", 1);
    if (rand(iProbability1) == 1 && !CheckMainHeroMap("map_trinidad"))
        AddItems(ch, "map_trinidad", 1);
    if (rand(iProbability1) == 1 && !CheckMainHeroMap("map_Curacao"))
        AddItems(ch, "map_Curacao", 1);
    if (rand(iProbability1) == 1 && !CheckMainHeroMap("map_martiniqua"))
        AddItems(ch, "map_martiniqua", 1);
    if (rand(iProbability2) == 1 && !CheckMainHeroMap("map_dominica"))
        AddItems(ch, "map_dominica", 1);
    if (rand(iProbability1) == 1 && !CheckMainHeroMap("map_puerto"))
        AddItems(ch, "map_puerto", 1);
    if (rand(iProbability1) == 1 && !CheckMainHeroMap("map_cuba"))
        AddItems(ch, "map_cuba", 1);
    if (rand(iProbability1) == 1 && !CheckMainHeroMap("map_hisp"))
        AddItems(ch, "map_hisp", 1);
    if (rand(iProbability1) == 1 && !CheckMainHeroMap("map_nevis"))
        AddItems(ch, "map_nevis", 1);
    if (rand(iProbability1) == 1 && !CheckMainHeroMap("map_guad"))
        AddItems(ch, "map_guad", 1);
    if (rand(iProbability1) == 1 && !CheckMainHeroMap("map_antigua"))
        AddItems(ch, "map_antigua", 1);
    if (rand(iProbability1) == 1 && !CheckMainHeroMap("map_TORTUGA"))
        AddItems(ch, "map_TORTUGA", 1);
    if (rand(iProbability2) == 1 && !CheckMainHeroMap("map_terks"))
        AddItems(ch, "map_terks", 1);
    if (rand(iProbability1) == 1 && !CheckMainHeroMap("map_sm"))
        AddItems(ch, "map_sm", 1);
    if (rand(iProbability1) == 1 && !CheckMainHeroMap("map_bermudas"))
        AddItems(ch, "map_bermudas", 1);
    if (rand(iProbability2) == 1 && !CheckMainHeroMap("map_Pearl"))
        AddItems(ch, "map_Pearl", 1);
    if (rand(iProbability1) == 1 && !CheckMainHeroMap("map_beliz"))
        AddItems(ch, "map_beliz", 1);
    if (rand(iProbability1) == 1 && !CheckMainHeroMap("map_santa"))
        AddItems(ch, "map_santa", 1);
    if (rand(iProbability2) == 1 && !CheckMainHeroMap("map_maine_1"))
        AddItems(ch, "map_maine_1", 1);
    if (rand(iProbability2) == 1 && !CheckMainHeroMap("map_maine_2"))
        AddItems(ch, "map_maine_2", 1);
    if (rand(iProbability1) == 1 && !CheckMainHeroMap("map_panama"))
        AddItems(ch, "map_panama", 1);
    if (rand(iProbability1) == 1 && !CheckMainHeroMap("map_maracaibo"))
        AddItems(ch, "map_maracaibo", 1);
    if (rand(iProbability1) == 1 && !CheckMainHeroMap("map_cumana"))
        AddItems(ch, "map_cumana", 1);
}
...
void GiveItemToTrader(aref ch)
{
    ...
    if (CheckAttribute(ch, "Merchant.type"))
        merType = ch.Merchant.type;
    else
        merType = "veteran"; // если типа нет то продаем скобяные изделия
    ...
    case "potion": // лекарства и травы + обереги
        ...
        if (irand == 0)
            AddItems(ch, "cannabis1", Rand(1) + 1);
        if (irand == 2)
            AddItems(ch, "cannabis2", Rand(1) + 1);
        if (irand == 4)
            AddItems(ch, "cannabis3", Rand(1) + 1);
        if (irand == 6)
            AddItems(ch, "cannabis4", Rand(1) + 1);
        if (irand == 8)
            AddItems(ch, "cannabis5", Rand(1) + 1);
        if (irand == 10)
            AddItems(ch, "cannabis6", Rand(1) + 1);
        // рецепты
        ...
        if (irand == 40)
            AddItems(ch, "recipe_totem_01", 1);
        if (irand == 80)
            AddItems(ch, "recipe_totem_02", 1);
        if (irand == 100)
            AddItems(ch, "recipe_totem_03", 1);
        if (irand == 120)
            AddItems(ch, "recipe_totem_04", 1);
        if (irand == 150)
            AddItems(ch, "recipe_totem_05", 1);
        if (irand == 180)
            AddItems(ch, "recipe_totem_06", 1);
        if (irand == 210)
            AddItems(ch, "recipe_totem_07", 1);
        if (irand == 230)
            AddItems(ch, "recipe_totem_08", 1);
        if (irand == 250)
            AddItems(ch, "recipe_totem_09", 1);
        if (irand == 270)
            AddItems(ch, "recipe_totem_10", 1);
        if (irand == 290)
            AddItems(ch, "recipe_totem_11", 1);
        if (irand == 300)
            AddItems(ch, "recipe_totem_12", 1);
        if (irand == 320)
    ...
    case "veteran": // скобяные изделия
        ...
        if (irand == 2)
            AddItems(ch, "mineral5", Rand(1) + 1);

        irand = drand(6);
        if (irand == 3)
            AddItems(ch, "mineral6", Rand(2) + 1);
        if (irand == 4)
            AddItems(ch, "mineral7", Rand(2) + 1);
        if (irand == 5)
            AddItems(ch, "mineral9", Rand(2) + 1);
        if (irand == 6)
            AddItems(ch, "mineral10", Rand(2) + 1);

        irand = drand(4);
        if (irand == 0)
            AddItems(ch, "mineral13", Rand(1) + 1);
        if (irand == 1)
            AddItems(ch, "mineral15", Rand(1) + 1);
        if (irand == 2)
            AddItems(ch, "mineral16", Rand(1) + 1);

        irand = drand(8);
        if (irand == 1)
            AddItems(ch, "mineral18", Rand(1) + 1);
        if (irand == 8)
            AddItems(ch, "mineral19", Rand(1) + 1);
        if (irand == 5)
            AddItems(ch, "mineral20", Rand(1) + 1);

        irand = drand(9);
        if (irand == 9)
            AddItems(ch, "mineral21", Rand(1) + 1);
        if (irand == 7)
            AddItems(ch, "mineral22", Rand(5) + 2);
        if (irand == 5)
            AddItems(ch, "mineral23", Rand(12) + 3);
        if (irand == 3)
            AddItems(ch, "mineral24", Rand(1) + 1);

        irand = drand(15);
        if (irand == 15)
            AddItems(ch, "mineral25", Rand(1) + 2);
        if (irand == 5)
            AddItems(ch, "mineral26", Rand(1) + 3);
        if (irand == 7)
            AddItems(ch, "mineral33", Rand(1) + 1);
        if (irand == 9)
            AddItems(ch, "mineral35", 1);

        // амулеты
        irand = drand(25);
        if (irand == 1)
            AddItems(ch, "obereg_5", 1);
        if (irand == 15)
            AddItems(ch, "obereg_4", 1);
        if (irand == 25)
            AddItems(ch, "obereg_3", 1);
        break;
    ...
    case "armourer": // оружие и боеприпасы
        ...
        irand = drand(15);
        if (irand == 1 && rank >= 6)
            AddItems(ch, "spyglass2", 1); // fix 160912
        irand = drand(4);
        // пистоли
        if (irand == 1)
        ...
        irand = drand(3);
        if (irand == 1)
            GenerateAndAddItems(ch, "slave_01", Rand(2) + 1);
        if (irand == 2)
            GenerateAndAddItems(ch, "slave_02", Rand(2) + 1);
        if (irand == 3)
        ...
        if (rank >= 5)
        {
            ...
            if (irand == 2)
                GenerateAndAddItems(ch, "blade_05", Rand(1) + 1);
            if (irand == 3)
                GenerateAndAddItems(ch, "blade_07", Rand(1) + 1);
            if (irand == 4)
                GenerateAndAddItems(ch, "blade_08", Rand(1) + 1);
            if (irand == 5)
                GenerateAndAddItems(ch, "blade_09", Rand(1) + 1);
        }
        // кирасы
        irand = drand(10);
        if (irand == 1 && rank >= 4)
            AddItems(ch, "cirass1", 1);
        if (irand == 5 && rank >= 6)
            AddItems(ch, "cirass5", 1);
        irand = drand(15);
        if (irand == 1 && rank >= 8)
            AddItems(ch, "cirass2", 1);
        if (irand == 7 && rank >= 10)
            AddItems(ch, "cirass6", 1);
        irand = drand(20);
        if (irand == 1 && rank >= 12)
            AddItems(ch, "cirass3", 1);
        if (irand == 8 && rank >= 14)
        ...
        if (rank >= 7)
        {
            irand = drand(9);
            if (irand == 1)
                GenerateAndAddItems(ch, "blade_11", Rand(1) + 1);
            if (irand == 3)
                GenerateAndAddItems(ch, "blade_12", Rand(1) + 1);
            if (irand == 5)
                GenerateAndAddItems(ch, "blade_14", Rand(1) + 1);
        }
        // хорошие сабельки
        if (rank >= 10)
        {
            irand = drand(12);
            if (irand == 1)
                GenerateAndAddItems(ch, "blade_13", 1);
            if (irand == 4)
                GenerateAndAddItems(ch, "blade_04", 1);
            if (irand == 7)
                GenerateAndAddItems(ch, "blade_06", 1);
            if (irand == 10)
                GenerateAndAddItems(ch, "blade_10", 1);
            if (irand == 12)
                GenerateAndAddItems(ch, "blade_13", 1);
        }
        // приборы
        irand = drand(16);
        if (irand == 5 && rank >= 5)
            AddItems(ch, "compass1", 1);
        if (irand == 1 && rank >= 5)
            AddItems(ch, "sand_clock", 1);
        if (irand == 10 && rank >= 7)
            AddItems(ch, "astrolab", 1);
        if (irand == 15 && rank >= 7)
            AddItems(ch, "clock1", 1);
        // рецепты
        ...
        irand = drand(30);
        if (irand == 5)
            AddItems(ch, "obereg_1", 1);
        if (irand == 15)
            AddItems(ch, "obereg_2", 1);
        if (irand == 30)
            AddItems(ch, "obereg_6", 1);
        irand = drand(6);
        if (irand == 0)
            AddItems(ch, "Mineral15", drand(2) + 1);
        if (irand == 3)
            AddItems(ch, "Mineral13", drand(2) + 1);
        if (irand == 6)
            AddItems(ch, "Mineral30", drand(5) + 1);
        break;
    ...
    case "monk":                                //  амулеты+всякая мурня, карты убрал
        ...
        irand = drand(10);
        if (irand == 1)
            AddItems(ch, "Mineral5", 1);
        irand = drand(5);
        if (irand == 1)
            AddItems(ch, "Mineral11", 1);
        irand = drand(12);
        if (irand == 1)
            AddItems(ch, "Mineral21", 1);
        irand = drand(7);
        if (irand == 1)
            AddItems(ch, "Mineral1", 1);
        irand = drand(4);
        if (irand == 1)
            AddItems(ch, "potionwine", drand(3));
        irand = drand(5);
        if (irand == 0)
            AddItems(ch, "jewelry45", drand(2) + 1);
        if (irand == 1)
            AddItems(ch, "jewelry48", drand(2) + 1);
        if (irand == 2)
            AddItems(ch, "jewelry46", drand(2) + 1);
        if (irand == 3)
            AddItems(ch, "jewelry43", 1);
        if (irand == 4)
            AddItems(ch, "jewelry41", 1);
        if (irand == 5)
            AddItems(ch, "jewelry40", 1);
        irand = drand(3);
        if (irand == 1)
            AddItems(ch, "jewelry50", 1);
        if (irand == 2)
            AddItems(ch, "jewelry49", 1);
        if (irand == 3)
            AddItems(ch, "jewelry47", 1);
        // церковные амулеты
        irand = drand(15);
        if (irand == 1)
        ...
        if (irand == 15)
            AddItems(ch, "amulet_8", 1);
        if (irand == 9)
            AddItems(ch, "amulet_9", 1);
        if (irand == 12)
            AddItems(ch, "amulet_10", 1);
        irand = drand(10);
        if (irand == 1)
            AddItems(ch, "obereg_11", 1);
        if (irand == 5)
            AddItems(ch, "obereg_8", 1);
        if (irand == 10)
            AddItems(ch, "obereg_10", 1);
        irand = drand(40 - GetCharacterSPECIALSimple(PChar, SPECIAL_L) * 2);
        if (irand == 1)
            AddItems(ch, "obereg_7", 1);
        // рецепты
    ...
    case "indian": // индейские артефакты
        ...
        irand = rand(10);
        if (irand == 1)
            AddItems(ch, "indian_1", 1);
        irand = rand(10);
        if (irand == 1)
            AddItems(ch, "indian_2", 1);
        irand = rand(40);
        if (irand == 1)
            AddItems(ch, "indian_3", 1);
        irand = rand(10);
        if (irand == 1)
            AddItems(ch, "indian_4", 1);
        irand = rand(10);
        if (irand == 1)
            AddItems(ch, "indian_5", 1);
        irand = rand(10);
        if (irand == 1)
            AddItems(ch, "indian_6", 1);
        irand = rand(40);
        if (irand == 1)
            AddItems(ch, "indian_7", 1);
        irand = rand(10);
        if (irand == 1)
            AddItems(ch, "indian_8", 1);
        irand = rand(10);
        if (irand == 1)
            AddItems(ch, "indian_9", 1);
        irand = rand(10);
        if (irand == 1)
            AddItems(ch, "indian_10", 1);
        irand = rand(10);
        if (irand == 1)
            AddItems(ch, "indian_11", 1);
        break;
    ...
    case "jeweller": // ювелир
        ...
        irand = rand(2);
        if (irand == 1)
            AddItems(ch, "chest_open", Rand(1) + 1); // пустые сундуки
        ...
        irand = rand(20);
        if (irand == 1)
            AddItems(ch, "jewelry7", 1);
        irand = rand(20);
        if (irand == 1)
            AddItems(ch, "jewelry8", 1);
        irand = rand(20);
        if (irand == 1)
            AddItems(ch, "jewelry9", 1);
        irand = rand(20);
        if (irand == 1)
            AddItems(ch, "jewelry10", 1);
        irand = rand(20);
        if (irand == 1)
            AddItems(ch, "jewelry11", 1);
        irand = rand(20);
        if (irand == 1)
            AddItems(ch, "jewelry12", 1);
        irand = rand(20);
        if (irand == 1)
            AddItems(ch, "jewelry13", 1);
        irand = rand(20);
        if (irand == 1)
            AddItems(ch, "jewelry14", 1);
        irand = rand(20);
        if (irand == 1)
            AddItems(ch, "jewelry15", 1);
        irand = rand(20);
        if (irand == 1)
            AddItems(ch, "jewelry16", 1);
        irand = rand(10);
        if (irand == 1)
            AddItems(ch, "jewelry17", Rand(4) + 1);
        irand = rand(10);
        if (irand == 1)
            AddItems(ch, "jewelry18", Rand(4) + 1);
        irand = rand(10);
        if (irand == 1)
            AddItems(ch, "jewelry19", Rand(4) + 1);
        irand = rand(10);
        if (irand == 1)
            AddItems(ch, "jewelry20", Rand(4) + 1);
        irand = rand(10);
        if (irand == 1)
            AddItems(ch, "jewelry21", Rand(4) + 1);
        irand = rand(10);
        if (irand == 1)
            AddItems(ch, "jewelry22", Rand(4) + 1);
        irand = rand(10);
        if (irand == 1)
            AddItems(ch, "jewelry23", Rand(4) + 1);
        break;
    ...
    case "lightman": //Jason - смотрители маяков
        ...
        irand = drand(130);
        if (irand == 1)
            AddItems(ch, "amulet_1", 1);
        if (irand == 10)
            AddItems(ch, "amulet_2", 1);
        if (irand == 20)
            AddItems(ch, "amulet_3", 1);
        if (irand == 30)
            AddItems(ch, "amulet_4", 1);
        if (irand == 40)
            AddItems(ch, "amulet_5", 1);
        if (irand == 50)
            AddItems(ch, "amulet_6", 1);
        if (irand == 60)
            AddItems(ch, "amulet_7", 1);
        ...
        if (irand == 75)
            AddItems(ch, "obereg_8", 1);
        if (irand == 85)
            AddItems(ch, "obereg_9", 1);
        if (irand == 95)
            AddItems(ch, "obereg_10", 1);
        if (irand == 105)
            AddItems(ch, "obereg_11", 1);
        if (irand == 115)
            AddItems(ch, "rat_poison", 1);
        irand = drand(120);
        if (irand == 9 && rank > 6)
            GenerateAndAddItems(ch, "blade_06", 1);
        if (irand == 49 && rank > 6)
            GenerateAndAddItems(ch, "blade_10", 1);
        if (irand == 99 && rank > 6)
            GenerateAndAddItems(ch, "blade_15", 1);
        irand = drand(250); // 170912
        if (irand == 6 && rank > 15)
            GenerateAndAddItems(ch, "blade_17", 1);
        if (irand == 96 && rank > 15)
            GenerateAndAddItems(ch, "blade_19", 1);
        if (irand == 226 && rank > 15)
            GenerateAndAddItems(ch, "blade_21", 1);
        break;
    ...
    case "LSC_trader": //Jason - торговец в LSC. Всегда есть: лечилки, пули, порох, картечь. Остальное - как рандом ляжет. Могут быть старшие лечилки, амулеты, группа jewelry, ХО и ОО, доспехи.
        ...
        irand = drand(6);
        if (irand == 0)
            GenerateAndAddItems(ch, "blade_05", rand(1));
        if (irand == 2)
            GenerateAndAddItems(ch, "blade_07", rand(1));
        if (irand == 4)
            GenerateAndAddItems(ch, "blade_08", rand(1));
        if (irand == 6)
            GenerateAndAddItems(ch, "blade_11", rand(1));
        //сабли получше
        irand = drand(15);
        if (irand == 1)
            GenerateAndAddItems(ch, "blade_15", rand(1));
        if (irand == 7)
            GenerateAndAddItems(ch, "blade_13", rand(1));
        if (irand == 14)
            GenerateAndAddItems(ch, "blade_10", rand(1));
        // достойные клинки
        irand = drand(100);
        if (irand == 5)
            GenerateAndAddItems(ch, "blade_19", rand(1));
        if (irand == 40)
            GenerateAndAddItems(ch, "blade_21", rand(1));
        if (irand == 95)
            GenerateAndAddItems(ch, "blade_18", rand(1));
        // пистоли
        irand = drand(12);
        if (irand == 1)
            AddItems(ch, "pistol1", rand(1));
        if (irand == 3)
            AddItems(ch, "pistol3", rand(1));
        if (irand == 6)
            AddItems(ch, "pistol4", rand(1));
        if (irand == 9)
            AddItems(ch, "pistol6", rand(1));
        if (irand == 12)
            AddItems(ch, "pistol2", rand(1));
        // мушкет
        irand = drand(99);
        if (irand == 25)
        ...
        irand = drand(40);
        if (irand == 1)
            AddItems(ch, "cirass1", rand(1));
        if (irand == 6)
            AddItems(ch, "cirass2", rand(1));
        if (irand == 12)
            AddItems(ch, "cirass5", rand(1));
        if (irand == 18)
            AddItems(ch, "cirass6", rand(1));
        // амулеты
        irand = drand(130);
        if (irand == 7)
            AddItems(ch, "indian_1", 1);
        ...
        irand = drand(8);
        if (irand == 0)
            AddItems(ch, "jewelry1", rand(3));
        if (irand == 1)
            AddItems(ch, "jewelry2", rand(5));
        if (irand == 2)
            AddItems(ch, "jewelry3", rand(4));
        if (irand == 3)
            AddItems(ch, "jewelry4", rand(3));
        if (irand == 4)
            AddItems(ch, "jewelry5", rand(3));
        if (irand == 5)
            AddItems(ch, "jewelry6", rand(5));
        if (irand == 6)
            AddItems(ch, "jewelry10", rand(4));
        if (irand == 7)
            AddItems(ch, "jewelry12", rand(5));
        if (irand == 8)
            AddItems(ch, "jewelry13", rand(5));
        irand = drand(6);
        if (irand == 3)
            AddItems(ch, "jewelry45", rand(2));
        if (irand == 4)
            AddItems(ch, "jewelry46", rand(2));
        if (irand == 4)
            AddItems(ch, "jewelry52", rand(5));
        if (irand == 6)
            AddItems(ch, "jewelry53", rand(7));
        break;
    ...
    case "cemeteryman": //Jason: смотрители кладбищ
        irand = drand(1);
        if (irand == 0)
            AddItems(ch, "cannabis1", 1 + (rand(3)));
        if (irand == 1)
            AddItems(ch, "cannabis2", 1 + (rand(3)));
        irand = drand(3);
        if (irand == 0)
            AddItems(ch, "cannabis3", 1 + (rand(3)));
        if (irand == 1)
            AddItems(ch, "cannabis4", 1 + (rand(3)));
        if (irand == 2)
            AddItems(ch, "cannabis5", 1 + (rand(3)));
        if (irand == 3)
            AddItems(ch, "cannabis6", 1 + (rand(3)));
        irand = drand(4);
        if (irand == 0)
            AddItems(ch, "jewelry12", 1 + (rand(1)));
        if (irand == 1)
            AddItems(ch, "jewelry13", 1 + (rand(1)));
        if (irand == 2)
            AddItems(ch, "jewelry22", 1 + (rand(1)));
        irand = drand(10);
        if (irand == 0)
        ...
        irand = drand(120);
        if (irand == 10)
            AddItems(ch, "blade_04", 1);
        if (irand == 40)
            AddItems(ch, "blade_06", 1);
        if (irand == 80)
            AddItems(ch, "blade_10", 1);
        irand = drand(7);
        if (irand == 0)
            AddItems(ch, "pistol1", 1);
        irand = drand(5);
        if (irand == 0)
            AddItems(ch, "bullet", 1 + (rand(10)));
        if (irand == 1)
            AddItems(ch, "grapeshot", 1 + (rand(10)));
        if (irand == 2)
            AddItems(ch, "cartridge", 1 + (rand(5)));
        if (irand == 3)
            AddItems(ch, "gunpowder", 1 + (rand(10)));
...
{                                                             // location = "random"  "random_free"  и "random_must_be" - должен быть, даже если все занято  random_must_be_near - рядом
    ...
    if (location == "random_free")
        location = "random"; // совместимость с пред. правкой
...
int FindFirstEmptyCharacter()
{
    for (int i = GlobalCharacters; i < TOTAL_CHARACTERS; i++)
    {
        ...
        {
            if (i >= MAX_CHARACTERS)
                MAX_CHARACTERS = i + 1; //сдвигаем планку НПС
            return i;
        }
    }
    return -1;
...
int FindNonEnemyColonyForAdventure(int iNation, string sBeginColony, bool _checkPort)
{
    ...
    for (int i = 0; i < MAX_COLONIES; i++)
    {
        ...
        {
            if (_checkPort && CheckAttribute(&Colonies[i], "HasNoFort"))
                continue; // без форта не берем
...
void EmptyAbordageCharacters()
{
    ...
    {
        if (CheckAttribute(&characters[i], "AboardFantom"))
...
{
    if (bAbordageStarted)
        return; // иначе трутся НПС при переходе в каюте
    int cn = -1;
    for (int i = GlobalCharacters; i < MAX_CHARACTERS; i++)
    {
...
{ // boal: метод соверненно не понятен, убрал из него массив НатионСтайт, заменив на 1.0, раз работало - пусть будет как было, но логика чумовая
    ...
    {
        if (rand(2) == 1)
            return HOLLAND; //голланцев на карту
...
{
    if (CheckAttribute(_loc, group) == 0)
        return "";
    aref grp;
