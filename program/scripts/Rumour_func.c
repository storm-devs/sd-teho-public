...
void SelectAction(ref rid) //CASE с действиями для слухов
{
    ...
    case "GetPrices": {
        ...
        if (icnum != -1)
        {
            ...
            rid.text = NO_RUMOUR_TEXT[rand(SIMPLE_RUMOUR_NUM - 1)]; // нету слухов
                                                                    ///////////////////////////////////////////
    }
    break;
        //============ Осады ===============
...
string SelectRumour() // Получить рандомный слух из очереди
{
    ...
    for (Rumour_Index; Rumour_Index < MAX_RUMOURS; Rumour_Index++)
    {
        ...
        if ((sti(CurrentRumour.actualtime) >= DateToInt(0)) && (st > 0) && CurrentRumour.text != "") // Слух не просрочен
        {                                                                                            // Не выпадал до этого несколько раз
            ...
            if (sti(CurrentRumour.starttime) <= DateToInt(0))
            {
                ...
                if (CurrentRumour.text != "" && CurrentRumour.text != "error")
                    rez = CurrentRumour.text; //fix
                return rez;
            }
            else
            {
...
bool RumourCheker(ref rRumour, string key, aref arPrm)
{
    ...
    {                                    // т.е. слух ходит во всех городах кроме заданного
        ...
        if (CheckAttribute(arPrm, "City")) // fix homo 15/03/07 (homo перенес из КВЛ 06/02/08)
        {
            ...
            a = false; // fix homo 15/03/07 (homo перенес из КВЛ 06/02/08)
    }
    //navy <--
...
{                                             // key - ключ спец. слуха
    ...
    else
    {
        ...
        {
        case "landcaptain":
            return CAPTAIN_RUMOUR_TEXT[rand(CITIZEN_RUMOUR_NUM - 1)] break;
        case "towngirl":
            return CITIZEN_WOMAN_RUMOUR_TEXT[rand(CITIZEN_RUMOUR_NUM - 1)];
            break;
        case "townman":
            return CITIZEN_MAN_RUMOUR_TEXT[rand(CITIZEN_RUMOUR_NUM - 1)];
            break;
        case "nobleman":
            return NOBLE_MAN_RUMOUR_TEXT[rand(CITIZEN_RUMOUR_NUM - 1)];
            break;
        case "noblegirl":
            return NOBLE_WOMAN_RUMOUR_TEXT[rand(CITIZEN_RUMOUR_NUM - 1)];
            break;
        case "sailor":
            return SAILOR_RUMOUR_TEXT[rand(CITIZEN_RUMOUR_NUM - 1)];
            break;
        }
    }
    return NO_RUMOUR_TEXT[rand(SIMPLE_RUMOUR_NUM - 1)];
...
int AddSimpleRumour(string stext, int nation, int terms, int qty)
{
    ...
    if (nation > 5)
        tmp.nonation = nation - 10;
    else
        tmp.onlynation = nation; //локализация
    tmp.starttime = 0;
