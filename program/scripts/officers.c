...
bool CheckForAllOfficers() // ugeen 2016 -  возвращает true при наличии всех офицеров в команде ГГ
{
    ...
    for (int i = 1; i < 4; i++)
    {
        if (sti(GetOfficersIndex(pchar, i)) < 0)
            bOk = false;
    }
    return bOk;
}
...
int FindFreeRandomOfficer()
{
    ...
    string OfficerId;
    // special -->
    if (GetCharacterMaxOfficersQty(Pchar) <= (GetOfficerPassengerQuantity(Pchar) + GetCompanionQuantity(Pchar) - 1))
        return -1;
    return 1;
}
...
bool SetOfficerToMushketer(ref rCharacter, string sMushket, bool _ToMushketer)
{
    ...
    if (_ToMushketer && sMushket != "") // Делаем мушкетером
    {
        ...
        if (!CheckAttribute(rCharacter, "MusketerDistance"))
            rCharacter.MusketerDistance = 10.0;
