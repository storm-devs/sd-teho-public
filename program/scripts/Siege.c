...
{
    if (CheckAttribute(pchar, "GenQuest.Intelligence.City") && pchar.GenQuest.Intelligence.City == sColony)
        return false;

    if (CheckAttribute(pchar, "GenQuest.Intelligence.MayorId") &&
        characters[GetCharacterIndex(pchar.GenQuest.Intelligence.MayorId)].City == sColony)
        return false;

    if (CheckAttribute(pchar, "questTemp.State") && pchar.questTemp.State == "EndOfQuestLine")
        return true;
...
int MakeSiegeSquadron(int ination)
{
    ...
    for (int i = 1; i <= icon; i++)
    {
        ...
        if (i == 1)
            SetRandGeraldSail(sld, sti(sld.Nation)); // homo Гербы
    }
...
int SetSiegeShip(ref rChar)
{
    ...
    if (sti(aData.imanofwars) > 0)
    {
        ...
        {
        case PIRATE:
            SiegeShips = SHIP_LINESHIP;
            break;
        case ENGLAND:
            SiegeShips = RandFromThreeDight(SHIP_LINESHIP, SHIP_LSHIP_ENG, SHIP_LINESHIP);
            break;
        case FRANCE:
            SiegeShips = RandFromThreeDight(SHIP_LINESHIP, SHIP_LSHIP_FRA, SHIP_LINESHIP);
            break;
        case SPAIN:
            SiegeShips = RandFromThreeDight(SHIP_LINESHIP, SHIP_LSHIP_SPA, SHIP_LINESHIP);
            break;
        case HOLLAND:
            SiegeShips = RandFromThreeDight(SHIP_LINESHIP, SHIP_LSHIP_HOL, SHIP_LINESHIP);
            break;
        }
    ...
    else
    {
        ...
        {
        case PIRATE:
            SiegeShips = RandFromThreeDight(SHIP_CORVETTE, SHIP_GALEON_H, SHIP_FRIGATE);
            break;
        case ENGLAND:
            SiegeShips = RandFromThreeDight(SHIP_CORVETTE, SHIP_FRIGATE, SHIP_FRIGATE_H);
            break;
        case FRANCE:
            SiegeShips = RandFromThreeDight(SHIP_CORVETTE, SHIP_FRIGATE, SHIP_FRIGATE_H);
            break;
        case SPAIN:
            SiegeShips = RandFromThreeDight(SHIP_POLACRE, SHIP_GALEON_H, SHIP_FRIGATE_H);
            break;
        case HOLLAND:
            SiegeShips = RandFromThreeDight(SHIP_XebekVML, SHIP_FRIGATE, SHIP_FRIGATE_H);
            break;
        }
        rez = SiegeShips;
    }
...
int CheckFortInjuri()
{
    ...
    if (CheckAttribute(FC, "Fort.Mode") && sti(FC.Fort.Mode) == FORT_DEAD)
        return 0;
...
void BattleOfTheColony(string tmp)
{
    ...
    if (CheckAttribute(PChar, "quest.BattleOfTheColony"))
        Pchar.quest.BattleOfTheColony.over = "yes";
    CheckGroupCommander("");
...
void PlayerKillSquadron(string tmp)
{
    ...
    aData.win = 0;
    if (GetNationRelation2MainCharacter(sti(aData.nation)))
        aData.PlayerHelpMayor = true;
    EndOfTheSiege("End");
}
...
void EndOfTheSiege(string tmp)
{
    ...
    if (idist == -1 || idist > 60 || tmp != "")
    {
        ...
        if (!CheckAttribute(comand, "dialog") || !CheckAttribute(comand, "dialog") || comand.dialog.currentnode != "no_return_02" || comand.DeckDialogNode != "no_return_02")
        {
        ...
        if (CheckAttribute(PChar, "quest.SiegeResult"))
            Pchar.quest.SiegeResult.over = "yes";
        if (CheckAttribute(PChar, "quest.LeaveBattle"))
            Pchar.quest.LeaveBattle.over = "yes";
        if (sti(aData.win) == 1)
        {
            ...
            // belamour форт пал под натиском
            If(CheckAttribute(PChar, "questTemp.Patria.GenGovernor"))
            {
                PlaySound("interface\notebook.wav");
                Log_Info("Монсеньор, " + NationNameSK(sti(aData.nation)) + "ая эскадра разграбила колонию " + GetConvertStr(aData.Colony + " Town", "LocLables.txt") + "!");
            }
        ...
        else
        {
            ...
            SiegeClear("");
            // belamour защита выстояла
            If(CheckAttribute(PChar, "questTemp.Patria.GenGovernor"))
            {
                PlaySound("interface\notebook.wav");
                Log_Info("Монсеньор, " + NationNameSK(sti(aData.nation)) + "ая эскадра не смогла разграбить колонию " + GetConvertStr(aData.Colony + " Town", "LocLables.txt") + "!");
            }
...
void SiegeClear(string tmp)
{
    ...
    if (CheckAttribute(NullCharacter, "Siege.Colony"))
    {
        ...
        DeleteAttribute(rColony, "Siege");             //снимаем осаду
        if (CheckAttribute(PChar, "quest.Union_with_Escadra"))
            Pchar.quest.Union_with_Escadra.over = "yes";
        if (CheckAttribute(PChar, "quest.BattleOfTheColony"))
            Pchar.quest.BattleOfTheColony.over = "yes";
        if (CheckAttribute(PChar, "quest.EndOfTheSiege"))
            Pchar.quest.EndOfTheSiege.over = "yes";
        if (CheckAttribute(PChar, "quest.ClearSiege"))
            Pchar.quest.ClearSiege.over = "yes";
        if (CheckAttribute(PChar, "quest.LeaveBattle"))
            Pchar.quest.LeaveBattle.over = "yes";
        if (CheckAttribute(PChar, "quest.SiegeResult"))
            Pchar.quest.SiegeResult.over = "yes";
        if (CheckAttribute(PChar, "quest.PlayerKillSquadron"))
            Pchar.quest.PlayerKillSquadron.over = "yes";
        if (CheckAttribute(PChar, "Siege_Start"))
