...
string GetIslandForTreasure()
{
    ...
    for (int i = 0; i < iNum; i++)
    {
        ...
        {
            if (sCurIsland == sAttr || CheckTreasureMaps(sAttr))
                continue;
            else
            {
                sArray[m] = sAttr;
...
bool CheckTreasureMaps(string sIsland)
{
    ...
    if (GetCharacterItem(pchar, "mapQuest") > 0)
    {
        ...
        if (CheckAttribute(itm, "MapIslId") && itm.MapIslId == sIsland)
            return true;
    }
...
void SetTreasureHunter(string temp)
{
    ...
    if (chrDisableReloadToLocation)
        return; // идет некий другой квест с запретом выхода
