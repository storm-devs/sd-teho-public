...
void AfterTownBattle()
{
    ...
    if (CheckAttribute(Pchar, "GenQuestFort.StartAttack"))
        DeleteAttribute(Pchar, "GenQuestFort.StartAttack");
    if (!CheckAttribute(Pchar, "GenQuestFort.fortCharacterIdx"))
        return; // еще не было
...
ref SetFantomSlaves(string group, string locator, string enemygroup)
{
    ...
    if (Cr.location.locator != "")
    {
        ...
        {
        case 0:
            blade = GetGeneratedItem("slave_01");
            break;
        case 1:
            blade = GetGeneratedItem("slave_02");
            break;
        case 2:
            blade = GetGeneratedItem("topor_05");
            break;
        }
    ...
    if (loadedLocation.type == "residence")
    {
        ...
        { // город нации
            ...
            if (j > 0)
            {
                ...
                for (i = 0; i < j; i++)
                {
                    if (sti(Pchar.GenQuestFort.TownCrew) < 1)
                        break;
    ...
    { // не резиденция - ворота или порт
        ...
        { // если есть враги
            ...
            for (i = 0; i < MAX_TOWN_MUSHKETER; i++)
            {
                ...
                if (rand(5) == 1)
                    sld.MushketType = "mushket3";  // 280313
                ...
                if (pchar.questTemp.Ascold != "Ascold_ImMummy")
                {
                    ...
                    for (i = 0; i < MAX_TOWN_MUSHKETER; i++) // Captain Beltrop, фикс моделей мушкетёров, раньше они определялись по флагу ГГ, теперь всё как и раньше... 31.10.2020
                    {
                        ...
                        if (isMainCharacterPatented() && sti(Items[sti(rCharacter.EquipedPatentId)].TitulCur) > 1) //если есть патент, оставлю, хотя рудимент, форма только со звания капитан
                        {
                            ...
                            sModel = "mush_ctz_" + (rand(2) + 4); // шесть моделей
...
void Move_Govenour_Nation(int iNation)
{
    ...
    if (iNation != PIRATE)
    {
        ...
        if (CheckAttribute(rChar, "notMoveAble"))
            return; //проверяем личный флаг губернатора на переезд
...
{
    // сухопутные города
    if (CheckAttribute(ChOff, "Mayor"))
        return; // сам себе командир (виртуальный форт)
...
void SetShipSquadron(ref rChar)
{
    ...
    if (rand(2) == 1)
    {
        ...
        {
        case ENGLAND:
            SiegeShips = SHIP_LSHIP_ENG;
            break;
        case FRANCE:
            SiegeShips = SHIP_LSHIP_FRA;
            break;
        case SPAIN:
            SiegeShips = SHIP_LSHIP_SPA;
            break;
        case HOLLAND:
            SiegeShips = SHIP_LSHIP_HOL;
            break;
        case PIRATE:
...
bool isHeroOwnCity(bool _isBoughtCare) // _isBoughtCare - true, то проверять проданность, если нет, то пофиг, просто город ГГ (для подарить)
{
    ...
    for (i = 0; i < MAX_COLONIES; i++)
    {
        ...
        {
            if (!_isBoughtCare || sti(colonies[i].isBought) == false)
                return true;
        }
    }
    return false;
