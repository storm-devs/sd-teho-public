...
int SetShipTypeMerchant(ref Cap)
{
    ...
    case ENGLAND:
        ...
        case 3:
            ...
            break;
        case 4:
            iShip = SHIP_FLEUT + rand(3);
            break;
        case 5:
            iShip = SHIP_NAVIO + rand(2);
            break;
        }
        break;
...
void SetMerchantShip(ref Cap, int igoods)
{
    ...
    ref MerPrm;
    makeref(MerPrm, MerchantParam);
    int killM = Statistic_AddValue(GetMainCharacter(), "Rob" + NationShortName(sti(MerPrm.nation)) + "MerchantOnMap", 0) if (MOD_SKILL_ENEMY_RATE > 5) // 20/01/07 homo
    {
...
string GenerateMerchant()
{
    ...
    int lngFileID;
    string HomeColony = All_GetColony();
    int iNation = sti(Colonies[FindColony(HomeColony)].nation) if (MerchantPrepare(iNation))
    {
