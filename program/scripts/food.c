...
int CalculateFood()
{
    ...
    for (int i = 0; i < COMPANION_MAX; i++)
    {
        ...
        if (chrIndex != -1)
        {
            if (!GetRemovable(&characters[chrIndex]))
                continue;
...
{
    if (IsCharacterEquippedArtefact(chref, "talisman1"))
        return; // крысобог
    if (IsCharacterEquippedArtefact(chref, "rat_poison"))
        return; // мышьяк
    if (CheckAttribute(pchar, "GenQuest.Ole"))
        return; // Оле Кристиансен
    int iGoods = GOOD_FOOD + rand(GOOD_OIL - GOOD_FOOD);
    if (iGoods == GOOD_BRICK)
        return;
    ...
    if (iQuantity > 60 && rand(4) != 2) // шанс не жрать, если весь спектр
    {
        ...
        if (IsCharacterPerkOn(chref, "HT2"))
            iQuantity = makeint(iQuantity / 2) + 1;
        if (CheckAttribute(pchar, "quest.givemecat"))
            iQuantity = makeint(iQuantity / 2) * 1.7; // лесник кошка на корабле
        RemoveCharacterGoodsSelf(chref, iGoods, iQuantity);
        //PlaySound("interface\notebook.wav");
        Log_SetStringToLog(RandSwear());
        Log_SetStringToLog("Крысы на корабле " +
...
void DailyEatCrewUpdate() // сюда пихаю все что в 1 день
{
    ...
    if (CheckAttribute(mainCh, "questTemp.ShipCapellan.Yes"))
        AddCrewMorale(mainCh, 2); //Jason, влияние корабельного капеллана на мораль

    ////////////////      ЕДА     /////////////////
    if (bNoEatNoRats)
        return; // betatest
    if (sti(mainCh.Ship.Type) == SHIP_NOTUSED)
        return;
...
void DailyEatCrewUpdateForShip(ref rChar, bool IsCompanionTraveler) // IsCompanionTraveler - спец флаг для компаньонов-путешественников
{
    ...
    if (rand(4) == 2)
    {
        ...
        if (cn > 0)
        {
            if (GetCargoGoods(rChar, GOOD_MEDICAMENT) < 1)
            {
                if (!IsCompanionTraveler)
                    Log_Info("На корабле " + rChar.Ship.Name + " от болезней умерло " + FindRussianSailorString(cn, "No"));
                ...
                if (CheckOfficersPerk(rChar, "IronWill"))
                    cn /= 1.5;
            ...
            {
                if (CheckShipSituationDaily_GenQuest(rChar) == 2)
                    cn = cn * 2;
                if (CheckShipSituationDaily_GenQuest(rChar) == 3)
                    cn = cn * 3;
                ...
                {
                    if (!IsCompanionTraveler)
                        Log_Info("На корабле " + rChar.Ship.Name + " осталось мало медикаментов");
                }
            }
        }
        // рабы
        ...
        if (cn > 30)
            cn = 30;
        cn = rand(cn) + 1;
        if (GetCargoGoods(rChar, GOOD_SLAVES) < cn)
        ...
        if (cn > 0)
        {
            ...
            {
                if (!IsCompanionTraveler)
                    Log_Info("На корабле " + rChar.Ship.Name + " от болезней умерло " + FindRussianSlavesString(cn, "No"));
    ...
    if (iCrewQty > 0)
    {
        if (GetCargoGoods(rChar, GOOD_RUM) >= iCrewQty)
        {
            ...
            // поднимем мораль
            if (CheckShipSituationDaily_GenQuest(rChar) == 1)
                AddCrewMorale(rChar, 2);
        }
        else
        {
    ...
    else
    {
        ...
        if (!IsCompanionTraveler)
            Log_Info("На корабле " + rChar.Ship.Name + " матросы голодают. Мораль команды падает!");
        ...
        if (cn > 1)
        {
            ...
            if (!IsCompanionTraveler)
                Log_Info("Матросы умирают от голода");
        }
        cn = GetCargoGoods(rChar, GOOD_SLAVES);
        if (cn > 0)
        {
            ...
            if (!IsCompanionTraveler)
                Log_Info("Рабы умирают от голода");
        }
        morale = sti(rChar.Ship.Crew.Morale);
        ...
        if (sti(rChar.Ship.Crew.Morale) < MORALE_MIN)
            rChar.Ship.Crew.Morale = MORALE_MIN;
    }
