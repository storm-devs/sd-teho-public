...
bool Ship_AutoAbordage(ref rCharacter, float fMinEnemyDistance)
{
    ...
    if (fMinEnemyDistance < 70)
    {
        ...
        if (CheckAttribute(rCharacter, "Tasks.CanBoarding") && !sti(rCharacter.Tasks.CanBoarding))
            return false;

        // Addon 2016-1 Jason Пиратская линейка
        if (sti(RealShips[sti(rCharacter.ship.type)].basetype) == SHIP_POLACRE_QUEST && rCharacter.id == "Ignasio")
            return false;
        if (rCharacter.id == "Terrax")
            return false;

        if (fMinEnemyDistance > fOurMaxAbordageDistance)
            return bSuccess;
        ...
        if (Ship_GetDistance2D(rCharacter, rShipCharacter) > 90)
            return bSuccess; // fix левых дистанций

        int iRelation = SeaAI_GetRelation(sti(rCharacter.index), sti(rShipCharacter.index));

        if (!Character_IsAbordageEnable(rShipCharacter) || sti(rShipCharacter.index) == GetMainCharacterIndex())
            return bSuccess;

        if (iRelation != RELATION_ENEMY)
            return bSuccess;
        // решение об абордаже
        if (sti(rCharacter.Ship.Crew.Quantity) < (stf(rShipCharacter.Ship.Crew.Quantity) * 0.6))
            return bSuccess;
        ...
        if (bGrapplingProfessional || bAngleTest)
        {
            ...
            { // победа
                ...
                if (IsCompanion(rCharacter))
                    ...
                    ShipDead(sti(rShipCharacter.index), KILL_BY_ABORDAGE, sti(rCharacter.index));
                //navy <--
            ...
            { // поражение
                ...
                if (IsCompanion(rShipCharacter))
                    ...
                    ShipDead(sti(rCharacter.index), KILL_BY_ABORDAGE, sti(rShipCharacter.index));
                //navy <--
            }
        }
    }
...
bool CheckChanceOfBetterShip(ref rChar, ref rTryChar)
{
    ...
    if (CheckAttribute(rChar, "Tasks.CanChangeShipAfterBoarding") && !sti(rChar.Tasks.CanChangeShipAfterBoarding))
        return false;
    if (CheckAttribute(rChar, "Tasks.CanBoarding") && !sti(rChar.Tasks.CanBoarding))
        return false;
    ...
    if (sti(rChar.Ship.Cannons.Type) == CANNON_TYPE_CANNON_LBS48 || sti(rTryChar.Ship.Cannons.Type) == CANNON_TYPE_CANNON_LBS48)
        return false;
    ...
    if (sti(rShip.SpeedRate) < sti(rTryShip.SpeedRate))
        fChance += 0.2;
    if (sti(rShip.TurnRate) < sti(rTryShip.TurnRate))
        fChance += 0.15;
    if (sti(rShip.CannonsQuantity) < sti(rTryShip.CannonsQuantity))
        fChance += 0.1;
    if (sti(rShip.MaxCrew) < sti(rTryShip.MaxCrew))
        fChance += 0.1;
    if (sti(rShip.Capacity) < sti(rTryShip.Capacity))
        fChance += 0.1;
...
bool CheckEnemyCompanionDistance2GoAway(bool _loadForm)
{
    ...
    for (k = 1; k < COMPANION_MAX; k++)
    {
        ...
        if (cn >= 0)
        {
            ...
            if (!GetRemovable(rCharacter))
                continue; // квестовые сами себя спасают
