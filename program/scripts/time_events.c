...
void WorldSituationsUpdate()
{
    ...
    case 0:
        ...
        { //Jason: еженедельное обновление паролей кланов LSC и ежедневное вытирание
            ...
            if (CheckAttribute(pchar, "questTemp.LSC.parol_nrv"))
                DeleteAttribute(pchar, "questTemp.LSC.parol_nrv");
            if (CheckAttribute(pchar, "questTemp.LSC.parol_rvd"))
    ...
    case 1:
        ...
        if (rand(2) == 0)
            Norman_ChangeFesivalFace(); // выходки Нормана
        if (GetDataDay() == 5 && CheckAttribute(pchar, "questTemp.OilTrade"))
            DoQuestFunctionDelay("Oil_SetSergioToMayak", 1.0);                                                                                      // генератор смол
        ...
        // калеуче - без НИ
        if (CheckAttribute(pchar, "questTemp.Caleuche") && pchar.questTemp.Caleuche == "Start")
            DoQuestFunctionDelay("Caleuche_StartGo", 1.0);
        // ФМК - без НИ
        if (!CheckAttribute(pchar, "questTemp.FMQ.Success") && sti(pchar.rank) > 3)
            DoQuestFunctionDelay("FMQ_SetConditions", 1.0);
    ...
    case 2:
        ...
        if (!CheckAttribute(pchar, "questTemp.Mtraxx.BugsFixer"))
            Mtraxx_BugsFixer(); // правки релиза
        break;
    ...
    case 3:
        ...
        if (!CheckAttribute(pchar, "questTemp.Patria_BugsFixer"))
            Patria_BugsFixer(); // 17-add
        ...
        if (CheckAttribute(pchar, "questTemp.LongHappy.BaronPart") && GetDataDay() == 5)
        {
            ...
            log_info("Жан Давид подготовил вашу долю добычи в Ле Франсуа");
        }
