...
int GetSalaryForShip(ref chref)
{
    ...
    if (shClass < 1)
        shClass = 6;
    if (!GetRemovable(chref) && sti(chref.index) != GetMainCharacterIndex())
        return 0; // считаем только своих, а то вских сопровождаемых кормить!!!
...
{
    if (!CheckAttribute(chr, "Ship.Crew.Exp." + sType))
        chr.Ship.Crew.Exp.(sType) = (1 + rand(50));
    ...
    if (stf(chr.Ship.Crew.Exp.(sType)) > 100)
        chr.Ship.Crew.Exp.(sType) = 100;
    if (stf(chr.Ship.Crew.Exp.(sType)) < 1)
        chr.Ship.Crew.Exp.(sType) = 1;
...
void UpdateCrewInColonies()
{
    ...
    {
        rTown = &colonies[i];
        if (rTown.nation == "none")
            continue;

        if (CheckAttribute(rTown, "AdditionalCrew"))
...
*/
            ...
            if (CheckAttribute(pchar, "GenQuest.Shipshine"))
                rTown.Ship.crew.quantity = sti(rTown.Ship.crew.quantity) * 2; //Jason
        ...
        { // Делим бабки
            if (bPartitionSet)
            {
                ...
                if (!CheckAttribute(Pchar, "Partition.MonthPart"))
                    Pchar.Partition.MonthPart = 0;
                if (!CheckAttribute(Pchar, "Partition.MonthPart.Crew"))
                    Pchar.Partition.MonthPart.Crew = 0;
                if (!CheckAttribute(Pchar, "Partition.MonthPart.Officers"))
                    Pchar.Partition.MonthPart.Officers = 0;
                if (!CheckAttribute(Pchar, "Partition.MonthPart.Hero"))
                    Pchar.Partition.MonthPart.Hero = 0;
                if (!CheckAttribute(Pchar, "Partition.MonthPart.Gower"))
                    Pchar.Partition.MonthPart.Gower = 0;
