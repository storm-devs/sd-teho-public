    ...
    if (CheckAttribute(chr, "PGGAi.Task") && chr.PGGAi.Task != PGG_TASK_NOTASK)
    {
        ...
        if (sTask == PGG_TASK_WORKWITHCONTRA)
        {
            ...
            if (sti(chr.PGGAi.Task.DaysCounter) > 3)
            {
                ...
                PGG_DebugLog(chr.id + " " + chr.PGGAi.Task + " " + chr.PGGAi.Task.DaysCounter);
        }
        //без денег и корабля...
...
void PGG_CheckDead(ref chr)
{
    if (chr.PGGAi.location == "Dead")
        return;
    if (IsCompanion(chr) && GetRemovable(chr) && CheckOfficersPerk(chr, "ShipEscape"))
        return;
...
void PGG_UpdateStats(ref chr, string sExpType)
{
    ...
    if (!CheckAttribute(chr, "PGGAi.OldRank"))
...
void PGG_UpdateEquip(ref chr)
{
    ...
    if (bShipNotUsed || bOk)
    {
        ...
        if (n != 0)
        {
            ...
            if (!bShipNotUsed)
            {
                if (rand(100) < makeint(10 * MOD_SKILL_ENEMY_RATE / 2))
                    bOk = true;
            }
            ...
            if (bOk)
            {
                ...
                if (sti(chr.Money) > i)
                {
                    ...
                    SetBaseShipData(chr);
                    if (!CheckAttribute(chr, "Ship.Name"))
...
void PGG_TavernCheckIsPGGHere()
{
    ...
    for (i = 1; i <= PsHeroQty; i++)
    {
        ...
        if (findsubstr(pchar.location, chr.PGGAi.location.town, 0) != -1 && !LAi_IsDead(chr) && chr.PGGAi.location != "Dead") //закрыл дополнительно.
        {
            ...
            if (!CheckAttribute(pchar, "GenQuest.PGG_Quest") && PGG_CheckForQuestOffer(chr))
                continue;
            //в таверне или нет.
            if (rand(3) == 1)
            {
...
int CheckFreeServiceForNPC(ref NPChar, string ServiceType)
{
    ...
    for (int i = 1; i <= PsHeroQty; i++)
    {
        ...
        {
            if (CheckAttribute(chr, "PGGAi.LockService") && chr.PGGAi.LockService == ServiceType)
...
int CheckAvailableTaskForNPC(ref NPChar, string PGGTask)
{
    ...
    for (int i = 1; i <= PsHeroQty; i++)
    {
        ...
        if (chr.PGGAi.location == "land" && chr.PGGAi.location.town == NPChar.City)
        {
            if (CheckAttribute(chr, "PGGAi.Task") && chr.PGGAi.Task == PGGTask)
                return sti(chr.index);
        }
    }
    return -1;
...
void PGG_AddRumour(ref chr, string _idTmpl)
{
    ...
    if (_idTmpl != PGG_TASK_WAITINTAVERN && _idTmpl != PGG_TASK_WORKONMAYOR &&
        ...
        _idTmpl != PGG_TASK_WORKWITHCONTRA)
        return;

    //нет слухов с пиратских поселений.
    iCol = FindColony(chr.PGGAi.location.town);
    if (iCol != -1 && sti(Colonies[iCol].nation) == PIRATE)
        return;
...
void PGG_OnInit_SailToIsland(ref rTmpl)
{
    ...
    oPrm.PsHero_Name = GetFullName(chr);
    oPrm.To = "в";
    if (chr.PGGAi.Task.Target == "Tortuga")
        oPrm.To = "на";
...
bool PGG_CheckForQuestOffer(ref chr)
{
    ...
    if (rand(2) == 1)
        return retVal; // пусть сам ГГ тоже подходит, а не только ПГГ достает, второй заход в таврену может вынудить подойти ПГГ

    int iTst = 110 - PGG_ChangeRelation2MainCharacter(chr, 0); //зависит от отношения, лучше = чаще
    // пусть будет честный тест
    if (bBettaTestMode && CheckAttribute(PChar, "PGGAlwaysQuest"))
        iTst = 10; //в версии нет этого аттрибута
...
bool PGG_IsQuestAvaible()
{
    ...
    if (sTown == "" || sTown == "Panama" || Colonies[FindColony(sTown)].from_sea == "" || CheckAttribute(Colonies[FindColony(sTown)], "HasNoFort"))
        retVal = false;
    return retVal;
}
...
void PGG_Q1PlaceShipsNearIsland()
{
    ...
    for (i = 0; i < iRnd; i++)
    {
        ...
        else
        {
            ...
            if (!CheckAttribute(PChar, "GenQuest.PGG_Quest.Goods.Qty"))
                PChar.GenQuest.PGG_Quest.Goods.Qty = 0;
...
void PGG_Q1AfterShoreFight()
{
    ...
    PChar.GenQuest.PGG_Quest.Goods.Taken = 500 + rand(500) + MakeInt(GetSquadronFreeSpace(PChar, sti(PChar.GenQuest.PGG_Quest.Goods)) / (3 + rand(2))) chr = CharacterFromID(PChar.GenQuest.PGG_Quest.PGGid);
    if (!CheckAttribute(PChar, "GenQuest.PGG_Quest.PGGid.Dead"))
    {
        PChar.Quest.PGGQuest1_PGGDead.Over = "yes";
