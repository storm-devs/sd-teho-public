...
void procQuestSceneCommand()
{
    ...
    case "gotopoint": // "ssass"
        ...
        arTask.locator = s2;
        if (QuestCheckName != "")
            arTask.QuestCheck = QuestCheckName;
        return;
        break;
    ...
    case "followcharacter": // "ssas"
        ...
        arTask.charID = s1;
        if (GetCharacterIndex(s1) < 0)
            trace("SCENE WARNING! Character wish id = <" + s1 + "> NOT FOUND!");
        if (QuestCheckName != "")
            arTask.QuestCheck = QuestCheckName;
        return;
        break;
    ...
    case "StopTask": // "ssa"
        if (GetLastTask(arCharacter, &arTask))
            arTask.StopTask = true;
        return;
        break;
    }
...
void qprocTaskEnd()
{
    ...
    aref taskRef;
    if (!GetFirstTask(arch, &taskRef))
        return;
    if (!CheckAttribute(taskRef, "Executing") || taskRef.Executing != "1")
        return;
    ...
    if (CheckAttribute(taskRef, "QuestCheck"))
        ExecuteQuestCheck(taskRef.QuestCheck);
...
{
    aref taskRef;
    if (!GetLastTask(character, &taskRef))
        return "";
...
{
    aref taskRef;
    if (!GetFirstTask(character, &taskRef))
        return "";
    string taskName = GetAttributeValue(taskRef);
    DeleteAttribute(character, "SceneTask." + GetAttributeName(taskRef));
    return taskName;
...
{
    aref staref;
    makearef(staref, character.SceneTask);
...
{
    aref scnref;
    if (!GetFirstTask(character, &scnref))
        return;
...
bool PerformLoopAnimation(ref chr, string action)
{
    ...
    {
    case "stay":
        LAi_ActorAnimation(chr, GetStayAniName(), QuestNameForChr(chr), SCENES_TIMEOUT);
        return;
        break;
    case "fightStand":
        LAi_ActorAnimation(chr, "fight stand_" + (rand(3) + 1), QuestNameForChr(chr), SCENES_TIMEOUT);
        return;
        break;
    case "sit":
        LAi_ActorAnimation(chr, GetSitAniName(), QuestNameForChr(chr), SCENES_TIMEOUT);
        return;
        break;
    }
...
string GetSitAniName()
{
    ...
    {
    case 0:
        return "Sit_Look_Around";
        break;
    case 1:
        return "Sit_Lower_head";
        break;
    case 2:
        return "Sit_HeadonHand";
        break;
    case 3:
        return "Sit_WaveHand";
        break;
...
string QuestNameForChr(aref chref)
{
    ...
    if (q <= 0)
    {
        questName = "_ScenesQuest_0";
    }
    else
    {
        questName = "_ScenesQuest_" + q;
    }
