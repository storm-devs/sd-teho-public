...
void QuestComplete(string sQuestName, string qname)
{
    ...
    case "Munity_on_Ship":
        ...
        if (!CheckAttribute(pchar, "GenQuest.SlavesMunity"))
        {
            for (i = 1; i <= 9; i++)
            {
                ...
                if (i >= (sti(PChar.Ship.Crew.Quantity) / ((8 - GetCharacterShipClass(PChar)) * 8)))
                    break;
        ...
        else
        {
            for (i = 1; i <= 9; i++)
            {
                ...
                if (i >= (GetCargoGoods(PChar, GOOD_SLAVES) / ((8 - GetCharacterShipClass(PChar)) * 8)))
                    break;
    ...
    case "saved_by_off":
        ...
        if (makeint(Pchar.money) < oSum)
            oSum = makeint(Pchar.money);
        ...
        if (CheckAttribute(pchar, "questTemp.Consumption") && pchar.location == "PortSpein_tavern")
            sld.dialog.currentnode = "without_moneysss";
        else
            sld.dialog.currentnode = "without_money"; // лесник ,выбор диалога если нажралдся в "цене чухотки"
        LAi_SetActorType(sld);
    ...
    case "Rand_ContrabandInterruptionAtSeaEnded":
        ...
        AddSimpleRumourCity("А вот давеча баталия у маяка была. Какой-то капитан пытался продать крупную партию контрабанды, так его патрульная эскадра за энтим делом накрыла. С поличным, значит. " +
                                "Грохоту было - весь город переполошился, а в местной лавке крышу ядром проломило! Да только без толку все. Местные-то - скупщики значит, в джунглях спрятались, а капитан морем ушел. Как есть, с носом нашу эскадру оставил! Так из бухты вырулил - любой лоцман позавидует!",
    ...
    case "sleep_in_tavern":
        ...
        if (CheckAttribute(pchar, "GenQuest.LigaAttack.Go"))
            DoQuestReloadToLocation(pchar.location + "_upstairs", "goto", "goto1", "LigaAttack_LoginKillers"); //лига ночных убийц атакует
        else
    ...
    case "LandEnc_RaidersBeforeDialog": //если выстрел до диалога
        ...
        for (i = 0; i < 4; i++)
        {
            if (GetCharacterIndex(sTemp + i) == -1)
                continue;
            sld = CharacterFromID(sTemp + i);
            LAi_RemoveCheckMinHP(sld);
    ...
    case "LandEnc_RaidersAfrer": //чистим за собой
        ...
        AddSimpleRumour(LinkRandPhrase("Вы слышали, капитан " + GetFullName(pchar) + " схватил" + GetSexPhrase("ся", "ась") + " с бандитом по имени " + pchar.GenQuest.(Gun).name + ". Е" + GetSexPhrase("го", "е") + " хотели ограбить, а он" + GetSexPhrase("", "а") + " не захотел" + GetSexPhrase("", "а") + " платить. Смел" + GetSexPhrase("ый кэп", "ая девушка") + " - он" + GetSexPhrase("", "а") + " просто его убил" + GetSexPhrase("", "а") + ".",
                                       ...
                                       "Вы не слышали о том, что местная шайка бандитов под предводительством негодяя " + pchar.GenQuest.(Gun).name + " напала на капитана по имени " + GetFullName(pchar) + "? Т" + GetSexPhrase("от", "а") + " отказал" + GetSexPhrase("ся", "ась") + " платить и перебил" + GetSexPhrase("", "а") + " бандитов."),
                        sti(pchar.GenQuest.(Gun).nation), 5, 1);
    ...
    case "LandEnc_RapersBeforeDialog": //если выстрел до диалога
        if (GetCharacterIndex("CangGirl") != -1)
            LAi_RemoveCheckMinHP(CharacterFromID("CangGirl"));
        for (i = 1; i <= 3; i++)
        {
            if (GetCharacterIndex("GangMan_" + i) == -1)
                continue;
    ...
    case "LandEnc_RapersBadExit":
        if (GetCharacterIndex("CangGirl") == -1) // если ГГ собственноручно прибил тетку
        {
            ...
            AddSimpleRumour(LinkRandPhrase("Вы слышали, кто-то убил в джунглях несчастную девушку по имени " + pchar.GenQuest.EncGirl.name + ". Отловить бы мерзавца и вздернуть!",
                                           ...
                                           "Ее звали " + pchar.GenQuest.EncGirl.name + ", и она погибла в джунглях. И ничего удивительного, скажу я вам, за городом так неспокойно..."),
                            sti(pchar.GenQuest.EncGirl.nation), 5, 1);
            for (i = 1; i <= 3; i++)
            {
                if (GetCharacterIndex("GangMan_" + i) == -1)
                    continue;
        ...
        else
        {
                ...
                else
                {
                    if (!CheckAttribute(pchar, "GenQuest.EncGirl.Ransom"))
                    {
                        ...
                        AddSimpleRumour(LinkRandPhrase("Вы слышали, " + GetSexPhrase("какой-то капитан", "какая-то морячка") + " спас" + GetSexPhrase("", "ла") + " девушку по имени " + pchar.GenQuest.EncGirl.name + " от бандитов, буквально вытащил ее из их лап!" + GetSexPhrase(" Какой прекрасный человек и настоящий мужчина.", "") + "",
                                                       ...
                                                       "Девушке по имени " + pchar.GenQuest.EncGirl.name + " здорово повезло - недавно на нее напали бандиты в джунглях, хотели изнасиловать, видать. Но ничего у них не вышло, хе-хе, и благодарить она должна капитана " + GetMainCharacterNameGen() + "!"),
            ...
            else // оставил на растерзание бандитам
            {
                ...
                AddSimpleRumour(RandPhraseSimple("Да уж, капитан " + GetFullName(pchar) + ", никто не думал, что вы способны оставить беззащитную женщину на поругание бандитам. Бедная " + pchar.GenQuest.EncGirl.name + ", у меня нет слов...",
                                                 "Знаете, капитан, " + GetSexPhrase("вы форменный мерзавец", "не ожидал от вас такого") + ". Как вы могли бросить беззащитную девушку в джунглях? Вы хоть представляете себе, что они с ней сделали? Бедная " + pchar.GenQuest.EncGirl.name + ", ее теперь никто не возьмет замуж..."),
                                sti(pchar.GenQuest.EncGirl.nation), 5, 1);
    ...
    case "Enc_FriendGirl_after": //если выстрел до диалога
        ...
        if (rand(1))
        {
            AddSimpleRumour(LinkRandPhrase("Вы слышали, какой-то капитан убил в джунглях несчастную девушку по имени " + pchar.GenQuest.EncGirlF.name + ". Отловить бы мерзавца и вздернуть!",
                                           ...
                                           "Ее звали " + pchar.GenQuest.EncGirlF.name + ", и она погибла в джунглях. И ничего удивительного, скажу я вам, за городом так неспокойно..."),
                            sti(pchar.GenQuest.EncGirlF.nation), 5, 1);
        }
        break;
    //------------------- патруль в джунглях -------------------------
    ...
    case "LandEnc_PatrolAfrer": //чистим за собой
        ...
        AddSimpleRumour(LinkRandPhrase("Недавно за городом нашли мертвым нашего офицера и приданных ему солдат. Кто мог их убить? Зачем?..",
                                       ...
                                       "Черт возьми, кто-то за городом перебил наши солдат, что патрулировали территорию..."),
    ...
    case "LandEnc_PatrolOver": //чистим за собой
        ...
        AddSimpleRumour(LinkRandPhrase("Вы слышали, капитана " + GetMainCharacterNameGen() + " проверял наш патруль за городом...",
                                       ...
                                       "Наш комендант усилит патрулирование прилегающей к городу территории. Ну вы и сами об этом знаете, вас же недавно проверяли в джунглях..."),
    ...
    case "DestroyGang_fight":
        if (rand(1))
        {
            ...
            if (iTemp <= 0)
                iTemp = 1; //если локаторов меньше четырех
            //--> генерим ранг
            if (sti(pchar.rank) > 6)
            {
                if (sti(pchar.rank) > 20)
                    Rank = sti(pchar.rank) + sti(MOD_SKILL_ENEMY_RATE * 4 / iTemp);
                else
                    Rank = sti(pchar.rank) + sti(MOD_SKILL_ENEMY_RATE * 2.5 / iTemp);
            }
            else
            ...
            while (i < iTemp)
            {
                ...
                if (iModel[ShipType] != "")
                {
                    ...
                    { //главарь банды
                        ...
                        sld = GetCharacter(NPC_GenerateCharacter("MayorQuestGang_" + i, iModel[ShipType], "man", "man", Rank, PIRATE, 0, true, "marginal"));
                    LAi_SetActorType(sld);
                    ...
                    if (i == 0)
                        ...
                        LAi_ActorFollow(sld, &characters[iMainGang], "", -1);
                    i++;
                    iModel[ShipType] = "";
                }
            }
    ...
    case "DestroyGang_Afrer":
        ...
        AddSimpleRumour(LinkRandPhrase("Вы знаете, губернатор " + sTemp + " поручил неко" + GetSexPhrase("ему капитану", "й морячке") + " " + GetMainCharacterNameDat() + " уничтожить известного бандита по имени " + GetFullName(arAll) + ". И, кажется, что " + GetSexPhrase("капитан выполнил", "она выполнила") + " задание губернатора...",
                                       ...
                                       "Ха, наконец-то наш губернатор взялся за наведение порядка в джунглях - он поручил капитану " + GetMainCharacterNameDat() + " разыскать в джунглях и убить наиболее известного бандита. И вы знате, этот бандит, " + GetFullName(arAll) + " - мертв!"),
                        sti(characters[GetCharacterIndex(arAll.MayorId)].nation), 5, 1);
        break;
    ////////////////////////////////////////////////////////////////////////
    ...
    case "GivePrisonFree_Death":
        ...
        Pchar.quest.DeletePrisonGroup.win_condition = "DeletePrisonGroup" break;
    ...
    case "DeliverToBander":
        if (CheckAttribute(pchar, "questTemp.jailCanMove.Deliver.City") && pchar.questTemp.jailCanMove.Deliver.City == locations[reload_location_index].fastreload)
        {
            ...
            if (sti(pchar.rank) > 6)
                Rank = 8 + sti(pchar.rank) + MOD_SKILL_ENEMY_RATE;
            else
    ...
    case "MayorSitBack": //посадить мэра обратно
        iTemp = GetCharacterIndex(Pchar.quest.MayorSitBack.mayorId) if (iTemp > 0)
        {
    ...
    case "Badboy_die":
        if (CheckAttribute(PChar, "quest.BadboyPlace"))
            Pchar.quest.BadboyPlace.over = "yes";
        if (CheckAttribute(PChar, "quest.BadboyDuelTimer"))
            Pchar.quest.BadboyDuelTimer.over = "yes";
    ...
    case "Guide_HeroWin": // ГГ выиграл фехт
        ...
        sld = characterFromId("Guide");
        if (sti(sld.quest.guide_win == 0))
            sld.dialog.currentnode = "guide_57"; // с первого раза побил
        else
            sld.dialog.currentnode = "guide_58";
        LAi_SetActorType(sld);
        LAi_ActorTurnToCharacter(sld, pchar);
        LAi_ActorDialogDelay(sld, pchar, "", 1.0);
        break;
    ...
    case "Guide_HeroFail": // ГГ проиграл фехт
        ...
        if (sti(sld.quest.guide_win == 1))
            sld.dialog.currentnode = "guide_59"; // один раз побили
        else
        {
            if (sti(sld.quest.guide_win == 2))
                sld.dialog.currentnode = "guide_60"; // два раза побили
            else
                sld.dialog.currentnode = "guide_61"; //три раза побили
        }
        LAi_SetActorType(sld);
        LAi_ActorTurnToCharacter(sld, pchar);
        LAi_ActorDialogDelay(sld, pchar, "", 1.0);
    ...
    case "Without_consciousness":
        ...
        if (2 - sti(RealShips[sti(pchar.ship.type)].basetype) < 0)
            pchar.Ship.Type = SHIP_NOTUSED;
    ...
    case "Saga_Gunfire": // выстрел
        ...
        if (sld.quest.canonada == "1" || sld.quest.canonada == "2")
            ftime = 2.5;
    ...
    case "Saga_GunAutoExplode": // взрыв
        ...
        if (iTotalTemp > 7)
            CreateLocationParticles("smoke", "goto", "burn" + iTotalTemp, 0, 0, 0, "");
        if (iTotalTemp == 2)
        {
            for (i = 5; i <= 6; i++)
            {
    ...
    case "Saga_MineBandos_05Die": // генерим партию бандюков в шесть рыл
        ...
        for (i = 1; i <= 6; i++)
        {
            ...
            LAi_group_MoveCharacter(sld, "EnemyFight");
            if (MOD_SKILL_ENEMY_RATE > 4)
                sld.MultiFighter = 0.5 + fMft; // мультифайтер
        }
        LAi_group_SetRelation("EnemyFight", LAI_GROUP_PLAYER, LAI_GROUP_ENEMY);
        LAi_group_FightGroups("EnemyFight", LAI_GROUP_PLAYER, true);
    ...
    case "LSC_NarvalConflict": // конфликт с нарвалами
        ...
        log_Testinfo("Конфликт с нарвалами достиг " + sti(pchar.GenQuest.NarvalConflict) + " ступени") break;
    ...
    case "LSC_RivadosConflict": // конфликт с ривадос
        ...
        log_Testinfo("Конфликт с ривадос достиг " + sti(pchar.GenQuest.RivadosConflict) + " ступени") break;
    ...
    case "LSC_SharkConflict": // конфликт с пиратами
        ...
        log_Testinfo("Конфликт с пиратами достиг " + sti(pchar.GenQuest.SharkConflict) + " ступени") break;
    ...
    case "LSC_MaryReturnNormal": // вертаем Мэри в норму
        ...
        if (CheckAttribute(pchar, "questTemp.LSC.MaryBye"))
            sld.dialog.currentnode = "LSC_love";
        else
            sld.dialog.currentnode = "LSC_love_3";
        LAi_SetOwnerType(sld);
        chrDisableReloadToLocation = false; //открыть локацию
        break;
    ...
    case "LSC_MaryReturnNormal_1": // вертаем Мэри в норму
        ...
        if (CheckAttribute(pchar, "questTemp.LSC.Mary_officer"))
            LAi_SetOfficerType(sld); // patch-10
        break;
    ...
    case "LSC_RingEnterInside":            // входим в трюм разбитого корабля
        ...
        for (i = 1; i <= 10; i++)
        {
            ...
            EquipCharacterbyItem(sld, "unarmed");
            if (MOD_SKILL_ENEMY_RATE > 4)
                sld.MultiFighter = stf(MOD_SKILL_ENEMY_RATE / 2.5);
            sld.SaveItemsForDead = true;
            sld.animal = true;
            LAi_SetHP(sld, iTemp, iTemp);
    ...
    case "AlexClock_BandosDie":
        LAi_group_Delete("EnemyFight");
        if (CheckCharacterItem(pchar, "alexs_chest"))
            DoQuestFunctionDelay("AlexClock_FindChest", 0.5);
        else
        {
    ...
    case "GuardOT_ArriveGaleon":
        ...
        if (sti(environment.time) < 21 && sti(environment.time) > 3)
        {
            ...
            if (iTime >= 21)
                iAddTime = 24 - iTime + 21;
            if (iTime < 7)
                iAddTime = 21 - iTime;
            if (iTime >= 7 && iTime < 21)
                iAddTime = 24 + 21 - iTime;
            StoreDayUpdate();
            WaitDate("", 0, 0, 0, iAddtime, 5);
            RecalculateJumpTable();
            RefreshWeather();
    ...
    case "GuardOT_WillyEscape":
        ...
        if (iTemp > 4)
            DoQuestCheckDelay("GuardOT_WillyMushketerDie", 5.0);
        else
    ...
    case "Ksochitam_FindFirstWay":
        ...
        sld = characterFromId("Jino");
        sld.dialog.currentnode = "ksochitam_25" pchar.quest.Ksochitam_jinotalk1.win_condition.l1 = "location";
    ...
    case "Ksochitam_FindFullWay":
        ...
        sld = characterFromId("Jino");
        sld.dialog.currentnode = "ksochitam_31" pchar.quest.Ksochitam_jinotalk3.win_condition.l1 = "location";
    ...
    case "Ksochitam_ShoreMaskAddSkeletons":
        if (CheckAttribute(pchar, "questTemp.Ksochitam.GuardMaskBeat"))
            break;
        log_testinfo("Скелеты создаются!!");
        PlaySound("Ambient\Teno_inside\teleporter.wav");
        if (MOD_SKILL_ENEMY_RATE < 6)
            iTotalTemp = iTotalTemp - 1;
        iRank = 25 + MOD_SKILL_ENEMY_RATE * 2;
        iScl = 70;
    ...
    case "Tieyasal_PrepareDichosoTrap":
        ...
        if (CheckAttribute(pchar, "questTemp.Tieyasal.MigelKnow"))
            AddQuestRecord("Tieyasal", "8");
        else
            AddQuestRecord("Tieyasal", "7");
    ...
    case "Tieyasal_CryptBattleComplete":
        if (CheckAttribute(pchar, "questTemp.Tieyasal.MigelKnow"))
            AddQuestRecord("Tieyasal", "11");
        else
            AddQuestRecord("Tieyasal", "10");
        DeleteAttribute(pchar, "GenQuest.Notsearchbody");
        pchar.GenQuest.NoDelBody = true;
        break;
    ...
    case "Tieyasal_ItzaWarriorPause":
        if (pchar.questTemp.Tieyasal.ItzaWarrior == "1")
            DoQuestCheckDelay("Tieyasal_CreateItzaWarriorSecondGroup", 7.0);
        else
            DoQuestCheckDelay("Tieyasal_CreateItzaWarriorThirdGroup", 9.0);
        break;
    ...
    case "Caleuche_GhostshipBoardingDeck": // расстановка абордажных групп
        ...
        for (i = 1; i <= n; i++)
        {
            ...
            LAi_SetWarriorType(sld);
            if (i < 8)
                ChangeCharacterAddressGroup(sld, "Deck_Galeon_Ship", "quest", "quest1");
            else
                ChangeCharacterAddressGroup(sld, "Deck_Galeon_Ship", "quest", "quest2");
            LAi_group_MoveCharacter(sld, LAI_GROUP_PLAYER);
        }
        // враги
        for (i = 1; i <= 20; i++)
        {
            ...
            LAi_SetWarriorType(sld);
            if (i < 8)
                ChangeCharacterAddressGroup(sld, "Deck_Galeon_Ship", "quest", "quest3");
            else
                ChangeCharacterAddressGroup(sld, "Deck_Galeon_Ship", "quest", "quest4");
            LAi_group_MoveCharacter(sld, "EnemyFight");
        }
        LAi_group_SetRelation("EnemyFight", LAI_GROUP_PLAYER, LAI_GROUP_ENEMY);
        LAi_group_FightGroups("EnemyFight", LAI_GROUP_PLAYER, true);
    ...
    case "Caleuche_MerrimanDie":                     // прибили Мерримана
        ...
        for (i = 1; i <= 11; i++)
        {
            if (GetCharacterIndex("merriman_skel_" + i) != -1)
            {
                sld = characterFromId("merriman_skel_" + i);
                if (LAi_GetCharacterHP(sld) > 0)
                    LAi_KillCharacter(sld); // прибьем скелетов
            }
        }
        sld = characterFromId("MerrimanChavinavi" + iGlobalTemp);
        if (LAi_GetCharacterHP(sld) > 0)
            LAi_KillCharacter(sld); // прибьем чавинави
        // на нефритовый череп
        pchar.GenQuest.NoDelBody = true;
    ...
    case "Caleuche_LeverFightOver": // прибили чавинави у рычагов
        ...
        if (sti(pchar.questTemp.Caleuche.LeverType) == 1)
    ...
    case "FMQN_HollandBattleFightAdd": // голландцы спешат на помощь
        ...
        for (i = 0; i <= 7; i++)
        {
            ...
            LAi_group_MoveCharacter(sld, LAI_GROUP_PLAYER);
            if (i == 6 || i == 7)
                ChangeCharacterAddressGroup(sld, "Marigo_Cave", "reload", "reload1");
            else
                Log_TestInfo(i + "-й солдат ждет");
        }
    ...
    case "FMQN_AfterShoreAttack": // после боя в бухте
        ...
        if (LAi_GetCharacterHP(sld) > 0)
        {
            ...
            pchar.questTemp.FMQN = "caspar_dead";
        for (i = 2; i <= 5; i++)
        {
            sld = CharacterFromID("FMQN_seafox_" + i);
            sld.lifeday = 0;
    ...
    case "FMQL_JungleExit": //
        ...
        DeleteAttribute(pchar, "GenQuest.MapClosedNoBattle");
        if (pchar.questTemp.FMQL == "shore")
            AddQuestRecord("FMQ_Lesson", "3");
        else
        {
            if (pchar.questTemp.FMQL == "poor")
                AddQuestRecord("FMQ_Lesson", "6");
            else
    ...
    case "FMQL_AfterCaribAttack": // после боя в джунглях
        ...
        else
        {
            ...
            if (iTotalTemp < 3)
            {
                ...
                sld.dialog.currentnode = "greguar_13";
            LAi_SetActorType(sld);
            LAi_ActorDialog(sld, pchar, "", -1, 0);
        }
        break;
    ...
    case "FMQL_EngDestroy": //
        ...
        if (CheckAttribute(pchar, "questTemp.FMQL.HolDestroy"))
            DoQuestCheckDelay("FMQL_CoastClear", 1.0);
        break;

    case "FMQL_HolDestroy": //
        pchar.questTemp.FMQL.HolDestroy = "true";
        if (CheckAttribute(pchar, "questTemp.FMQL.EngDestroy"))
            DoQuestCheckDelay("FMQL_CoastClear", 1.0);
        break;
    ...
    case "Mtraxx_CartahenaExittownBattle": // бой на выходе из города
        ...
        for (i = 5; i <= 10; i++) // буканьеры Леприкона
        {
            ...
            if (i < 10)
                ChangeCharacterAddressGroup(sld, "Cartahena_ExitTown", "rld", "aloc" + (i - 4));
            else
                ChangeCharacterAddressGroup(sld, "Cartahena_ExitTown", "rld", "aloc11");
            LAi_CharacterDisableDialog(sld);
            LAi_SetWarriorType(sld);
            LAi_group_MoveCharacter(sld, LAI_GROUP_PLAYER);
            sld.lifeday = 3;
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (MOD_SKILL_ENEMY_RATE > 2)
                        sld.cirassId = Items_FindItemIdx("cirass2");
                    if (i > 5 && i < 11)
                        ChangeCharacterAddressGroup(sld, "Cartahena_ExitTown", "rld", "aloc" + i);
                    else
                        ChangeCharacterAddressGroup(sld, "Cartahena_ExitTown", "rld", "aloc14");
                }
            }
            LAi_SetWarriorType(sld);
            LAi_group_MoveCharacter(sld, "EnemyFight");
    ...
    case "Mtraxx_RetributionInBrothel": // в комнате с девочками
        ...
        for (i = 1; i <= 6; i++)
        {
            ...
            SetFantomParamFromRank(sld, 1, true);
            if (i < 3)
                ChangeCharacterAddressGroup(sld, "Tortuga_Brothel_room", "goto", "goto" + i);
            else
    ...
    case "Mtraxx_RetributionSleep": // снится Мерида
        ...
        {
            if (i < 5)
                sld = GetCharacter(NPC_GenerateCharacter("Sleep_Meridavictim_" + i, "women_" + (9 + i), "woman", "towngirl", 1, SPAIN, 1, true, "quest"));
            else
                sld = GetCharacter(NPC_GenerateCharacter("Sleep_Meridavictim_" + i, "citiz_1" + i, "man", "man", 1, SPAIN, 1, true, "quest"));
            ChangeCharacterAddressGroup(sld, "Merida_town", "quest", "quest" + i);
            LAi_SetActorType(sld);
            if (i < 8)
                LAi_ActorGoToLocation(sld, "merchant", "merchant3", "none", "", "", "", 40);
            else
                LAi_ActorGoToLocation(sld, "merchant", "merchant3", "none", "", "", "Mtraxx_RetributionSleep1", 40);
        }
        break;
    ...
    case "LongHappy_MarryInit":                      //
        ...
        if (CheckCharacterItem(pchar, "suit1"))
    ...
    case "LongHappy_InSPcabin": //
        ...
        if (CheckAttribute(pchar, "questTemp.LongHappy.HambitOfficer"))
        {
            ...
            ChangeCharacterAddressGroup(sld, "none", "", "");
            LAi_SetOfficerType(sld);
            if (pchar.questTemp.LongHappy.HambitOfficer == "Longway")
                sld.Dialog.Filename = "Quest\HollandGambit\Longway.c";
            if (pchar.questTemp.LongHappy.HambitOfficer == "Knippel")
                sld.Dialog.Filename = "Quest\HollandGambit\Knippel.c";
            if (pchar.questTemp.LongHappy.HambitOfficer == "Tonzag")
    ...
    case "LongHappy_InSPComplete": //
        ...
        iTime = sti(environment.time);
        if (iTime >= 21)
            iAddTime = 24 - iTime + 12;
        if (iTime < 7)
            iAddTime = 12 - iTime;
        if (iTime >= 7 && iTime < 21)
            iAddTime = 24 + 12 - iTime;
        StoreDayUpdate();
        WaitDate("", 0, 0, 0, iAddtime, 5);
        RecalculateJumpTable();
        RefreshWeather();
    ...
    case "LongHappy_IslaTesoroTavern":    //
        ...
        sld.Dialog.Filename = "Quest\LongHappy.c";
        if (CheckAttribute(pchar, "questTemp.Saga.Helena_officer"))
            sld.dialog.currentnode = "Svenson_2";
        else
            sld.dialog.currentnode = "Svenson_7";
    ...
    case "LongHappy_IslaTesoroBedroom": //
        LAi_SetPlayerType(pchar);
        if (CheckAttribute(pchar, "questTemp.GoldenGirl.Angerran"))
            sld = characterFromId("Angerran"); // fix 02-04-20
        else
    ...
    case "LongHappy_BarbExit": //
        if (CheckAttribute(pchar, "questTemp.LongHappy.Terrax.Prisoner"))
            pchar.questTemp.LongHappy.Terrax.Free = "true";
    ...
    case "LongHappy_WifeInTown": //
        ...
        if (CheckAttribute(pchar, "questTemp.LongHappy.Tichingitu_Rum"))
            sld.dialog.currentnode = "LongHappy_52";
        else
            sld.dialog.currentnode = "LongHappy_49";
        ChangeCharacterAddressGroup(sld, "Pirates_town", "rld", "loc1");
        LAi_SetActorType(sld);
        LAi_ActorDialog(sld, pchar, "", -1, 0);
        if (CheckAttribute(pchar, "questTemp.LongHappy.Tichingitu_Victim") && !CheckAttribute(pchar, "questTemp.LongHappy.Tichingitu_died"))
            LongHappy_SetTichingituDelay();
        break;
    ...
    case "LongHappy_IslaMarryComplete": //
        ...
        LAi_group_MoveCharacter(sld, LAI_GROUP_PLAYER);
        if (CheckAttribute(pchar, "questTemp.LSC.Mary_officer"))
            sld.CompanionDisable = true; // fix 25-03-20 блок компаньона у Мэри
        CloseQuestHeader("LongHappy");
        pchar.questTemp.LongHappy = "end";
        break;
    }
...
void WaitNextHours(string qName)
{
    ...
    if (sti(pchar.quest.waithours) == 1)
        sHour = "Прошел один час...";
    if (sti(pchar.quest.waithours) == 24)
        sHour = "Прошли одни сутки...";
...
void Flag_PIRATE()
{
    ...
    // сторожевики Тортуги
    if (pchar.location == "Tortuga" && !CheckAttribute(pchar, "questTemp.Sharlie.DelTerGuard"))
