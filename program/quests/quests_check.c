...
bool ProcessCondition(aref condition, int n)
{
    ...
    if (CheckAttribute(condition, "character"))
    {
        ...
        if (i != -1)
            condition.characterIdx = i;
        else
            condition.characterIdx = sti(condition.character); // делаем вывод, что это число
        // совместимость с кодом акеллы, где в character хранят не ID (строка), а IDX (число) <--
    ...
    case "Timer":
        if (GetDataYear() < sti(condition.date.year))
            return false;
        if (GetDataYear() > sti(condition.date.year))
            return true;
        if (GetDataMonth() < sti(condition.date.month))
            return false;
        if (GetDataMonth() > sti(condition.date.month))
            return true;
        if (GetDataDay() < sti(condition.date.day))
            return false;
        if (CheckAttribute(condition, "date.hour") && GetDataDay() <= sti(condition.date.day)) //fix
        {
            if (GetHour() < stf(condition.date.hour))
                return false;
            if (GetHour() >= stf(condition.date.hour))
                return true;
        }
    ...
    case "Hour":
        if (stf(environment.time) >= stf(condition.start.hour) && stf(environment.time) < stf(condition.finish.hour))
            return true;
        return false;
        break;
    ...
    case "HardHour":
        if (stf(environment.time) >= stf(condition.hour))
            return true;
        return false;
        break;
    ...
    case "Nation":
        if (sti(pchar.nation) == sti(condition.nation))
            return true;
        return false;
        break;
        //<-- прерывание на нацию
    ...
    case "Location_Type":
        ...
        {
            if (loadedLocation.type == condition.location_type)
                return !CharacterIsDead(refCharacter);
        }
        return false;
        break;
        ...
        if (IsEntity(loadedLocation))
        {
            if (loadedLocation.type == "town")
            {
                ...
                if (CheckAttribute(&Locations[iLocation], "fastreload"))
                {
                    ...
                    if (iNation == sti(Colonies[FindColony(sLocation)].nation))
                        return true;
                }
            }
        }
    ...
    case "Character_Capture":
        if (CheckAttribute(refCharacter, "Killer.status") && sti(refCharacter.Killer.status) == KILL_BY_ABORDAGE)
            return true;
        return false;
        break;
    ...
    //a&m <--  03/02
    case "Ship_location":
        if (CheckAttribute(refCharacter, "location.from_sea") && refCharacter.location.from_sea == condition.location)
            return true;
        return false;
        break;
    case "Group_Death":
...
void QuestsCheck()
{
    if (bQuestCheckProcess || bQuestCheckProcessFreeze || dialogRun)
        return; //boal
    ...
    {
        if (bQuestCheckProcessFreeze || dialogRun)
            continue; // boal 230804 fix замораживать проверку квестов
