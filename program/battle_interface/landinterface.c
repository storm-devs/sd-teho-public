...
void BLI_ExecuteCommand()
{
    ...
    case "BI_FastReload": {
        ...
        if (FindUserIcon(evntName, &uiref))
        {
            ...
            if (CheckAttribute(uiref, "locator"))
            {
                sTravelLocator = uiref.locator;
            }
...
{
    aref arroot, ar;
    makearef(arroot, objLandInterface.UserIcons);
...
void BLI_UpdateObjectData()
{
    ...
    if (sti(objLandInterface.data.Alarm) != bIsRiskAlarm)
...
void BLI_SetPossibleCommands()
{
    ...
    if (chrMode == 0)
    {
        ...
        ///// boal -->
        i = -1;
        if (CheckAttribute(pchar, "dialogready"))
            i = sti(pchar.dialogready);
        if (i >= 0)
        {
            objLandInterface.Commands.DialogStart.enable = true;
...
void procFindDialogChar()
{
    ...
    int oldDlgChar = -1;
    if (CheckAttribute(pchar, "dialogready"))
        oldDlgChar = sti(pchar.dialogready);
...
{
    aref ar;
    makearef(ar, objLandInterface.Parameters);
...
string GetNodeForIcon(int nTex, int nPic)
{
    ...
    if (CheckAttribute(objIconsNote, attrName))
        return objIconsNote.(attrName);
    return "";
}
