...
void LI_SetWindowSize()
{
    ...
    if (bTVused)
        SetShowWindowParameters(bTVused, w, h, 40, 24, w - 40, h - 24);
    else
        SetShowWindowParameters(bTVused, w, h, 0, 0, w, h);
}
...
void BI_FastCommand()
{
    ...
    if (ILogAndActions.type == "sea")
    {
        ...
        case "Map":
            bEC = true;
            Sea_MapLoad();
            break; //  pchar.location = ""; тут нах не нужно
        case "Reload":
