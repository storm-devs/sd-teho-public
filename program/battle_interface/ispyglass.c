...
void SetSpyGlassData()
{
        ...
        if (iShip != SHIP_NOTUSED)
        {
            ...
            if (ShipsTypes[iShip].name == "fort")
            {
                ...
                if (CheckAttribute(arScopeItm, "scope.show.cannons") && sti(arScopeItm.scope.show.cannons) != 0)
                {
                    ...
                    if (!CheckAttribute(chref, "fort.Cannons.Quantity"))
                    {
                        shipMaxCannons = 0;
                    }
                    else
                    {
                        shipMaxCannons = sti(chref.Fort.Cannons.Quantity);
                    }
                }
    ...
    if (bBettaTestMode)
    {
        ...
        if (CheckAttribute(chref, "SeaAI.Task"))
        {
            ...
            {
                sCaptainName = "none";
                if (sti(chref.SeaAI.Task.Target) != -1)
                    sCaptainName = Characters[sti(chref.SeaAI.Task.Target)].id;
        ...
        if (CheckAttribute(chref, "EncType"))
            Log_Info("EncType = " + chref.EncType);
        if (CheckAttribute(chref, "Ship.Mode"))
            Log_Info("Ship.Mode = " + chref.Ship.Mode);
        if (CheckAttribute(chref, "Ship.LastBallCharacter"))
            Log_Info("LastBallCharacter = " + chref.Ship.LastBallCharacter);
        if (CheckAttribute(arScopeItm, "scope.show.mushketshot") && sti(arScopeItm.scope.show.mushketshot) != 0 && CheckOfficersPerk(chref, "MusketsShoot"))
            Log_Info("MushketShot enabled");
        if (CheckAttribute(chref, "Situation"))
            Log_Info("Cap. Situation : " + chref.situation + " type :" + chref.situation.type);
        if (CheckAttribute(chref, "DontRansackCaptain"))
            Log_Info("Не сдастся!");
        if (CheckAttribute(chref, "surrendered"))
            Log_Info("Уже сдался!");
        if (CheckAttribute(chref, "ShipHideImmortal"))
            Log_Info("Непотопляемый!");
        if (CheckAttribute(chref, "SeaBoss"))
            Log_Info("Морской босс: урон - " + stf(chref.SeaBoss) + "");
