...
ref BI_GetFortRelation()
{
    ...
    if (chrIdx >= 0)
    {
        ...
        case RELATION_FRIEND:
            BI_intRetValue = BI_RELATION_FRIEND;
            break;
        case RELATION_NEUTRAL:
            BI_intRetValue = BI_RELATION_NEUTRAL;
            break;
        case RELATION_ENEMY:
            BI_intRetValue = BI_RELATION_ENEMY;
            break;
        }
    }
...
ref BI_GetSSShow()
{
    ...
    case CHEAP_SPYGLASS:
        BI_intRetValue = false;
        break;
    case COMMON_SPYGLASS:
        BI_intRetValue = true;
        break;
    case GOOD_SPYGLASS:
        BI_intRetValue = true;
        break;
    case SUPERIOR_SPYGLASS:
        BI_intRetValue = true;
        break;
    }
    return &BI_intRetValue;
...
void BI_LaunchCommand()
{
    ...
    case "BI_Bomb":
        if (GetCargoGoods(chRef, GOOD_POWDER) >= 200 && GetRemovable(chRef))
            SetMineFree(chRef, 1); // fix ugeen 21.12.13
        else
            PlaySound("interface\knock.wav");
        break;
...
void BI_CreateShip()
{
    int charIndex = GetEventData();
    if (charIndex >= 0)
        ClearActiveChrPerks(GetCharacter(charIndex));
...
ref BI_GetData()
{
    ...
    case BIDT_SHIPPICTURE:
        ...
        if (!CheckAttribute(&RealShips[st], "basetype"))
            Log_TestInfo("BIDT_SHIPPICTURE нет basetype у корабля НПС ID=" + chRef.id);
    ...
    if (!bSeaActive || mchar.location == mchar.SystemInfo.CabinType) // спим в каюте
    {
        for (i = 0; i < COMPANION_MAX; i++)
        {
            ...
            if (CheckAttribute(chref, "Ship.HP_penalty"))
            {
                ...
                if (hp > makeint(stf(chref.Ship.HP_penalty)))
                    RealShips[sti(chref.ship.type)].HP = hp - makeint(stf(chref.Ship.HP_penalty));
                else
                    RealShips[sti(chref.ship.type)].HP = 1;
...
bool CheckRepairPerks(ref chref)
{
    if (CheckOfficersPerk(chref, "Carpenter") ||
        ...
        CheckOfficersPerk(chref, "SelfRepair"))
        return true;
    return false;
}
            ...
            if (CheckRepairPerks(chref))
            {
                ...
                if (GetHullPercent(chref) < 100.0)
                {
                    ...
                    if (tmpf > 0)
                    {
                        if (tmpf < matQ)
                        {
                            repPercent = tmpf / GetHullPPP(chref);
                        }
                ...
                if (GetSailPercent(chref) < GetAllSailsDamagePercent(chref))
                {
                    ...
                    if (tmpf > 0)
                    {
                        if (tmpf < matQ)
                        {
                            repPercent = tmpf / GetSailSPP(chref);
                        }
...
ref procGetRiggingData()
{
    ...
    if (datName == "GetShipFlagTexNum")
    {
        ...
        {
            if (s == 1)
                BI_intNRetValue[1] = GetShipFlagTextureNum(rCharacter);
            else
                BI_intNRetValue[1] = FLAG_CMN;
        }
    }
    ...
    if (datName == "GetTownFlagTexNum")
    {
        ...
        {
            if (s == 1)
                BI_intNRetValue[1] = FLAG_FRT;
            else
                BI_intNRetValue[1] = FLAG_FRT; // приходится вот так вот изгаляться, тк не все модели тулом читаются :(
        }
    }
    return &BI_intNRetValue;
}
...
{
    if (!CheckAttribute(rModel, "fortcmdridx"))
        return;
    ...
    else
    {
        ...
        else
        {
            ...
            else
            {
                ...
                if (IsShipCommander(chr))
                    return FLAG_SHP;
                else
                {
                    // купчишки
                    if (IsShipMerchant(chr))
                        return FLAG_MER;
...
bool CheckInstantRepairCondition(ref chref)
{
    ...
    if (CheckOfficersPerk(chref, "InstantRepair"))
        return false; // уже в деле
    if (!GetOfficersPerkUsing(chref, "InstantRepair"))
        return false; // можно включить?
                      //boal <--
    ...
    if (bYesHPRepair)
    {
        bYesHPRepair = GetCargoGoods(chref, GOOD_PLANKS) > 0;
    }
    if (bYesSPRepair)
    {
        bYesSPRepair = GetCargoGoods(chref, GOOD_SAILCLOTH) > 0;
    }
...
void RemoveRepairGoods(bool bIsHull, ref chref, float matQ)
{
    ...
    if (bIsHull)
    {
        if (fGoodsQ <= matQ)
        {
            DeleteAttribute(chref, "RepairMaterials.forHull");
        }
        else
        {
            fGoodsQ -= matQ;
    ...
    else
    {
        if (fGoodsQ <= matQ)
        {
            DeleteAttribute(chref, "RepairMaterials.forSails");
        }
        else
        {
            fGoodsQ -= matQ;
...
float GetRigDamage(int shootIdx, int iBallType, ref damage_chr)
{
    ...
    if (shootIdx >= 0)
    {
        ...
        {
            if (CheckOfficersPerk(shoot_chr, "SailsDamageUp"))
            {
                fDmgRig *= 1.15;
            }
        }
    ...
    {
        if (CheckOfficersPerk(damage_chr, "AdvancedBattleState"))
        {
            fDmgRig *= 0.75;
        } // fix
        else
        {
            if (CheckOfficersPerk(damage_chr, "BasicBattleState"))
            {
                fDmgRig *= 0.85;
            } // fix
        }
    ...
    if (CheckAttribute(damage_chr, "SeaBoss"))
    {
        fDmgRig *= 0.1;
    }
...
ref ProcessRandomSailDmg()
{
    ...
    int hc_old = 0;
    if (CheckAttribute(arSail, "hc"))
        hc_old = sti(arSail.hc);
    float fDmg_old = 0.0;
    if (CheckAttribute(arSail, "dmg"))
        fDmg_old = stf(arSail.dmg);
    ...
    if (itmp < mhc)
        fTopDmg = GetNeedDmgFromHole(itmp + 1, sailDmgMax, mhc) - 0.0001;
    if (fTopDmg > fDmg_old + fDmg)
        fTopDmg = fDmg_old + fDmg;
...
ref procGetSRollSpeed()
{
    ...
    BI_g_fRetVal = 0.0;
    if (chrIdx >= 0)
        BI_g_fRetVal = GetRSRollSpeed(GetCharacter(chrIdx));
    return &BI_g_fRetVal;
}
...
float GetRSRollSpeed(ref chref)
{
    ...
    if (crewOpt <= 0)
        crewOpt = 0; // fix для профилактики деления на ноль
...
ref BI_GetLandData()
{
    ...
    if (CheckAttribute(arLoc, "fort.model"))
    {
        ...
        if (chrIdx >= 0)
        {
            ...
            {
            case RELATION_FRIEND:
                BI_intNRetValue[1] = BI_RELATION_FRIEND;
                break;
            case RELATION_NEUTRAL:
                BI_intNRetValue[1] = BI_RELATION_NEUTRAL;
                break;
            case RELATION_ENEMY:
                BI_intNRetValue[1] = BI_RELATION_ENEMY;
                break;
            }
...
bool CheckSuccesfullBoard(ref rBoarderChr, ref rSieger)
{
    ...
    if (pow(rand(1000) / 1000.0, k) < topLimit)
        return true;
    return false;
}
