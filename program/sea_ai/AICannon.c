...
{
    if (sti(Characters[iCharacterIndex].Ship.Cannons.Type) == CANNON_TYPE_NONECANNON)
    {
        return;
    }
...
float Cannon_GetRechargeTime()
{
    ...
    if (sti(refBaseShip.BaseType) != SHIP_FORT)
    {
        ...
        if (sti(aCharacter.index) != GetMainCharacterIndex())
        {
            ...
            if (fReloadTime < 5.0)
                fReloadTime = 5.0; //Jason - фикс пулеметов 3 калибра
        }
        // boal <--
...
float Cannon_DamageEvent()
{
    ...
    if (IsCharacterEquippedArtefact(aCharacter, "talisman3"))
        return 0.0;
    ...
    if (IsCharacterPerkOn(aCharacter, "HT4"))
        fCurDamage = fCurDamage / 2.0;
