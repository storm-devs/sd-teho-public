...
void Group_DeleteGroup(string sGroupID)
{
    ...
    if (iGroupIndex < 0)
    {
        if (sGroupID != PLAYER_GROUP && MOD_BETTATESTMODE == "On")
        {
            Trace("Group_DeleteGroup sGroupID = " + sGroupID + ", can't find group");
        }
        return;
    }
...
ref Group_GetGroupByIndex(int iGroupIndex)
{
    ...
    {
        if (MOD_BETTATESTMODE == "On")
            Log_Info("Error: Group_GetGroupByIndex нет группы");
...
bool Group_isTaskLockR(ref rGroup)
{
...
// Task: Attack
void Group_SetTaskAttack(string sGroupID, string sAttackedGroup)
{
    Group_SetTaskAttackEx(sGroupID, sAttackedGroup, true);
}
...
void Group_CheckTask()
{
    ...
    {
        if (MOD_BETTATESTMODE == "On")
            Log_Info("Error: Group_CheckTask нет группы");
        return;
    }
