...
void AISeaGoods_ShipDead()
{
    ...
    aref aGoods;
    makearef(aGoods, rCharacter.Ship.Cargo.Goods);
    int iNumGoods;
...
void SetMineFree(ref xi_refCharacter, int type)
{
    ...
    if (!CheckAttribute(pchar, "questTemp.Sharlie.DelTerGuard") && pchar.location == "tortuga" && sti(xi_refCharacter.index) == GetMainCharacterIndex() && !bDisableMapEnter)
