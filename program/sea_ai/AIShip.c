...
void DeleteShipEnvironment()
{
    ...
    for (int i = 0; i < iNumShips; i++)
    {
        ...
        if (CheckAttribute(&Characters[Ships[i]], "Ship.Sounds.WaterID"))
            ReleaseSound(sti(Characters[Ships[i]].Ship.Sounds.WaterID));
        if (CheckAttribute(&Characters[Ships[i]], "Ship.Sounds.SailsID"))
            ReleaseSound(sti(Characters[Ships[i]].Ship.Sounds.SailsID));
        if (CheckAttribute(&Characters[Ships[i]], "Ship.Sounds.TurnID"))
            ReleaseSound(sti(Characters[Ships[i]].Ship.Sounds.TurnID));
    }
...
{                                                                  // используется на суше при атаке города
    ...
    if (CheckAttribute(rCharacter, "Coastal_Captain"))
        return; // есть флаг есть, то нет ругани, это таможенник и квестовый
    ...
    {
        if (CheckAttribute(rCharacter, "ShipEnemyDisable") || CheckAttribute(rCharacter, "AlwaysFriend"))
            return; // флаг не обижаться на выстрелы
...
void Ship_FireAction()
{
    ...
    if (LAi_IsDead(rCharacter))
        return; // fix - нефиг палить в труп!
    ...
    {
        if (CheckAttribute(rCharacter, "ShipEnemyDisable"))
            return; // флаг не обижаться на выстрелы
    ...
    for (i = 0; i < iNumShips; i++)
    {
        ...
        if (SeaAI_GetRelation(GetMainCharacterIndex(), Ships[i]) != RELATION_ENEMY)
        {
            continue;
        }
...
float Ship_MastDamage()
{
    ...
    if (iDamageType == SHIP_MAST_TOUCH_BALL)
    {
        ...
        if (sti(rCharacter.Ship.LastBallCharacter) == GetMainCharacterIndex() && GetNationRelation2MainCharacter(sti(rCharacter.nation)) != RELATION_ENEMY)
        {
...
void Ship_Add2Sea(int iCharacterIndex, bool bFromCoast, string sFantomType, bool isOffline)
{
    ...
    aref arCharacter;
    makearef(arCharacter, Characters[sti(rCharacter.index)]);
    ...
    if (!CheckAttribute(&RealShips[iShipType], "name"))
    {
        ...
        if (MOD_BETTATESTMODE == "On")
            Log_Info("Ship_Add2Sea ERROR : SHIP " + iShipType + ", have no name!   NPCid = " + rCharacter.id);
        return;
    }
    ...
    float range = 550;
    if (CheckAttribute(rCannon, "FireRange"))
        range = stf(rCannon.FireRange);
    ...
    if (CheckAttribute(rCharacter, "Ship.Sink"))
    {
        trace("ShipSink rCharacter.id = " + rCharacter.id);
...
void Ship_CreateStaticSounds(ref rCharacter)
{
    if (!CheckAttribute(rCharacter, "Ship.Pos.x"))
        return; // fix
    if (GetCharacterShipType(rCharacter) == SHIP_NOTUSED)
        return; // может уже не быть
    ...
    {
        Sound_SetVolume(sti(rCharacter.Ship.Sounds.WaterID), 0.0);
    }
    ...
    {
        Sound_SetVolume(sti(rCharacter.Ship.Sounds.SailsID), 0.0);
    }
    ...
    {
        Sound_SetVolume(sti(rCharacter.Ship.Sounds.TurnID), 0.0);
    }
...
void Ship_PlaySound3DComplex(ref rCharacter, string sSoundID, float fVolume, float fX, float fY, float fZ)
{
    ...
    if (iSoundID > 0)
    {
        Sound_SetVolume(iSoundID, fVolume);
    }
}
...
void Ship_OnBortReloaded()
{
    ...
    if (!bSeaLoaded || bAbordageStarted)
    {
        return;
    } // fix
    ...
    if (sti(aCharacter.index) != nMainCharacterIndex)
    {
        return;
    } // fix
    /*{
    ...
    if (sBort == "cannonl")
    {
        aCharacter.Tmp.BortsReloaded.cannonl = true;
    }
    if (sBort == "cannonr")
    {
        aCharacter.Tmp.BortsReloaded.cannonr = true;
    }
}
...
void Ship_BortReloadEvent()
{
    ...
    if (sti(aCharacter.Tmp.BortsReloaded.Event))
    {
        ...
        else
        {
            if (bLeft)
            {
                PlaySound("interface\_GunReadyL.wav");
            }
            if (bRight)
            {
                PlaySound("interface\_GunReadyR.wav");
            }
        }
    }
...
{
    if (LAi_IsDead(rCharacter) || sti(rCharacter.ship.type) == SHIP_NOTUSED)
        return;
...
bool Ship_CheckSurrendered(ref echr)
{
    ...
    if (LAi_IsDead(echr) || sti(echr.ship.type) == SHIP_NOTUSED)
        return false;
    if (sti(echr.index) == nMainCharacterIndex)
        return false;
    if (IsCompanion(echr))
        return false;
    if (GetNationRelation2MainCharacter(sti(echr.nation)) != RELATION_ENEMY)
        return false;
    if (CheckAttribute(echr, "situation"))
        return false;

    if (CheckAttribute(echr, "DontSurrender"))
        return false;

    if (sti(echr.Nation) == PIRATE || CheckAttribute(echr, "DontRansackCaptain"))
        return false; // квестовые и пираты не выбрасывают белый флаг

    ref mchr = GetMainCharacter();

    if (sti(echr.rank) > (40 - MOD_SKILL_ENEMY_RATE))
        bOk = false;
    if (sti(mchr.rank) < (sti(echr.rank) - MOD_SKILL_ENEMY_RATE / 2))
        bOk = false;
    ...
    if (mcrew > ecrew)
    {
        ...
        if (GetSailPercent(echr) < 25.0 || GetHullPercent(echr) < 25.0)
            return true; // выбросил белый флаг
    }
...
void Ship_CheckSituation()
{
    ...
    if (Ship_AutoAbordage(rCharacter, fMinEnemyDistance))
    {
        return;
    }
    ///////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    {
        if (GetCargoGoods(rCharacter, GOOD_BALLS) < iShipCannonsNum)
        {
            bBalls = false;
        }
        if (GetCargoGoods(rCharacter, GOOD_BOMBS) < iShipCannonsNum)
        {
            bBombs = false;
        }
        if (GetCargoGoods(rCharacter, GOOD_KNIPPELS) < iShipCannonsNum)
        {
            bKnippels = false;
        }
        if (GetCargoGoods(rCharacter, GOOD_GRAPES) < iShipCannonsNum)
        {
            bGrapes = false;
        }
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            {
                                if (Ship_GetDistance2D(GetMainCharacter(), rCharacter) > 3000 && CheckAttribute(pchar, "questTemp.Mtraxx.LosadaEscape"))
                ...
                else
                {
                    ...
                    if (sti(rCharacter.nation) != PIRATE || bBettaTestMode) //boal 030804 нужно чтоб при тесте флаг с пиратами мирил
                    {
                        ...
                        if (rCharacter.id == "Saga_vampire")
                        {
                            ...
                            {
                                if (CheckAttribute(pchar, "questTemp.Saga.BarbTemptation.Bomb"))
                                    Saga_HitSeaBomb();
                            }
                        }
                        // --> Джекман, Центурион
                        if (rCharacter.id == "Jackman")
                        {
                            ...
                            if (Ship_GetDistance2D(GetMainCharacter(), rCharacter) < 25)
                            {
                                if (CheckAttribute(pchar, "questTemp.Saga.BarbTemptation.Fugas"))
                                    Saga_HitSeaFugas();
                            }
                        }
                        // --> Моллиган, Устрица
                        ...
                        {
                            if (Ship_GetDistance2D(GetMainCharacter(), rCharacter) < 500 && CheckAttribute(pchar, "questTemp.Saga.Molligan.enemy"))
                                Saga_MolliganAttack(); // атакует
                        }
                        // --> Бейкер, лодка
                        if (rCharacter.id == "Baker_Cap" && CheckAttribute(pchar, "questTemp.Saga.Shadows.Baker"))
                        {
                            if (Ship_GetDistance2D(GetMainCharacter(), rCharacter) < 20)
                        ...
                        {
                            if (Ship_GetDistance2D(GetMainCharacter(), rCharacter) < 500 && !CheckAttribute(rCharacter, "quest.seaattack"))
                                FMQM_TartanaAlarm(); //
                        }
                    }
                    else
...
void Ship_CheckFlagEnemy(ref rCharacter)
{
    ...
    for (i = 0; i < COMPANION_MAX; i++)
    {
        ...
        if (cn >= 0)
        {
            ...
            if (GetCharacterShipClass(chref) < iClass)
                iClass = GetCharacterShipClass(chref);
        }
    }
    ...
    { // смотрим флаг у ГГ и узнаем в нем...
        if (CheckAttribute(pchar, "questTemp.Guardoftruth") && CheckCharacterItem(pchar, "VerifyPaper") && pchar.location == "cuba1")
            return; // грамота отца Винсенто
        if (CheckAttribute(pchar, "questTemp.Guardoftruth.Trinidad") && CheckCharacterItem(pchar, "VerifyPaper") && pchar.location == "Trinidad")
            return; // грамота отца Винсенто

        fSneak = stf(mChar.TmpSkill.Sneak); // 0.01..1.0
        int rep = sti(abs(REPUTATION_NEUTRAL - sti(mChar.reputation.nobility)) * 0.75);
        if (CheckCharacterPerk(mChar, "HT2"))
            fPerkInfl = 2.0;
...
void Ship_BranderDetonate()
{
    ...
    if (CheckAttribute(rCharacter, "Situation") && rCharacter.situation == "Explosion") // ситуация со взрывом
    {
        ...
        if (GetCargoGoods(rCharacter, GOOD_POWDER) < 300)
        {
            fMaxDamage += 400.0;
        }
        ...
        if (iDiffClass > 0)
            fMaxDamage += iDiffClass * 500.0;
        fMaxDamage = fMaxDamage / 20.0 * MOD_SKILL_ENEMY_RATE;
    }
    if (!GetEntity(&arShipObjects[0], "ship"))
    {
        return;
    } // can't be
    while (FindEntityNext(&arShipObjects[iBranderNumShips]))
    {
        iBranderNumShips++;
    }
    ...
    for (int i = 0; i < iBranderNumShips; i++)
    {
        ...
        if (iCharacterIndex == sti(rCharacter.index))
        {
            continue;
        }
...
void Ship_SailDamage()
{
    ...
    if (rand(100) < 15)
    {
        Play3DSound("fly_ball_misc", x, y, z);
    }
...
{ // че-то распук метод "неподецки" - переделал 29.07.06 boal
    ...
    Log_SetStringToLog("fMultiply " + fMultiply + " на выходе");                          // для тестов
    ...
    if (fNewCrewQuantity < f5Percent)
    {
        fNewCrewQuantity = f5Percent;
    }
    if (fNewCrewQuantity < 0.0)
    {
        fNewCrewQuantity = 0.0;
    }
    ...
    if (stf(rOurCharacter.Ship.Crew.Quantity) < 0.0)
    {
        rOurCharacter.Ship.Crew.Quantity = 0.0;
    }
    // boal fix утопленников   <--
}
...
void Ship_ApplyHullHitpoints(ref rOurCharacter, float fHP, int iKillStatus, int iKillerCharacterIndex)
{
    ...
    if (LAi_IsDead(rOurCharacter) || !CheckAttribute(rOurCharacter, "Ship.HP"))
    {
        return;
    } // fix
    ...
    {
        if (sti(Characters[iKillerCharacterIndex].TmpPerks.HullDamageUp))
            fPlus = 0.15;
        if (sti(Characters[iKillerCharacterIndex].TmpPerks.CannonProfessional))
            fPlus = 0.3;

        if (IsCharacterEquippedArtefact(&Characters[iKillerCharacterIndex], "indian_8"))
            fPlus += 0.05; // belamour произведение дает 0 без перков
    }

    if (sti(rOurCharacter.TmpPerks.BasicBattleState))
        fMinus = 0.15;
    if (sti(rOurCharacter.TmpPerks.AdvancedBattleState))
        fMinus = 0.25;
    if (sti(rOurCharacter.TmpPerks.ShipDefenseProfessional))
        fMinus = 0.35;

    if (IsCharacterEquippedArtefact(rOurCharacter, "amulet_8"))
        fMinus += 0.05; // belamour произведение дает 0 без перков

    if (CheckAttribute(rOurCharacter, "SeaBoss"))
        fMinus = fPlus + stf(rOurCharacter.SeaBoss); // Addon 2016-1 Jason Пиратская линейка
    ...
    if (CheckAttribute(RealShips[sti(rOurCharacter.ship.type)], "QuestShip"))
        fPercent = 1.0;
    else
        fPercent = 1.0 + (makefloat(MOD_SKILL_ENEMY_RATE) / 4.0);
...
void ShipDead(int iDeadCharacterIndex, int iKillStatus, int iKillerCharacterIndex)
{
    ...
    {
        if (MOD_BETTATESTMODE == "On")
            Log_Info("Error: ShipDead Нет корабля у iDeadCharacterIndex = " + iDeadCharacterIndex);
        return;
    }
    ...
    if (iKillerCharacterIndex != -1)
    {
        ...
        if (!bDeadCompanion && iKillStatus != KILL_BY_SPY)
        {
            ...
            case KILL_BY_TOUCH:
                ...
                if (sti(rKillerCharacter.nation) == PIRATE)
                    Achievment_SetStat(rKillerCharacter, 50, 1);
                // boal statistic info 17.12.2003 <--
                break;
            case KILL_BY_BALL:
    ...
    if (rand(10) >= 6) // глупое условие... это все классы :)  && sti(rBaseShip.Class) <= 6)
    {
        ...
        case KILL_BY_BALL:
            ...
            /*case KILL_BY_SPY:    
                bDetonate = true; 
            break;  */
            // идет вылет, если второй раз рекурсию Ship_Detonate делать
        }
        if (bDetonate)
        {
    ...
    rDead.Ship.Sink = "";
    aref aSink;
    makearef(aSink, rDead.Ship.Sink);
...
void Ship_HullHitEvent()
{
    ...
    if (bDead)
        return; // фикс, не нужно обсчитывать труп
    ...
    if (sti(rBallCharacter.TmpPerks.CriticalShoot) && rand(19) == 10)
    {
        bSeriousBoom = true;
    } // +5%
    if (sti(rBallCharacter.TmpPerks.CannonProfessional) && rand(9) == 4)
    {
        bSeriousBoom = true;
    } // +10%
    ...
    case GOOD_GRAPES:
        ...
        if (rand(100) < 30)
        {
            Play3DSound("episode_boom", x, y, z);
        }
        break;
    case GOOD_KNIPPELS:
        bSeriousBoom = false;
    ...
    if (sti(rOurCharacter.TmpPerks.ShipDefenseProfessional) && rand(1000) < 700)
    {
        bSeriousBoom = false;
    } // no seriouse boom
...
*/
    ...
    bool bhol = (sti(rOurCharacter.Ship.LastBallCharacter) == GetMainCharacterIndex()) && (sti(rOurCharacter.nation) == HOLLAND) if (CheckCharacterItem(pchar, "HolTradeLicence"))
    {
        if (btrade || bhol)
        {
    ...
    {
        if (!CheckAttribute(pchar, "GenQuest.MarchCap.Battlestart"))
            DoQuestFunctionDelay("MarchCap2_CheckBattle", 0.5);
    }
    if (rOurCharacter.id == "Cap_Vensan") //Jason: сага, бригантина венсана
    {
        if (!CheckAttribute(pchar, "questTemp.Saga.BarbTemptation.Vensan_free"))
            DoQuestFunctionDelay("Saga_BrigantineDetonate", 0.5);
    }
...
void Ship_SetFantomData(ref rFantom)
{
    ...
    if (CheckAttribute(rFantom, "Ship.Masts"))
    {
        DeleteAttribute(rFantom, "Ship.Masts");
    }
    if (CheckAttribute(rFantom, "Ship.Blots"))
    {
        DeleteAttribute(rFantom, "Ship.Blots");
    }
    if (CheckAttribute(rFantom, "Ship.Sails"))
    {
        DeleteAttribute(rFantom, "Ship.Sails");
    }
    if (CheckAttribute(rFantom, "Ship.Hulls"))
    {
        DeleteAttribute(rFantom, "Ship.Hulls");
    }
}
...
void Ship_CheckMainCharacter()
{
    ...
    if (!CheckAttribute(arUpdate, "Ships"))
    {
        return;
    }
    if (!CheckAttribute(arUpdate, "Forts"))
    {
        return;
    }
    ...
    for (i = 0; i < iFortsNum; i++)
    {
        ...
        if (!CheckAttribute(aRLoc, "Radius"))
        {
            continue;
        } // fix
        ...
        case FORT_ABORDAGE:
            ...
            if (iRelation == RELATION_ENEMY)
            {
                ...
                if (fMinFortAbordDistance > fDistance)
                {
                    fMinFortAbordDistance = fDistance;
                } //fix входа в порт
            }
            if (fDistance > fRadius)
            {
            ...
            if (!Character_IsAbordageEnable(rFortCharacter) || bDisableMapEnter)
            {
                continue;
            } // bDisableMapEnter - значит враги рядом, нефиг брать форт под пушками кораблей boal
    ...
    if (bMakeCurrentUpdate)
    {
        ...
        case SHIP_SAIL:
            if (!Whr_IsStorm())
            {
                ...
                if (Whr_IsDay())
                    SetMusic("music_sea_day");
                else
                    SetMusic("music_sea_night");
...
void Ship_OnStrandNext()
{
    ...
    if (!bOnStrand)
    {
        ...
        if (iSoundID != 0)
        {
            StopSound(sti(rCharacter.Ship.Strand.SndID), 3000);
        }
        rCharacter.Ship.Strand.SndID = SOUND_INVALID_ID;
        return;
    }
...
void Ship_Detonate(ref rCharacter, bool bMakeSmallBoom, bool bMakeDead)
{
    ...
    if (bMakeDead)
    {
        ShipDead(sti(rCharacter.Index), KILL_BY_SPY, -1);
    }

    if (bMakeSmallBoom)
    {
        PostEvent(SHIP_DETONATE_SMALL, 600 + rand(1000), "l", sti(rCharacter.index));
    }
}
...
void Ship_UpdateParameters()
{
    ...
    {
    case 0:
        fTRFromSailState = 0.05;
        break;
    case 1:
        fTRFromSailState = 1.0;
        break;
    case 2:
        fTRFromSailState = 0.68;
        break;
    }
    ...
    */
    ...
    if (bStorm && bSeaActive)
    {
        ...
        {
        case 0:
            MakeSailDmg(iCharacterIndex, fDamageMultiply * (fBaseSailHP / 100.0) * 0.15);
            break;
        case 1:
            MakeSailDmg(iCharacterIndex, fDamageMultiply * (fBaseSailHP / 100.0) * 0.55);
            break;
        case 2:
            MakeSailDmg(iCharacterIndex, fDamageMultiply * (fBaseSailHP / 100.0) * 1.1);
            break;
        }
                            ...
                            if (LAi_IsDead(rCharacter) == false && LAi_IsDead(rTargetCharacter) == false)
                            {
                                if (sti(rTargetCharacter.index) == nMainCharacterIndex)
                                {
                                    ...
                                    if (fAbordageDistance < fCharacterDistance && fAng < fCharacterAngle && fRelativeSpeed < fCharacterSpeed)
                                    {
                                        ...
                                        // код К3 пошел лесом, тк иначе случаи захвата очень редки
                                        //if (sti(rTargetCharacter.index) == nMainCharacterIndex)
                                        //{
                                        if (fRatio > 1.2)
                                        {
...
void Ship_UpdateTmpSkills(ref rCharacter)
{
    ...
    if (!CheckAttribute(rCharacter, "TmpSkillRecall") || sti(rCharacter.TmpSkillRecall) <= 0)
    {
        ...
        aref aTmpSkill;
        makearef(aTmpSkill, rCharacter.TmpSkill);
...
{
    aref arImpulse;
    makearef(arImpulse, rCharacter.Ship.Impulse);
...
void DropGoodsToSea()
{
    ...
    {
        sGood = Goods[i].name;
        if (CheckAttribute(&Goods[i], "DontDrop"))
            continue;
        ...
        if (CheckAttribute(rCharacter, "Ship.Cargo.Goods." + sGood))
        {
            ...
            if (CheckAttribute(rCharacter, "Ship.Cargo.Goods." + sGood + ".Lock"))
                continue; // залоченные товары не выкидывать !!!!! <-- ugeen fix

            if (iQuantity < 1 && rand(1) == 1)
                continue;
