...
float Fort_CannonDamage()
{
    ...
    if (iBallCharacterIndex == iFortCharacterIndex)
    {
        return fDamage;
    }
    ...
    if (iBallCharacterIndex == nMainCharacterIndex)
    {
        ...
        if (iRelation != RELATION_ENEMY)
        {
            ...
            {
                fCurPlayerDamage = fCurPlayerDamage + fDamagePiece * 0.8;
            }
            if (iRelation == RELATION_NEUTRAL)
            {
                fCurPlayerDamage = fCurPlayerDamage + fDamagePiece * 0.5;
            }
    ...
    if (rFortCharacter.id == "Tortuga Fort Commander")
        fkoeff = 1.15 - MOD_SKILL_ENEMY_RATE / 100;
...
void Fort_SetAbordageMode(ref rKillerCharacter, ref rFortCharacter)
{
    ...
    if (CheckAttribute(pchar, "questTemp.Mtraxx.Cartahena.Fort") && rFortCharacter.id == "Cartahena Fort Commander")
        Mtraxx_CartahenaPrepareFortBattle();
    // Jason НСО
    if (CheckAttribute(pchar, "questTemp.Patria.SanJoseAttack") && rFortCharacter.id == "PortSpein Fort Commander")
        Patria_SanJoseFortDestroyed();
    if (CheckAttribute(pchar, "questTemp.Patria.CuracaoAttack") && rFortCharacter.id == "Villemstad Fort Commander")
        Patria_CuracaoFortDestroyed();
}
...
void Fort_CheckAttributes(ref rCharacter)
{
    ...
    {
        rCharacter.Ship.Cannons.Type = CANNON_TYPE_CULVERINE_LBS18;
    }
