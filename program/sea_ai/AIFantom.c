...
int Fantom_GenerateEncounterExt(string sGroupName, object oResult, int iEType, int iNumWarShips, int iNumMerchantShips, int iNation)
{
    ...
    {
        if (iNumShips + i >= MAX_SHIPS_ON_SEA)
            return i;
    ...
    {
        if (iNumShips + iNumMerchantShips + i >= MAX_SHIPS_ON_SEA)
            return iNumMerchantShips + i;
...
int Fantom_GetShipTypeExt(int iClassMin, int iClassMax, string sShipType, string sGroupName, string sFantomType, int iEncounterType, int iNation)
{
    ...
    for (i = SHIP_TARTANE; i <= SHIP_LSHIP_ENG; i++) //энкаунтеры только до линкора, квестовые корабли отдельно
    {
        ...
        if (sti(rShip.CanEncounter) != true)
        {
            continue;
        }
...
void Fantom_SetRandomMoney(ref rFantom, string sFantomType)
{
    ...
    if (!CheckAttribute(rFantom, "ship.type"))
        return; // fix
...
// Stone-D : It was all backwards!
{
    ...
    {
    case 1:
        minSkillLevel = iSClass1;
        break;
    case 2:
        minSkillLevel = iSClass2;
        break;
    case 3:
        minSkillLevel = iSClass3;
        break;
    case 4:
        minSkillLevel = iSClass4;
        break;
    case 5:
        minSkillLevel = iSClass5;
        break;
    case 6:
        minSkillLevel = iSClass6;
        break;
    case 7:
        minSkillLevel = iSClass7;
        break;
    }
    minSkillLevel = minSkillLevel * 10; // приведение скила к 1-100

    if (iSkill < minSkillLevel)
        iSkill = minSkillLevel;
    // boal 15.03.04 <--
    if (iSkill < 1)
        iSkill = 1;
    if (iSkill > SKILL_MAX)
...
void Fantom_SetSails(ref rFantom, string sFantomType)
{
    ...
    if (sFantomType == "war")
        rFantom.Ship.Sails.Gerald = true;
}
...
void Fantom_SetBalls(ref rFantom, string sFantomType)
{
    ...
    if (GetCharacterShipClass(rFantom) == 1)
        fKBalls = fKBalls * 2.2;
    if (GetCharacterShipClass(rFantom) == 2)
        fKBalls = fKBalls * 1.8;
    // boal 20.01.2004 -->
...
void Fantom_SetGoods(ref rFantom, string sFantomType)
{
    ...
    if (CheckAttribute(rFantom, "situation"))
        bOk = true;
...
int Fantom_SetCharacterGoods(ref rFantom, int iGoods, int iQuantity, bool isLock)
{
    ...
    if (iQuantity > iFreeQuantity)
    {
        iQuantity = iFreeQuantity;
    }
    ...
    if (isLock)
    {
        rFantom.Ship.Cargo.Goods.(sGood).Lock = true;
    }
    RecalculateCargoLoad(rFantom);
    return iQuantity;
}
...
int GetPossibilityCannonsUpgrade(ref rFantom, bool isUpgrade)
{
    ...
    if (CheckAttribute(shTo, "CannonsQuantityMax"))
        cannonMaxQ = sti(shTo.CannonsQuantityMax);
    else
    {
    ...
    if (isUpgrade)
    {
        if ((cannonMaxQ - cannonQ) == 0)
            return 0; // максимум орудий - апгрейд невозможен
    }
    else
    {
        if ((cannonQ - cannonMinQ) == 0)
            return 0; // минимум орудий - даунгрейд невозможен
    }
    return 1;
}
...
void SetShipToFantom(ref _chr, string _type, bool _setgoods)
{
    ...
    case "trade":
        ...
        {
            if (Nation == SPAIN)
                ShipType = RandFromThreeDight(SHIP_CARACCA, SHIP_NAVIO, SHIP_GALEON_H);
            else
    ...
    case "war":
        ...
        if (Rank >= 11 && Rank <= 20)
        {
            ...
            {
            case ENGLAND:
                ShipType = RandFromThreeDight(SHIP_BRIG, SHIP_GALEON_L, SHIP_CORVETTE);
                break;
            case FRANCE:
                ShipType = RandFromThreeDight(SHIP_SCHOONER_W, SHIP_GALEON_L, SHIP_CORVETTE);
                break;
            case SPAIN:
                ShipType = RandFromThreeDight(SHIP_BRIGANTINE, SHIP_GALEON_L, SHIP_XebekVML);
                break;
            case HOLLAND:
                ShipType = RandFromFiveDight(SHIP_SCHOONER_W, SHIP_GALEON_L, SHIP_CORVETTE, SHIP_POLACRE, SHIP_SCHOONER_W);
                break;
            }
        }
        if (Rank > 20)
        {
            ...
            {
            case ENGLAND:
                ShipType = RandFromThreeDight(SHIP_FRIGATE, SHIP_FRIGATE_H, SHIP_LINESHIP);
                break;
            case FRANCE:
                ShipType = RandFromThreeDight(SHIP_FRIGATE, SHIP_FRIGATE_H, SHIP_LINESHIP);
                break;
            case SPAIN:
                ShipType = RandFromFiveDight(SHIP_POLACRE, SHIP_GALEON_H, SHIP_FRIGATE_H, SHIP_GALEON_H, SHIP_LINESHIP);
                break;
            case HOLLAND:
                ShipType = RandFromThreeDight(SHIP_FRIGATE, SHIP_FRIGATE_H, SHIP_LINESHIP);
                break;
