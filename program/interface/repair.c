        ...
        if (cn > 0)
        {
            ...
            if (CheckAttribute(chr, "repair"))
            {
                ...
                if (tSail > 0 || tHull > 0 || tMast > 0)
                {
                    ...
                    if (makeint(stf(chr.repair.totaltime)) > totalTime)
                        totalTime = makeint(stf(chr.repair.totaltime));
                    if (totalTime < 1)
                        totalTime = 1;
                }
            }
...
int SetSailMaximum(ref rChar)
{
    ...
    if (sti(rChar.repair.mast) == GetShipFallMastsQuantity(rChar))
        iSP = 100;
    return iSP;
}
...
void GetLimits(ref chr)
{
    ...
    if (i + hp >= 100)
        chr.repair.hull_limit = 100;
    else
        chr.repair.hull_limit = i + hp;

    // sail
    i = cur_sAvail / GetSailRepairMathQty(chr, 1);
    if (i + sp >= sti(chr.repair.sailmax))
        chr.repair.sail_limit = sti(chr.repair.sailmax);
    else
        chr.repair.sail_limit = i + sp;
...
void SetButtonsAccess()
{
    ...
    if (GetCompanionQuantity(mc) > 1 && bOk)
        SetSelectable("REPAIR_ALL_SQUADRON", bOk);
}
    ...
    if (_type == "sail")
    {
        ...
        if (add > 0)
        {
            if ((rSail + add) <= sti(chr.repair.sailmax))
            {
                ...
                if ((rSail + sp) > sti(chr.repair.sailmax))
                    rSail = sti(chr.repair.sailmax) - sp;
            }
        }
        else
        {
    ...
    if (_type == "mast")
    {
        if ((rHull + hp) >= 85)
        {
            ...
            if (add > 0)
            {
                ...
                if ((rMast + mp) > GetShipMastsQuantity(chr))
                    rMast = GetShipMastsQuantity(chr) - mp;
            }
            else
            {
                rMast = rMast + add;
...
void RepairOk()
{
    ...
    for (int i = 0; i < COMPANION_MAX; i++)
    {
        ...
        if (cn > 0)
        {
            ...
            if (sti(chr.repair.preq) > 0)
                RemoveCharacterGoods(pchar, GOOD_PLANKS, sti(chr.repair.preq));
            if (sti(chr.repair.sreq) > 0)
                RemoveCharacterGoods(pchar, GOOD_SAILCLOTH, sti(chr.repair.sreq));
        }
    }
