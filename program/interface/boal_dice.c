...
void BetaInfo()
{
    if (bBettaTestMode)
    {
        ...
        if (CheckAttribute(NPchar, "Quest.DiceCheats"))
            GameInterface.strings.Beta_WinLose = GameInterface.strings.Beta_WinLose + " Cheats " + NPchar.Quest.DiceCheats;
    }
}
...
void ClickHeroDice(int d)
{
    if (bLockClick)
        return;
    if (bStartGame < 2 || bStartGame > 3)
        return;
    sGlobalTemp = "d" + d;
    if (DiceState.Hero.(sGlobalTemp).Mix == true)
        return; // mitrokosta фикс множественного переброса
    if (iMoneyP >= iRate)
    {
    ...
    if (dir_i == -1) // комп первый
    {
        for (i = 1; i <= 5; i++)
        {
            ...
            if (DiceState.Desk.(sGlobalTemp).Mix == true)
            {
                ...
                if (bStartGame > 0)
                    DiceState.Desk.(sGlobalTemp).Mix = false;
            }
        }
        SortDiceOnHand("Comp");
...
void RecalcDiceOnHand(string _whom)
{
    ...
    if (ok)
    {
        ...
        for (k = 1; k <= 5; k++)
        {
            iOk = 5;
            sTemp = "d" + k;
            if (sti(DiceState.(_whom).(sTemp)) == sti(DiceState.(_whom).Result.Rate1))
                continue;
    ...
    if (ok)
    {
        ...
        for (k = 1; k <= 5; k++)
        {
            iOk = 5;
            sTemp = "d" + k;
            if (sti(DiceState.(_whom).(sTemp)) == sti(DiceState.(_whom).Result.Rate2))
                continue;
        ...
        if (ok || ok2 || ok3)
        {
            ...
            for (i = 1; i <= 6; i++)
            {
                ...
                if (sti(DiceState.Comp.Result.(sGlobalTemp)) == 1)
                {
                    ...
                    {
                        sGlobalTemp = "d" + j;
                        if (sti(DiceState.Comp.(sGlobalTemp).Mix) == true)
                            continue;
