...
void AddToTable()
{
    ...
    for (i = ITEMS_QUANTITY; i < TOTAL_ITEMS; i++)
    {
        ...
        if (TradeMode == 1)
            continue; // рандомящиеся предметы за дублоны не продаем
    ...
    for (i = 0; i < ITEMS_QUANTITY; i++)
    {
        ...
        {
            if (!CheckAttribute(&Items[i], "PriceDublon"))
                continue;
        }
        else
        {
            if (!bBettaTestMode)
            {
                if (CheckAttribute(&Items[i], "Price") && sti(Items[i].price) == 0)
                    continue;
            }
        }
...
void SetVariable()
{
    ...
    else
    {
        ...
        if (TradeMode == 0)
            sText = XI_ConvertString("TraderMoney") + " " + FindRussianMoneyString(sti(refStoreChar.money));
        else
            sText = XI_ConvertString("TraderMoney") + " " + FindRussianDublonString(sti(refStoreChar.dublon));
...
void ShowGoodsInfo(int iGoodIndex)
{
    ...
    if (TradeMode == 0)
        iStorePrice = GetTradeItemPrice(iGoodIndex, PRICE_TYPE_SELL, refStoreChar);
    else
        iStorePrice = GetTradeItemPriceDublon(iGoodIndex, PRICE_TYPE_SELL, refStoreChar);
    SetFormatedText("QTY_INFO_STORE_PRICE", XI_ConvertString("Price buy") + NewStr() + its(iStorePrice));

    if (TradeMode == 0)
        iCharPrice = GetTradeItemPrice(iGoodIndex, PRICE_TYPE_BUY, refStoreChar);
    else
        iCharPrice = GetTradeItemPriceDublon(iGoodIndex, PRICE_TYPE_BUY, refStoreChar);
...
int GetTradeItemPrice(int itmIdx, int tradeType, ref chr)
{
    ...
    if (itmIdx < 0 || itmIdx > TOTAL_ITEMS)
        return 0;
    ...
    if (tradeType == PRICE_TYPE_BUY) // покупка предметов героем
    {
        ...
        if (CheckAttribute(chr, "Merchant.type")) //Jason
        {
            ...
            {
                if (itmtype == ITEM_TRADE_ARTEFACT || sItemid == "potionrum")
                    skillModify *= 30.0;
                if (sItemid == "jewelry8")
                    skillModify *= 10.0;
            }
        ...
        for (int j = 0; j < GetAttributesNum(rootItems); j++)
        {
            ...
            if (Items_FindItem(GetAttributeName(curItem), &arItem) >= 0)
            {
                if (CheckAttribute(arItem, "TradeType"))
                {
                    ...
                    {
                    case "potion":
                        if (sItemType != ITEM_TRADE_NORMAL && sItemType != ITEM_TRADE_POTION)
                    ...
                    case "armourer":
                        ...
                        break;
                    case "monk":
                        if (sItemType != ITEM_TRADE_ARTEFACT && sItemType != ITEM_TRADE_MAP)
                            DeleteAttribute(chr, "items." + arItem.id);
                        break;
                    case "indian":
                        if (sItemType != ITEM_TRADE_NORMAL && sItemType != ITEM_TRADE_ARTEFACT)
                            DeleteAttribute(chr, "items." + arItem.id);
                        break;
                    case "jeweller":
