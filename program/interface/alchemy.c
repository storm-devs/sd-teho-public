...
void InitInterface(string iniName)
{
    ...
    if (CheckAttribute(nulChr, "alchemy.items"))
        DeleteAttribute(nulChr, "alchemy.items");
...
void FillItemsScroll()
{
    ...
    for (int n = 0; n < ITEMS_QUANTITY; n++)
    {
        ...
        {
            if (itm.multiobject.alchemy == true && !CheckCharacterPerk(refCharacter, "Alchemy"))
                continue;
...
void AddToTable(ref rItem)
{
    ...
    for (i = 0; i < iNum; i++) // кол-во компонентов и прочего
    {
        ...
        if (CheckAttribute(alchemy, "items." + (sItmId)))
            iRightQty = sti(alchemy.items.(sItmId));
        else
            iRightQty = 0;
...
int CheckAlchemy(string sItemID)
{
    ...
    for (int i = 0; i < iNum; i++) // кол-во компонентов и прочего
    {
        ...
        {
            if (GetCharacterFreeItem(alchemy, sItmId) >= itmReq)
                bOk = true;
            else
            {
                bOk = false;
