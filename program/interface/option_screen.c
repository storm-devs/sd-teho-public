...
void GetMouseOptionsData()
{
    ...
    {
        fCurXSens = stf(InterfaceStates.mouse.x_sens);
    }
    if (CheckAttribute(&InterfaceStates, "mouse.y_sens"))
    {
        fCurYSens = stf(InterfaceStates.mouse.y_sens);
    }
    if (fCurXSens < 0.0)
        fCurXSens = 0.0;
    if (fCurXSens > 1.0)
...
void ISetColorCorrection(float fContrast, float fGamma, float fBright, float fSeaDetails)
{
    ...
    if (fCurBright < 0.0)
        fCurBright = 0.0;
    if (fCurSeaDetails < 0.0)
        fCurSeaDetails = 0.0;
    if (fCurSeaDetails > 1.0)
        fCurSeaDetails = 1.0;
...
bool DoMapToOtherKey(int keyIdx, int stickUp)
{
    ...
    if (CheckAttribute(arKey, "stick") && sti(arKey.stick) == true)
        return false;
    ...
    if (CheckAttribute(arKeyRoot, tmpstr + ".stick") && sti(arKeyRoot.(tmpstr).stick) == true)
        return false;
