...
int doDescribe(int gNum)
{
    ...
    if (isShipEncounterType > 1)
    {
        ...
        case 0:
            if (IsDay())
                loadScr = "loading\sea_1.tga";
            else
                loadScr = "loading\sea_2.tga";
            break;
        case 1:
...
void findWarring(string fType)
{
    ...
    for (i = 0; i < nWME, i++)
    {
        ...
        if (!CheckAttribute(rRawGroup, "type") || rRawGroup.type != fType)
            continue;
        if (encID == origEncID)
            continue;

        if (!CheckAttribute(rRawGroup, "encdata"))
            continue;
        ...
        if (tShips > MAX_SHIPS_IN_LOCATION)
            continue;
        x = stf(rRawGroup.x) * dstScale;
        z = stf(rRawGroup.z) * dstScale;
        ay = stf(rRawGroup.ay);
