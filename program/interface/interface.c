...
void LaunchStorage(int storageNum) // интерфейс склада
{
    if (storageNum < 0)
        return;
    if (storageNum > STORE_QUANTITY - 1)
        return;
...
{
    if (!CheckAttribute(&InterfaceStack, "size"))
        InterfaceStack.size = 0;
...
void IDoSoundEvent()
{
    ...
    case 1:
        ...
        break; // любая клавиша
    case 2:
        PlaySound("interface\menu_select_01.wav");
        break; // выбор элемента
    }
}
...
{
    string ControlName = GetEventData();
    if (!CheckAttribute(&InterfaceStates, "VideoBreakControls." + ControlName))
    {
        return;
    }
    DeleteAttribute(&InterfaceStates, "VideoBreakControls");
...
{
    if (procEnableInterfaceLaunch(interfaceCode) == false)
        return false;
    if (g_ibVideoExecuting)
        return false;
...
void ReloadAfterFortCapture()
{
    ...
    if (!CheckAttribute(pchar, "from_interface"))
    {
        if (MOD_BETTATESTMODE == "On")
            Log_Info("Error: ReloadAfterFortCapture miss from_interface.fortCharacterIdx");
        return;
    }
...
void SetSkillShowEx(aref xi_refCharacter, string skillName, int skillVal, int skillTempVal, int skillUpVal, int iX, int iY)
{
    ...
    if (skillVal >= MAX_CHARACTER_SKILL)
        color = COLOR_MONEY;
...
void MakeQuickSave()
{
    ...
    {
        return;
    } // лесник - если диялог ,то быстре сохарнение отменяется. лесник
