...
int _GetDistanceToColony2D(string _sColony)
{
    ...
    float X1, Z1;
    GetCorrectShipCoords(&X1, &Z1) float X2 = makefloat(worldMap.islands.(sColonyIslandID).(sColonyTown).position.x) + 1000;
...
void InitInterface(string iniName)
{
    ...
    for (int i = 0; i < MAX_COLONIES; i++)
    {
        ...
        if (sColony == "KhaelRoa")
            continue;
        if (sColony == "Minentown")
            continue;
        if (sColony == "Ksochitam")
            continue;
        if (sColony == "RockIsland")
            continue;
        if (sColony == "SantaQuiteria")
            continue;
        if (sColony == "IslaDeVieques")
            continue;
        if (sColony == "SanAndres")
            continue;
        if (sColony == "Is")
            continue;
        if (sColony == "IslaMona" && !CheckAttribute(CharacterFromID("Islamona_carpenter"), "Storage.Activate"))
            continue;
    ...
    CreateString(true, "ProvisionsInfo", XI_ConvertString("Food for") + CalculateShipFood(pchar) + "/" + CalculateShipRum(pchar) + XI_ConvertString("days") + " (" + XI_ConvertString("FoodAcc") + "/" + XI_ConvertString("RumAcc") + ")", FONT_CAPTION, COLOR_NORMAL, 630, 10, SCRIPT_ALIGN_CENTER, 0.8);
