...
void CS_TableSelectChange()
{
    ...
    sti(xi_refCharacter.rank_exp) if (CurTable == "TABLE_SKILL_1" || CurTable == "TABLE_SKILL_2")
    {
        string skillName;
        int skillVal;
...
void FillControlsList(int nMode)
{
    ...
    {
    case 1:
        FillPerksTable("Self", true);
        break; // Личные перки
    case 2:
        FillPerksTable("Ship", true);
        break; // Корабельные перки
    }
}
    ...
    if (checkAttribute(GameInterface, "PASSENGERSLIST." + attributeName2 + ".character"))
    {
        ...
        if (!IsEntity(&worldMap) && !bSeaActive && bNeedFollow) // только для офов
        {
            ...
            else // Jason: залупу на воротник, а не обмен с офами, где их нет и быть не может. Супердыра закрыта.
            {
                ...
                if (num < 1 || CheckAttribute(loc, "DisableOfficers"))
                    ChangeCharacterAddressGroup(&Characters[iChar], "none", "", "");
            }
...
void FillPerksTable(string _type, bool _refresh)
{
    ...
    for (i = 0; i < perksQ; i++)
    {
        ...
        if (xi_refCharacter.id == pchar.id && CheckAttribute(arPerksRoot, perkName + ".NPCOnly"))
            continue;
        if (xi_refCharacter.id != pchar.id && CheckAttribute(arPerksRoot, perkName + ".PlayerOnly"))
            continue;
        if (CheckAttribute(arPerksRoot, perkName + ".Hiden"))
            continue;
