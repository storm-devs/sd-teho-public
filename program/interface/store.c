    ...
    for (i = 0; i < GOODS_QUANTITY; i++)
    {
        ...
        else
        {
            ...
            if (refStore.Colony == "none")
            {
                ...
                if (sti(GameInterface.TABLE_LIST.(row).td5.str) < 1)
                    GameInterface.TABLE_LIST.(row).td5.str = 1;
            }
        }
...
void TransactionOK()
{
    ...
    { // SELL
        ...
        if (iCurGoodsIdx == GOOD_SLAVES)
            Achievment_SetStat(pchar, 43, nTradeQuantity);
        // boal  check skill -->
