...
void FillItemsTable(int _mode) // 1 - все 2 - оружие 3 - остальное
{
    ...
    for (i = 0; i < GetAttributesNum(rootItems); i++)
    {
        ...
        if (Items_FindItem(GetAttributeName(curItem), &arItem) >= 0)
        {
            ...
            if (CheckAttribute(arItem, "itemType"))
                itemType = arItem.itemType;
...
*/
            ...
            if (_mode == 1 && groupID == MAPS_ITEM_TYPE)
                continue;
            if (_mode == 2 && !ok)
                continue;
            if (_mode == 3)
            {
                if (!ok1)
                    continue;
                if (GetCharacterFreeItem(xi_refCharacter, arItem.id) < 1 && arItem.id != "MapsAtlas")
                    continue;
            }
            if (_mode == 4 && !ok3)
                continue;
...
void EquipPress()
{
    ...
    if (CheckAttribute(itmRef, "groupID"))
    {
        ...
        else
        {
            ...
            {
                if (itmRef.id == "Ultimate_potion")
                    UltimatePotionEffect();          // зелье команчей
                ...
                // мангароса
                if (itmRef.id == "mangarosapower")
                    MangarosaEffect("power"); // зелье силы
                if (itmRef.id == "mangarosafast")
                    MangarosaEffect("fast"); // зелье реакции
                if (itmRef.id == "mangarosatotal")
                    MangarosaEffect("total"); // комплексное зелье
                // калеуче
                ...
                if (itmRef.id == "MerrimanBook")
                    Caleuche_MerrimanBoxOpen();
                ...
                if (itmRef.id == "specialletter") // спецписьмо
                {
                    ...
                    if (pchar.GenQuest.specialletter.read == "Letter_Vincento")
                        GuardOT_VinsentoLetterRead();
                    if (pchar.GenQuest.specialletter.read == "Letter_Vincento_next")
                        GuardOT_VinsentoNextLetterRead();
                    if (pchar.GenQuest.specialletter.read == "Letter_Vincento_last")
                        Ksochitam_VinsentoLastLetterRead();
                    if (pchar.GenQuest.specialletter.read == "Letter_Dichoso")
                        Tieyasal_DichosoLetterRead();
                }
                if (itmRef.id == "skinmap") // // Jason 120712 карта двух появлений
                {
                    ...
                    {
                        if (pchar.questTemp.Ksochitam == "skinmap_1")
                            Ksochitam_LookSkinmapFirstTime();
                        if (pchar.questTemp.Ksochitam == "skinmap_2")
                            Ksochitam_LookSkinmapSecondTime();
                    }
                    return;
                }
                ...
                if (itmRef.id == "wolfreeks_book")
                    Mtraxx_WolfreekReadLogbook(); // // Addon 2016-1 Jason Пиратская линейка журнал Вульфрика
                SetVariable();
                return;
            }
            else
            {
                bool bCanmakeMushketer = (IsMainCharacter(xi_refCharacter)) || (CheckAttribute(xi_refCharacter, "CanTakeMushket")) if (HasSubStr(itmRef.id, "Mushket") && bCanmakeMushketer)
                {
