...
void WhrCreateLightningEnvironment()
{
    ...
    aref aLightning;
    makearef(aLightning, aCurWeather.Lightning);

    DeleteAttribute(&Lightning, "");
    Lightning.Clear = "";
    DelEventHandler(WHR_LIGHTNING_DOIT, "Lightning_DoIt");

    if (sti(aLightning.Enable) != true)
        return;
...
void Lightning_DoIt()
{
    ...
    // next lightning
    PostEvent(WHR_LIGHTNING_DOIT, 300 + rand(600) + rand(600));

    // if interface launched, return
    if (sti(InterfaceStates.Launched) && CurrentInterface != INTERFACE_MAINMENU)
    {
        return;
    }
