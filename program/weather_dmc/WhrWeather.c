...
void CreateWeatherEnvironment()
{
	...
	if(CheckAttribute(pchar, "location"))
	{
		...
		if(iLoc != -1)
		{
			...
			{
				if(Locations[iLoc].type == "residence" 
					|| Locations[iLoc].type == "tavern" 
					|| Locations[iLoc].type == "house" 
					|| Locations[iLoc].type == "shop" 
					|| Locations[iLoc].type == "shipyard" 
					|| Locations[iLoc].type == "church" )
				{
