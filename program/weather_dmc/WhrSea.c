...
void WhrCreateSeaEnvironment()
{
    ...
    aref aSea;
    makearef(aSea, aCurWeather.Sea);
    aref aSea2;
    makearef(aSea2, aCurWeather.Sea2);
    aref aCommon;
    makearef(aCommon, WhrCommonParams.Sea);
    ...
    if (iLocation != -1)
    {
        ...
        /*if(CheckAttribute(&locations[iLocation], "MaxWaveHeigh"))
        {
            if(stf(locations[iLocation].MaxWaveHeigh) > 0.0)
            {
                fMaxSeaHeight = stf(locations[iLocation].MaxWaveHeigh);
            }
        }*/
    }
    else
    ...
    // harmonics
    aref aHarmonics;
    makearef(aHarmonics, aSea.Harmonics);
