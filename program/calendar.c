...
void SetDayTime(string name)
{
    ...
    case DAY_TIME_NIGHT:
        SetCurrentTime(0, 1);
        break;
    case DAY_TIME_MORNING:
        SetCurrentTime(6, 1);
        break;
    case DAY_TIME_DAY:
        SetCurrentTime(11, 1);
        break;
    case DAY_TIME_EVENING:
        SetCurrentTime(19, 1);
        break;
    }
}
                ...
                int currentYear, int currentMonth, int currentDay, float currentTime)
{
    ...
    if (dday < 0)
    {
        dmonth--;
        if (pastMonth > 1)
            dday = dday + GetMonthDays(pastMonth - 1);
        else
            dday = dday + GetMonthDays(12);
    }
    if (dmonth < 0)
    {
        dyear--;
    ...
    if (timeUnit == "hour")
        return dayCount * 24 + makeint(dtime);
    if (timeUnit == "minute")
        return dayCount * 1440 + makeint(dtime * 60.0);
    return dayCount;
}
