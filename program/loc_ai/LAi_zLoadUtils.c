...
void CreateLostShipsCity(aref loc)
{
    ...
    if (loc.id == "LostShipsCity_town" && isDay())
    {
        ...
        for (i = 0; i < MAX_CHARACTERS; i++)
        {
            ...
            if (sld.location == "GloriaChurch")
                qtyChurch++;
            //if (sld.location == "TartarusPrison") qtyPrison++;
            if (sld.location == "EsmeraldaStoreBig")
                qtyStore++;
        }
        //размещаем нпс
        for (i = 0; i < MAX_CHARACTERS; i++)
...
void CreatePlantation(aref loc)
{
    if (CheckAttribute(loc, "type") && loc.type == "Plantation")
    {
        ...
        if (GetNpcQuestPastDayWOInit(loc, "Plantation_date") < 2)
            return;
        ...
        while (i < num)
        {
            ...
            if (model[iMassive] != "")
            {
                sAnime = "man_B" if (model[iMassive] == "prison_5") sAnime = "man2";
