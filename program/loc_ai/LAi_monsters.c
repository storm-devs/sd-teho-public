...
void LAi_GenerateFantomFromMe(aref chr)
{
    ...
    if (!TestRef(loadedLocation))
        return;
    if (!IsEntity(&loadedLocation))
        return;
    if (LAi_LocationIsFantomsGen(loadedLocation) == false)
        return;
    //Проверяем возможность перерождения персонажа
    if (LAi_CharacterIsReincarnation(chr) == false)
        return;
    //Проверяем какую модельку использовать для перерождения
    bool isGen = LAi_CharacterReincarnationMode(chr);
    if (CheckAttribute(chr, "model") == false)
        isGen = true;
    if (CheckAttribute(chr, "model.animation") == false)
        isGen = true;
    if (chr.model == "")
        isGen = true;
    if (chr.model.animation == "")
        isGen = true;
    string model, ani;
    if (isGen)
    {
        int nat = GetLocationNation(loadedLocation) if (nat < 0) nat = PIRATE;
        if (nat >= MAX_NATIONS)
            nat = PIRATE;
    ...
    else
    {
        ...
        //Инициализируем тип
        if (!CheckAttribute(fnt, "chr_ai.type"))
            fnt.chr_ai.type = "";
...
{ // выращивание травки: раз в 10 дней при заходе в локацию вырастает новый экземпляр - садовник Jason
    ref rItm;
    if (CheckAttribute(location, "fastreload") && location.id.label != "exittown")
        return false;
    if (!CheckAttribute(location, "locators.item") || location.id.label == "Cave entrance" || location.type == "seashore" || location.type == "cave")
        return false;
    ...
    {
        int n = 1;
        if (location.id.label == "exittown" && !CheckAttribute(location, "fastreload"))
            n = 2; // за городскими воротами выращиваем больше
        for (int i = 0; i < n; i++)
        {
...
bool LAi_CreateShoreChest(ref location) // Jason: выброшенные на берег сундуки - 5%
{
    ...
    if (CheckAttribute(location, "Chestgennot"))
        return false;
...
bool LAi_CreateEncounters(ref location)
{
    ...
    if (CheckAttribute(location, "fastreload"))
        return false; //отсекаем локации exitTown у пиратских городов
    ...
    if (CheckAttribute(location, "ItzaLand"))
        return false;
    ...
    case 0:
        ...
        if (CheckAttribute(pchar, "questTemp.Sharlie.Lock"))
            return false;
        if (CheckAttribute(location, "onUninhabitedIsland") || location.type == "seashore" || location.type == "mayak" || location.type == "questisland")
            return false; // На необитаемых нельзя 300912
        if (location.id == "Curacao_jungle_03" || location.id == "Hispaniola_Jungle_02")
            return false;
        num = GetAttributesNum(grp) - rand(2); //кол-во человек в банде
        if (num <= 0)
            num = 1; //если локаторов меньше четырех
        str = "Gang" + location.index + "_";
        //--> генерим ранг
        if (sti(pchar.rank) > 6)
        {
            if (sti(pchar.rank) > 20)
                iRank = sti(pchar.rank) + sti(MOD_SKILL_ENEMY_RATE * 2.5 / num);
            else
                iRank = sti(pchar.rank) + sti(MOD_SKILL_ENEMY_RATE * 1.6 / num);
        }
        else
        ...
        while (i < num)
        {
            ...
            if (model[iMassive] != "")
            {
                ...
                if (sEncType == "war")
                {
                    ...
                    if (drand(9) < 8)
                        LAi_SetCheckMinHP(chr, LAi_GetCharacterHP(chr) - 1, true, "LandEnc_CaribBeforeDialog");
                    else
                        LAi_SetCheckMinHP(chr, LAi_GetCharacterHP(chr) - 1, true, "LandEnc_MiskitoBeforeDialog");
                }
                else
                {
    ...
    case 2:
        if (rand(12) > 6)
            return false;
        if (CheckAttribute(location, "onUninhabitedIsland") || location.type == "seashore" || location.type == "mayak")
            return false;            // На необитаемых  островах, маяках и бухтах нельзя
        ...
        if (CheckAttribute(pchar, "questTemp.Sharlie.Lock"))
            return false;
        if (CheckAttribute(pchar, "questTemp.Sharlie.DefendSP"))
            return false;
    ...
    case 4:
        ...
        if (CheckAttribute(pchar, "questTemp.Sharlie.DefendSP"))
            return false;
        if (CheckAttribute(location, "onUninhabitedIsland"))
            return false; // На необитаемых нельзя
    ...
    // --------------------------------- Беглые каторжники -------------------------------------
    case 5:
        if (rand(15) > 3 || CheckAttribute(pchar, "GenQuest.Convict") || location.type == "seashore" || location.type == "mayak")
            return false;
        if (CheckAttribute(location, "onUninhabitedIsland"))
            return false; // На необитаемых нельзя
        if (sAreal == "Panama")
        ...
        iRnd = rand(9);
        if (iRnd <= 5)
            pchar.GenQuest.Convict.variant = 1;
        if (iRnd > 5 && iRnd < 8)
            pchar.GenQuest.Convict.variant = 2;
        if (iRnd >= 8)
            pchar.GenQuest.Convict.variant = 3;
        pchar.GenQuest.Convict.var = rand(2);
        Log_TestInfo("Каторжане: сгенерился квест");
        break

            // Warship, 15.05.11. Генер "Правосудие на продажу".
            case 6 : if (rand(10) > 6 && !bBettaTestMode) return false;
...
bool LAi_CreateCaveEncounters(ref location) // Jason 061012 пещерные энкаунтеры
{
    if (CheckAttribute(location, "type") && location.type == "cave")
    {
        ...
        if (CheckAttribute(location, "DisableEncounters") && location.DisableEncounters == true)
            return false; // mitrokosta фикс неправильного включения энкаунтеров
        if (CheckAttribute(location, "enc") && GetNpcQuestPastDayParam(location, "enc") < 1)
            return false;
        if (findsubstr(location.id, "Ksochitam_", 0) != -1 || findsubstr(location.id, "mine_", 0) != -1)
            return false;
        if (CheckAttribute(pchar, "questTemp.Sharlie.Lock"))
            return false;
        if (location.id == "Bermudes_Cavern" || location.id == "FortFrance_Dungeon")
            return false; // patch
        if (drand(12) > 5)
            return false;
            ...
            while (i < num)
            {
                ...
                if (model[iMassive] != "")
                {
                    ...
                    if (drand(14) == 1) // кладокопатели
                    {
                        ...
                        if (drand(5) == 1)
                            TakeNItems(chr, "chest", 1);
                        if (drand(10) == 2)
                            TakeNItems(chr, "icollection", 1);
                        if (drand(7) == 0)
                            TakeNItems(chr, "jewelry1", drand(50));
                        if (drand(7) == 1)
                            TakeNItems(chr, "jewelry2", drand(30));
                        if (drand(7) == 2)
                            TakeNItems(chr, "jewelry3", drand(40));
                        if (drand(7) == 3)
                            TakeNItems(chr, "jewelry4", drand(30));
                        if (drand(7) == 4)
                            TakeNItems(chr, "jewelry5", drand(100));
                        if (drand(7) == 5)
                            TakeNItems(chr, "jewelry6", drand(200));
                        if (drand(7) == 6)
                            TakeNItems(chr, "jewelry52", drand(200));
                        if (drand(7) == 7)
                            TakeNItems(chr, "jewelry53", drand(500));
                    }
                    i++;
                    model[iMassive] = "";
                }
                ...
                if (model[iMassive] != "")
                {
                    ...
                    if (drand(10) == 0) // богатенькие
                    {
                        ...
                        if (drand(3) == 0)
                            TakeNItems(chr, "jewelry5", drand(100));
                        if (drand(3) == 1)
                            TakeNItems(chr, "jewelry6", drand(150));
                        if (drand(3) == 2)
                            TakeNItems(chr, "jewelry52", drand(300));
                        if (drand(3) == 3)
                            TakeNItems(chr, "jewelry53", drand(700));
                    }
                    i++;
                    model[iMassive] = "";
                }
        ...
        case 3: // скелеты с лутом
            if (CheckAttribute(location, "id.label") && location.id.label == "Grot")
                return false;
        ...
        case 4: // заполним сундук
            ...
            {
            case 0:
                sItm = "indian_" + (drand(10) + 1);
                break;
            case 1:
                sItm = "amulet_" + (drand(10) + 1);
                break;
            case 2:
                sItm = "obereg_" + (drand(10) + 1);
                break;
            }
            pchar.GenQuestBox.(str) = true;
            if (drand(10) == 0)
                pchar.GenQuestBox.(str).box1.items.gold_dublon = drand(100);
            if (drand(10) == 1)
                pchar.GenQuestBox.(str).box1.items.chest = drand(1);
            if (drand(10) == 2)
                pchar.GenQuestBox.(str).box1.items.jewelry7 = drand(3);
            if (drand(10) == 3)
                pchar.GenQuestBox.(str).box1.items.jewelry5 = drand(100);
            if (drand(10) == 4)
                pchar.GenQuestBox.(str).box1.items.jewelry6 = drand(200);
            if (drand(10) == 5)
                pchar.GenQuestBox.(str).box1.items.jewelry6 = drand(200);
            if (drand(10) > 5)
                pchar.GenQuestBox.(str).box1.items.(sMin) = 1;
            if (drand(7) == 0)
                pchar.GenQuestBox.(str).box1.items.(sItm) = 1;
            if (drand(7) == 1)
                pchar.GenQuestBox.(str).box1.items.(sJew) = drand(20);
            if (drand(7) == 2)
                pchar.GenQuestBox.(str).box1.items.(sGem) = drand(30);
            SaveCurrentNpcQuestDateParam(location, "enc");
            break;
        }
        return true;
