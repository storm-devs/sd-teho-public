...
bool LAi_tmpl_afraid_InitTemplate(aref chr)
{
    ...
    else
    {
        ...
        if (chr.chr_ai.tmpl.state == "falure")
            chr.chr_ai.tmpl.state = "wait";
        if (!CheckAttribute(chr, "chr_ai.tmpl.canmove"))
            chr.chr_ai.tmpl.canmove = "1";
...
void LAi_tmpl_afraid_updatetemplate(aref chr)
{
    if (sti(chr.chr_ai.tmpl.canmove))
    {
        ...
        {
        case "goto":
            if (SetCharacterTask_GotoPoint(chr, "goto", chr.chr_ai.tmpl.locator))
                isWait = false;
            CharacterPlayAction(chr, "");
            break;
        case "runto":
            if (SetCharacterTask_RuntoPoint(chr, "goto", chr.chr_ai.tmpl.locator))
                isWait = false;
            CharacterPlayAction(chr, "");
            break;
        case "escape":
            if (chr.chr_ai.tmpl.who != "")
            {
                if (SetCharacterTask_Escape(chr, &Characters[sti(chr.chr_ai.tmpl.who)]))
                    isWait = false;
            }
            CharacterPlayAction(chr, "");
            break;
...
float LAi_tmpl_afraid_DistByChr(aref chr)
{
    ...
    float dist = 0.0;
    if (!GetCharacterDistByChr3D(chr, &Characters[idx], &dist))
        dist = -1.0;
    return dist;
}
