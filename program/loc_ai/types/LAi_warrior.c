                ...
                for (int i = 0; i < num; i++)
                {
                    if (nMainCharacterIndex == sti(chrFindNearCharacters[i].index))
                    {
                        ...
                        if (bMainCharacterInBox)
                        {
                            ...
                            if (chr.chr_ai.group == "LSC_NARVAL")
                                DoQuestCheckDelay("LSC_NarvalConflict", 0.5);
                            if (chr.chr_ai.group == "LSC_RIVADOS")
                                DoQuestCheckDelay("LSC_RivadosConflict", 0.5);
                            if (chr.chr_ai.group == "LSC_SHARK")
                                DoQuestCheckDelay("LSC_SharkConflict", 0.5);
                            if (rand(100) > 95)
                            {
                                LAi_type_warrior_PlaySound(chr);
                            }
