...
void LAi_type_patrol_Init(aref chr)
{
    ...
    {
        if (!CheckAttribute(chr, "chr_ai.type.locator"))
            chr.chr_ai.type.locator = "";
        if (!CheckAttribute(chr, "chr_ai.type.state"))
            chr.chr_ai.type.state = "stay";
        if (!CheckAttribute(chr, "chr_ai.type.time"))
            chr.chr_ai.type.time = rand(5);
        if (!CheckAttribute(chr, "chr_ai.type.player"))
            chr.chr_ai.type.player = "0";
    }
    ...
    if (chr.chr_ai.tmpl != LAI_TMPL_FIGHT && !LAi_grp_alarmactive)
    {
        ...
        else
        {
            ...
            if (!LAi_tmpl_SetFight(chr, &Characters[trg]))
            {
                ...
                chr.chr_ai.type.checkTarget = rand(4) + 3; //таймер на проверялку расстояния до таргета
        }
    }
    else
    {
...
void LAi_type_patrol_Attacked(aref chr, aref by)
{
    ...
    // boal <--
    if (rand(100) > 95 && !LAi_IsDead(chr) && !LAi_IsDead(pchar))
...
void LAi_type_patrol_Goto(aref chr)
{
    //Идём в новую точку
    string newloc;
    if (!CheckAttribute(chr, "CityType"))
        Log_SetStringToLog("Ghf: " + chr.id);
...
void LAi_type_patrol_TestControl(aref chr)
{
    ...
    if (GetNationRelation2MainCharacter(sti(chr.nation)) == RELATION_ENEMY || GetNationRelation(sti(chr.nation), GetBaseHeroNation()) == RELATION_ENEMY)
        iRand = 2;
    else
        iRand = GetRelation2BaseNation(sti(chr.nation)); // 0- друг 1- нейтрал 2- враг
    if (isDay())
    {
