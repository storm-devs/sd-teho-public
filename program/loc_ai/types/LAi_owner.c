                ...
                for (i = 0; i < num; i++)
                {
                    if (nMainCharacterIndex == sti(chrFindNearCharacters[i].index))
                    {
                        ...
                        if (bMainCharacterInBox && chr.chr_ai.type.state != "afraid")
                        {
                            ...
                            //<-- снимаем если есть флаг на таймер ссоры у хозяина - снимаем
                            if (chr.sex == "man")
                                chr.greeting = "VOICE\Russian\soldier_arest_1.wav";
                            else
                                chr.greeting = "VOICE\Russian\Gr_Woman_Citizen_11.wav";
...
bool LAi_type_owner_CanDialog(aref chr, aref by)
{
    ...
    if (chr.chr_ai.type.state == "afraid")
        return false;
    if (chr.chr_ai.tmpl == LAI_TMPL_STAY || chr.chr_ai.tmpl == LAI_TMPL_GOTO || chr.chr_ai.tmpl == LAI_TMPL_ANI)
        return true;
    return false;
}
...
void LAi_type_owner_SetAfterGoto(aref chr)
{
    if (rand(100) < 80 && chr.chr_ai.type.locator != "stay")
    {
        ...
        if (chr.sex == "man")
            LAi_tmpl_ani_PlayAnimation(chr, "Barman_idle", 1.0 + rand(10));
        else
            LAi_tmpl_ani_PlayAnimation(chr, "taverngirl", 1.0 + rand(12));
    }
    else
    {
        ...
        chr.chr_ai.type.state = "stay";
        if (chr.chr_ai.type.locator == "stay")
            chr.chr_ai.type.wait = stf(5.0 + frnd() * 20.0);
        else
            chr.chr_ai.type.wait = stf(1.0 + frnd() * 4.0);
    }
}
