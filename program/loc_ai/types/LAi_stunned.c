...
{
    string snd = "";
    if (CheckAttribute(chr, "nodisarm"))
        return;
    if (IsMainCharacter(chr))
        return;
    if (LAi_IsBoardingProcess() && CheckAttribute(chr, "chr_ai.group") && chr.chr_ai.group != LAI_GROUP_PLAYER)
        return;
    ...
    if (!CheckAttribute(chr, "stuntime"))
    {
        ...
        if (!CheckAttribute(chr, "first_oldtype"))
            chr.first_oldtype = chr.chr_ai.type;
    ...
    if (resumeoldtype)
    {
        ...
        DeleteAttribute(chr, "stuntime.old_type_resume");
...
bool LAi_Stunned_AwakenCharacter(aref chr)
{
    ...
    {
        if (!CheckAttribute(chr, "first_oldtype"))
            chr.first_oldtype = chr.stuntime.old_type;
    ...
    if (!Lai_IsBoardingProcess())
        PostEvent("LAi_StandsUp", 3750, "i", chr);
    else
        PostEvent("LAi_StandsUp", 3970, "i", chr);
    return true;
}
...
void LAi_StunnedStandsUp()
{
    ...
    {
        string func;
        if (IsOfficer(chr) || bAbordageStarted)
            func = "LAi_Set" + chr.stuntime.old_type + "Type";
        else
            func = "LAi_Set" + chr.stuntime.old_type + "TypeNoGroup";
        call func(chr);
    ...
    if (CheckAttribute(chr, "stuntime.group"))
        LAi_group_MoveCharacter(chr, chr.stuntime.group);
    if (CheckAttribute(chr, "stuntime"))
    {
...
*/
    ...
    else
    {
        ...
        {
        case 0:
            chr.actions.dead.d1 = "death_afraid_1";
            break;
        case 1:
            chr.actions.dead.d1 = "death_afraid_2";
            break;
        }
    }
...
void LAi_type_stunned_CharacterUpdate(aref chr, float dltTime)
{
    if (CheckAttribute(chr, "stuntime"))
    {
        ...
        {
            LAi_Stunned_AwakenCharacter(&chr);
            return;
        }
    }
