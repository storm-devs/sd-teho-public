...
void LAi_CheckKillCharacter(aref chr)
{
    ...
    if (stf(chr.chr_ai.hp) < 1.0)
    {
        ...
        if (CheckAttribute(chr, "HalfImmortal"))
        {
            ...
            if (CheckAttribute(chr, "CompanionDisable"))
            { // mitrokosta общий фикс полубессмертных
                    ...
                    if (rand(MOD_SKILL_ENEMY_RATE + 10) == 2)
                    {
                        ...
                        if (CheckAttribute(rItem, "quality") && rItem.quality != "excellent" && rItem.quality != B_UNIQUE) // ugeen --> на обычных трупах топовое оружие не даем !!!
                        {
                            ...
                            { // залупу на воротник, а не кучи мушкетов
                                if (rand(MOD_SKILL_ENEMY_RATE * 5 + 50) == 50)
...
int Dead_FindCloseBody()
{
    ...
    for (int i = 0; i < Dead_Char_num; i++)
    {
        ...
        rd = sqrt(sqr(x1 - x) + sqr(z1 - z)) if (rd < 1.8)
        {
            if (min_rd > rd)
            {
                min_rd = rd;
...
void MakePoisonAttack(aref attack, aref enemy)
{
    if (!CheckAttribute(enemy, "chr_ai.poison"))
    {
        if (enemy.index == GetMainCharacterIndex())
            ...
            Log_SetStringToLog(GetFullName(enemy) + XI_ConvertString("is poisoned"));
    }
    //Отравляем персонажа
    float poison = 0.0;
    if (CheckAttribute(enemy, "chr_ai.poison"))
...
{
    if (IsCharacterEquippedArtefact(attacked, "kaleuche_amulet3"))
        return; // калеуче
    if (enemy.sex == "skeleton" || enemy.sex == "crab" || IsCharacterEquippedArtefact(enemy, "indian_10") || CheckAttribute(enemy, "viper"))
    {
        if (rand(1000) < 150)
            MakePoisonAttack(enemy, attacked);
        else
        {
            if (rand(2) < 2 && CheckAttribute(enemy, "viper"))
                MakePoisonAttack(enemy, attacked);
        }
    }
}
        ...
        if (isBig == true)
        {
            if (DamageType == "fire")
            {
                if (rand(1) == 0)
                    ...
                    CreateParticleSystem("blood_massshoot", x, y, z, 0, 1.0, 0, 0);
            }
            else
                CreateParticleSystem("blood_big", x, y, z, 0, 1.0, 0, 0);
        }
...
*/
                if (rand(1) == 0)
                    CreateParticleSystem("blood_fire1", x, y, z, 0, 1.0, 0, 0);
                else
                {
                    if (rand(1) == 0)
                        CreateParticleSystem("blood_fire2", x, y, z, 0, 1.0, 0, 0);
                    else
                        CreateParticleSystem("blood_fire3", x, y, z, 0, 1.0, 0, 0);
                }
            }
            else
...
void LaunchIncasFire(aref chr, bool isBig) // 280712
{
    ...
    y = y + 0.1;
    if (isBig)
        CreateParticleSystem("fire_incas_Simple", x, y, z, 0, 1.0, 0, 0);
    else
        CreateParticleSystem("incas_blood", x, y, z, 0, 1.0, 0, 0);
}
