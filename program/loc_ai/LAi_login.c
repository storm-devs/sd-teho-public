...
bool LAi_CharacterLogin(aref chr, string locID)
{
    ...
    if (!actLoadFlag)
    {
        if (hp < 1)
        {
            ...
            {
                if (sti(chr.location.norebirth) != 0)
                    return false;
            }
    ...
    if (hp < 1)
        return false;
    //Проверяем персонажа
    if (LAi_CheckCharacter(chr, "LAi_CharacterLogin") == false)
        return false;
...
bool LAi_CharacterLogoff(aref chr)
{
    ...
    //chr.location = "none";
    if (LAi_CheckCharacter(chr, "LAi_CharacterLogoff") == false)
        return false;
