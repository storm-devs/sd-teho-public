...
void LAi_StartBoarding(int locType, ref echr, bool isMCAttack)
{
    ...
    if (echr.id == "WorldMarchCap1" || echr.id == "WorldMarchCap2" || echr.id == "WorldMarchCap3")
    {
        if (!CheckAttribute(pchar, "GenQuest.MarchCap.Battlestart"))
            DoQuestFunctionDelay("MarchCap2_CheckBattle", 0.5); //Jason
    }
    ...
    if (CheckOfficersPerk(mchr, "MusketsShoot") && IsFort == false) // для ГГ
    {
        ...
        if (IsCharacterEquippedArtefact(mchr, "indian_5"))
            ecrew = makeint(ecrew / 1.1);
    ...
    {
        bOk = true;
        if (IsCharacterEquippedArtefact(ecrew, "indian_5"))
            mcrew = makeint(mcrew / 1.1);
    ...
    if (bOk)
    {
        ...
        else
        {
            if (rand(2) > 1)
                ...
                PlaySound("INTERFACE\MusketFire2.wav");
        }
    }
    ...
    if (mcrew > ecrew)
    {
        ...
        if (boarding_player_hp > 1000)
            boarding_player_hp = 1000;
    ...
    else
    {
        ...
        if (boarding_enemy_hp > 1500)
            boarding_enemy_hp = 1500;
        // boal 30.01.2004 <--
        if (ecrew > maxcrew)
        {
...
void LAi_ReloadEndFade()
{
    ...
    if (CheckAttribute(&Locations[boarding_location], "UpDeckType"))
    {
        ...
        boarding_enemy_crew = inside_ecrew_2 * 2; // увеличиваем внутри число тушек вдвое

    if (IsFort)
    {
        if (CheckAttribute(&Locations[boarding_location], "insidenext"))
            boarding_enemy_crew = inside_ecrew_2 * 2;
        else
            boarding_enemy_crew = inside_ecrew_1 * 2;
    }
    //if(boarding_enemy_crew <= 0) canReload = false;
    if (!CheckAttribute(&Locations[boarding_location], "boarding.nextdeck"))
        canReload = false;
    if (Locations[boarding_location].boarding.nextdeck == "")
        canReload = false;
    ...
    else
    {
        ...
        if (CheckAttribute(pchar, "questTemp.ShipCapellan.Yes"))
            deadCrew = makeint(deadCrew / 2);                            //Jason, капеллан на борту - потери в 2 раза меньше
...
void LAi_EnableReload()
{
    ...
    if (CheckAttribute(&Locations[boarding_location], "UpDeckType"))
    {
        ...
        {
        case 1:
            Locations[boarding_location].boarding.nextdeck = "Boarding_GunDeck";
            break;
        case 2:
            Locations[boarding_location].boarding.nextdeck = "Boarding_GunDeck";
            break;
        case 3:
            Locations[boarding_location].boarding.nextdeck = "Boarding_Campus";
            break;
        case 4:
            Locations[boarding_location].boarding.nextdeck = "Boarding_Campus";
            break;
        case 5:
            Locations[boarding_location].boarding.nextdeck = "Boarding_Cargohold";
            break;
        case 6:
...
void LAi_ActivateReload()
{
    ...
    if (controlName != "ChrAction")
        return;
    if (CheckAttribute(pchar, "GenQuest.CannotReloadBoarding"))
        return; // Jason
...
void LAi_SetBoardingActors(string locID)
{
    ...
    if (CheckAttribute(chr, "boarding.locatorNum"))
        limit = sti(chr.boarding.locatorNum);
        ...
        if (CheckOfficersPerk(mchr, "MusketsShoot") && CheckAttribute(&Locations[locIndex], "UpDeckType") && !CheckAttribute(boarding_enemy, "GenQuest.CrewSkelMode"))
        {
            ...
            for (i = 1; i <= iQty; i++)
            {
                if (LAi_CheckLocatorFree("rld", sLocType + "mush" + i))
                {
                    ...
                    if (i == iQty && mclass <= 3)
                        chr.MushketType = "mushket3"; // 280313
                    chr.AboardFantom = true;
                    if (!IsFort)
    ...
    for (i = 0; i < limit; i++) // <= тк loc0 .. loc4 = 5
    {
        ...
        if (i == 0 && CheckAttribute(&Locations[locIndex], "CabinType"))
        {
            ...
            {
                if (bSeaCanGenerateShipSituation)
                    SetQuestAboardCabinDialogSituation(chr);
                else
                    DeleteAttribute(chr, "Situation");
            }
            else
            {
...
string LAi_GetBoardingModel(ref rCharacter, ref ani)
{
    ...
    if (CheckAttribute(rCharacter, "Ship.Mode") && rCharacter.Ship.Mode == "Pirate" && !IsMainCharacter(rCharacter))
        iNation = PIRATE;
...
string LAi_GetBoardingMushketerModel(ref rCharacter)
{
    ...
    if (sti(rCharacter.index) == GetMainCharacterIndex()) // для ГГ
    {
        ...
        if (isMainCharacterPatented() && sti(Items[sti(rCharacter.EquipedPatentId)].TitulCur) > 1) //если есть патент, оставлю, хотя рудимент, форма только со звания капитан
        {
            ...
            model = "mush_ctz_" + (rand(2) + 4); // три модели
    }
    else // для НПС
    {
        if (CheckAttribute(rCharacter, "Ship.Mode")) // если прописан тип команды
        {
            ...
            case "hunter":
                ...
                break; // ОЗГ
            case "mercenary":
                model = "mush_ctz_" + (rand(2) + 10);
                break; // наемники на квестовиках
            }
        }
