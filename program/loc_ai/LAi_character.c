...
void LAi_AlcoholSetDrunk(aref chr, float alcoholDegree, float time)
{
    ...
    if (CheckAttribute(chr, "chr_ai.drunk"))
        return;
    //float energyMax;
...
bool LAi_IsCharacterControl(aref chr)
{
    ...
    {
        if (chr.chr_ai.tmpl == LAI_TMPL_PLAYER)
            return true;
    }
    return false;
}
...
bool LAi_CharacterReincarnationMode(aref chr)
{
    ...
    if (sti(chr.chr_ai.reincarnation) != 0)
        return true;
    return false;
}
...
string LAi_CharacterReincarnationGroup(aref chr)
{
    ...
    if (chr.chr_ai.reincarnation.group != "")
        return chr.chr_ai.reincarnation.group;
    return "goto";
}
...
bool LAi_SetReincarnationRankStep(aref _chr, int _step)
{
    if (CheckAttribute(_chr, "chr_ai.reincarnation"))
    {
        ...
        Trace("LAi_SetReincarnationRankStep -> no field chr.chr_ai.reincarnation");
    return false;
}
...
string LAi_SetCharacterDefaultBulletType(ref rChar)
{
    ...
    if (sGun != "")
    {
        ...
        if (!isBulletSet)
            trace("can't set default bullet for character id " + rChar.id);
    }
    return sBulletType;
}
    ...
    if (sGun != "")
    {
        ...
        for (int i = 0; i < iNum; i++)
        {
            ...
            if (sBulletType == sBullet)
            {
                ...
                if (CheckAttribute(rItm, "chargeQ"))
                    LAi_GunSetChargeQuant(rChar, sti(rItm.chargeQ));
                else
                    LAi_GunSetChargeQuant(rChar, 0);
                if (CheckAttribute(rItm, "chargespeed") && stf(rItm.chargespeed) > 0.0)
                    LAi_GunSetChargeSpeed(rChar, 1.0 / stf(rItm.chargespeed));
                else
                    LAi_GunSetChargeSpeed(rChar, 0.0);
                if (CheckAttribute(rItm, "dmg_min"))
                    LAi_GunSetDamageMin(rChar, stf(rItm.dmg_min));
                else
                    LAi_GunSetDamageMin(rChar, 0.0);
                if (CheckAttribute(rItm, "dmg_max"))
                    LAi_GunSetDamageMax(rChar, stf(rItm.dmg_max));
                else
                    LAi_GunSetDamageMax(rChar, 0.0);
                if (CheckAttribute(rItm, "accuracy"))
                    LAi_GunSetAccuracy(rChar, stf(rItm.accuracy) * 0.01);
                else
                    LAi_GunSetAccuracy(rChar, 0.0);
...
{
    float ay = 0.0;
    if (GetCharacterAy(chr, &ay) == false)
        ay = 0.0;
