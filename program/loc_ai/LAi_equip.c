...
void LAi_NPC_Equip(ref chr, int rank, bool isWeapons, bool isGun)
{
    ...
    if (isGun)
    {
        if (CheckAttribute(chr, "PhantomType") && chr.PhantomType == "native")
            return; //Jason
        if (CheckAttribute(chr, "PhantomType") && chr.PhantomType == "slave")
            return; //Jason
...
void LAi_NPC_MushketerEquip(ref chr)
{
    ...
    else
    {
        if (CheckAttribute(chr, "model"))
        {
            ...
            if (HasSubStr(chr.model, "mush_rvd_"))
                sMush = "mushket1";
        }
    }
...
string LAi_NPC_EquipBladeSelection(ref chr)
{
    ...
    if (CheckAttribute(chr, "PhantomType"))
    {
        ...
        case "slave":
            ...
            if (blade == "")
                SelectGeneratedItem(TGT_MARGINAL, B_ORDINARY, "");
            break;
        case "native":
            blade = SelectGeneratedItem(TGT_YANKEE, "", "");
            if (blade == "")
