...
float LAi_CalcUseEnergyForBlade(aref character, string actionType)
{
    ...
    case "hit_parry": // boal fix эту энергию тратит не атакующий, а атакуемый в анимации fgt_hit_parry
        energy = 20.0;
        if (CheckAttribute(character, "animal"))
            energy = 0.0;
        break;
    case "feintc":
...
float LAi_GunCalcProbability(aref attack, aref enemy, float kDist)
{
    ...
    if (!IsDay() && IsEquipCharacterByArtefact(attack, "totem_12"))
        p = p * 2;

    if (IsEquipCharacterByArtefact(attack, "indian_2"))
        p = p * 1.15;
    if (IsEquipCharacterByArtefact(enemy, "indian_1"))
        p = p * 1.10;
    if (IsEquipCharacterByArtefact(attack, "amulet_1"))
        p = p * 0.90;
...
float LAi_GunCalcDamage(aref attack, aref enemy)
{
    ...
    if (sBullet == "powder_pellet")
        LaunchBlastPellet(enemy);
    if (sBullet == "grenade")
        LaunchBlastGrenade(enemy);
    ...
    if (CheckAttribute(enemy, "cirassId"))
    {
        ...
        {
            if (sBullet != "powder_pellet")
                Lai_CharacterChangeEnergy(enemy, -stf(attack.chr_ai.EnergyP_C));
            else
            {
    ...
    else
    {
        ...
        {
            if (sBullet != "powder_pellet")
                Lai_CharacterChangeEnergy(enemy, -stf(attack.chr_ai.EnergyP_NC));
            else
            {
                if (enemy.chr_ai.group != LAI_GROUP_PLAYER)
                    Lai_CharacterChangeEnergy(enemy, -stf(attack.chr_ai.EnergyP_C));
    ...
    if (IsEquipCharacterByArtefact(attack, "indian_1"))
        dmg += 15;
    if (IsEquipCharacterByArtefact(enemy, "indian_2"))
        dmg *= 1.1;
    if (IsEquipCharacterByArtefact(enemy, "amulet_1"))
        dmg -= 15;
    if (IsEquipCharacterByArtefact(attack, "amulet_2"))
        dmg *= 0.9;
    if (IsEquipCharacterByArtefact(attack, "KhaelRoa_item"))
        dmg = dmg * 10; // калеуче
    ...
    if (CheckAttribute(enemy, "GuardMask"))
    {
        dmg = 0;
        if (attack.id == "Blaze")
            log_info("Ваши пули бессильны против Стража!");
    }
    // калеуче
    bool bTutt = (IsCharacterEquippedArtefact(pchar, "kaleuche_amulet2")) || (IsCharacterEquippedArtefact(pchar, "kaleuche_amulet3")) if (CheckAttribute(enemy, "KhaelRoaMonster") && !bTutt)
    {
        dmg = 0.0;
        if (attack.id == "Blaze")
            log_info("Ваши пули не наносят урона чавинави!");
    }
...
float LAi_GunReloadSpeed(aref chr)
{
    ...
    //Учтем абилити
    if (CheckAttribute(chr, "MultiShooter"))
        charge_dlt = charge_dlt * 2.00; // may-16
    if (IsCharacterPerkOn(chr, "HT4"))
        charge_dlt = charge_dlt * 1.40; // суперперк!!
...
void LAi_ApplyCharacterAttackDamage(aref attack, aref enemy, string attackType, bool isBlocked)
{
    ...
    if (IsEquipCharacterByArtefact(enemy, "amulet_3"))
        chance = 0.85;
    if (IsEquipCharacterByArtefact(attack, "amulet_4"))
        chance = 0.90;
    ...
    bool bMonster = (CheckAttribute(enemy, "monster")) || (enemy.chr_ai.group == LAI_GROUP_MONSTERS) if (GetCharacterEquipByGroup(attack, BLADE_ITEM_TYPE) == "knife_01" && bMonster)
    {
        dmg = dmg * 3;
    }
    ...
    if (enemy.chr_ai.group == "KSOCHITAM_MONSTERS")
    {
        if (GetCharacterEquipByGroup(attack, BLADE_ITEM_TYPE) == "knife_01")
            dmg = dmg * 4;
        else
            dmg = dmg / 4;
    }
    ...
    if (CheckAttribute(enemy, "GuardMask") && GetCharacterEquipByGroup(attack, BLADE_ITEM_TYPE) != "knife_01")
    {
        dmg = 0.0;
        if (attack.id == "Blaze")
            log_info("Ваше оружие бессильно против Стража!");
    }
    // калеуче
    bool bTutt = (IsCharacterEquippedArtefact(pchar, "kaleuche_amulet2")) || (IsCharacterEquippedArtefact(pchar, "kaleuche_amulet3")) if (CheckAttribute(enemy, "KhaelRoaMonster") && !bTutt)
    {
        dmg = 0.0;
        if (attack.id == "Blaze")
            log_info("Ваше оружие не наносит урона чавинави!");
    }
    if (CheckAttribute(attack, "KhaelRoaMonster") && !bTutt)
    {
        dmg = dmg * 5;
    ...
    if (CheckAttribute(enemy, "cirassId"))
    {
        ...
        if (makeint(stf(fCritical) * 1000) <= rand(999))
            critical = 0.0;
    }
    //<-- фикс кирас
    ...
    if (dmg > 0.0)
    {
        ...
        if (!IsCharacterEquippedArtefact(enemy, "talisman8"))
            MakePoisonAttackCheckSex(enemy, attack);
        // яд таино // Addon 2016-1 Jason Пиратская линейка
        if (IsCharacterEquippedArtefact(attack, "indian_poison") && !CheckAttribute(pchar, "IsMushketer"))
            MakeIndianPoisonAttack(enemy, attack);
    }
    ...
    if (LAi_IsDead(enemy))
    {
        ...
        if (attack.model.animation == "mushketer")
        {
            if (CheckAttribute(enemy, "City"))
                AddCharacterExpToSkill(attack, LAi_GetMushketFencingType(attack), makefloat((10.0 + ((1 + re) / (1 + ra)) * 6.5) / 20));
            else
                AddCharacterExpToSkill(attack, LAi_GetMushketFencingType(attack), makefloat(10.0 + ((1 + re) / (1 + ra)) * 6.5));
        }
        else
        {
            if (CheckAttribute(enemy, "City"))
                AddCharacterExpToSkill(attack, LAi_GetBladeFencingType(attack), makefloat((10.0 + ((1 + re) / (1 + ra)) * 6.5) / 20));
            else
    ...
    if (!noExp)
    {
        ...
        {
            if (CheckAttribute(enemy, "City"))
                AddCharacterExpToSkill(attack, LAi_GetMushketFencingType(attack), Makefloat(exp * 0.02));
            else
                AddCharacterExpToSkill(attack, LAi_GetMushketFencingType(attack), Makefloat(exp * 0.2));
        }
        else
        {
            if (CheckAttribute(enemy, "City"))
                AddCharacterExpToSkill(attack, LAi_GetBladeFencingType(attack), Makefloat(exp * 0.02));
            else
...
void LAi_ApplyCharacterFireDamage(aref attack, aref enemy, float kDist)
{
    ...
    if (damage > 0.0)
    {
        ...
        LAi_CheckKillCharacter(enemy);
        // яд таино // Addon 2016-1 Jason Пиратская линейка
        if (IsCharacterEquippedArtefact(attack, "indian_poison"))
            MakeIndianPoisonAttack(enemy, attack);
    }
    //Есть ли оружие у цели
    bool isSetBalde = (CheckAttribute(enemy, "equip.blade") == true); //(SendMessage(enemy, "ls", MSG_CHARACTER_EX_MSG, "IsSetBalde") != 0);
    ...
    {
        if (CheckAttribute(enemy, "City"))
            AddCharacterExpToSkill(attack, SKILL_PISTOL, Makefloat(exp * 0.1));
        else
...
{
    if (LAi_GetBladeFencingType(chr) == attackType)
        return fOn;
    return fOff;
}
