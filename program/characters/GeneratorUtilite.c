...
int GetBoarding_player_hp_Bonus(int mcrew, int ecrew) // бонус от перевеса, когда if (mcrew > ecrew)
{
    ...
    e_rank_bonus = 0;
    if (ecrew == 0)
        ecrew = 1; // дабы деления на 0 не вознкало
    if (true)
    { // только этот код
        if (mcrew > ecrew)
        {
...
float Sea_ApplyMaxSpeedZ(aref arCharShip, float fWindDotShip, ref rCharacter) //float fTRFromSailDamage,
{
    ...
    else // default sailing model for square rigged ships or if enhanced sailing mod not used
    {
        ...
        if (stf(fkoeff) < 1)
            fkoeff = 1; // движение прямых парусов против ветра оставляем без изменений
...
float AgainstWindFactor(ref rCharacter)
{
    ...
    if (Diff < BestCourse)
        Factor = 0.5; // Ship sails faster between wind on the beam and best course arrows
