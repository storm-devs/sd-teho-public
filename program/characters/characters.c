...
void CharactersInit()
{
    ...
    LoadSegment(sPath + "init\Providence.c");
    ReloadProgressUpdate();
...
void SetOverloadNormWalk(ref character)
{
    ...
    bool bOk = (GetItemsWeight(character) > GetMaxItemsWeight(character)) && (!IsEquipCharacterByArtefact(character, "totem_06")) if (bOk || character.model == "protocusto" || CheckAttribute(character, "GenQuest.CantRun"))
