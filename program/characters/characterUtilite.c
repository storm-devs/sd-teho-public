...
int GetCharacterShipClass(ref _refCharacter)
{
    int nShipType = GetCharacterShipType(_refCharacter);
    if (nShipType == SHIP_NOTUSED)
        return 6; // -1 неправильно, иначе сравнение врет, нет кораля - это лодка
...
float GetSailRPD(ref _refCharacter) // процент ремонта парусов в день
{
    ...
    if (CheckOfficersPerk(_refCharacter, "SelfRepair"))
        repairSkill = repairSkill * 1.25;
    else
    {
        if (CheckOfficersPerk(_refCharacter, "Builder"))
            repairSkill = repairSkill * 1.00;
        else
        {
            if (CheckOfficersPerk(_refCharacter, "LightRepair"))
                repairSkill = repairSkill * 0.75;
            else
            {
                if (CheckOfficersPerk(_refCharacter, "Carpenter"))
                    repairSkill = repairSkill * 0.50;
                else
                    repairSkill = 0.0;
            }
        }
    ...
    if (IsCharacterEquippedArtefact(_refCharacter, "talisman7"))
        repairSkill = repairSkill * 2.0; // вдвое увеличивает
    if (IsCharacterEquippedArtefact(_refCharacter, "indian_5"))
        repairSkill = repairSkill * 0.9; // тсантса
    if (IsCharacterEquippedArtefact(_refCharacter, "indian_6"))
        repairSkill = repairSkill * 0.9; // Коралловая голова
                                         //<-- belamour
...
bool CheckSelfRepairConditions()
{
    ...
    if (!CheckAttribute(pchar, "ship.type"))
        return false;
...
float GetEnergyMushketDrain(ref attack, float _weight, string _bladeType)
{
    ...
    {
    case "FencingL":
        if (IsCharacterPerkOn(attack, "HT4"))
            drain = _weight / 22.0;
        else
            drain = _weight / 22.0 + 0.4;
        break;
    case "FencingS":
        if (IsCharacterPerkOn(attack, "HT4"))
            drain = _weight / 17.5;
        else
            drain = _weight / 17.5 + 0.4;
        break;
    case "FencingH":
        if (IsCharacterPerkOn(attack, "HT4"))
            drain = _weight / 10.0;
        else
            drain = _weight / 10.0 + 0.4;
        break;
    }
    return drain;
}
...
void EquipCharacterByItem(ref chref, string itemID)
{
    ...
    if (chref.model.animation == "mushketer")
    {
        ...
        {
            if (groupName == GUN_ITEM_TYPE && !HasSubStr(arItm.id, "mushket"))
                return;
        }
    }
    else
    {
        if (groupName == GUN_ITEM_TYPE)
        {
            ...
            if (HasSubStr(arItm.id, "mushket") && !IsCharacterPerkOn(chref, "Gunman"))
                return;
        }
        if (groupName == GUN_ITEM_TYPE && HasSubStr(arItm.id, "mushket"))
            return;
...
void EnableEquip(ref chref, string equiping_group, bool enable)
{
    ...
    else
    {
        if (equiping_group == "")
        {
            ...
            chref.equip.disabled_group.(stmp) = true;
            //
            stmp = SPECIAL_ITEM_TYPE;
            chref.equip.disabled_group.(stmp) = true; // зелье команчей
        }
        else
        {
...
bool IsEquipCharacterByArtefact(ref chref, string itemID)
{
    ...
    if (!CheckAttribute(rItem, "groupID"))
        return false;
    if (!CheckAttribute(chref, "equip_item"))
        return false;
...
{
    if (CheckAttribute(chref, "equip_item." + slotID))
        return chref.equip_item.(slotID);
...
bool IsCanArtefactBeEquipped(ref chref, string itemID)
{
    ...
    for (int i = 0; i < q; i++)
    {
        ...
        else
        {
            ...
            if (!CheckAttribute(rItem1, "type") || !CheckAttribute(rItem2, "type"))
                return false;
            if (rItem1.type == rItem2.type)
                continue;
            if (rItem1.type == ITEM_OBEREG || rItem1.type == ITEM_TOTEM)
                continue;
            if (rItem2.type == ITEM_OBEREG || rItem2.type == ITEM_TOTEM)
                continue;
            else
                return false; // разного типа - одеть нельзя !!!!!
        }
    }
    return true;
}
...
void EquipCharacterByArtefact(ref chref, string itemID)
{
    ...
    {
        if (IsEquipCharacterByArtefact(chref, itemID))
            return; // если уже экипирован этим артефактом
...
bool IsCharacterEquippedArtefact(ref rChar, string itemID)
{
    ...
    {
        if (IsEquipCharacterByArtefact(pchar, itemID))
            return true;
        ...
        if (CheckAttribute(rItem, "kind"))
        {
            ...
            if (sKind == "fighter")
            {
                for (int i = 1; i < 4; i++)
                {
                    ...
                    if (iOfficer != -1)
                    {
                        if (IsEquipCharacterByArtefact(&characters[iOfficer], itemID))
                            return true;
                    }
                }
            }
        ...
        else
        {
            for (int io = 0; io < GetNotQuestPassengersQuantity(rChar); io++)
            {
                ...
                if (iOfficer != -1)
                {
                    ...
                    {
                        if (IsEquipCharacterByArtefact(sld, itemID))
                            return true;
                    }
                }
            }
...
{
    if (IsCharacterEquippedArtefact(rChar, sItem))
        return fOn;
    return fOff;
}
...
{
    if (IsEquipCharacterByArtefact(rChar, sItem))
        return fOn;
    return fOff;
}
...
int ChangeCharacterComplexReputationABS(ref chref, string repName, float incr)
{
    ...
    if (CheckAttribute(chref, "reputation." + repName))
        curVal = stf(chref.reputation.(repName));

    if (curVal < COMPLEX_REPUTATION_NEUTRAL)
        incr = -incr;
...
int ChangeCharacterComplexReputation(ref chref, string repName, float incr)
{
    ...
    if (CheckAttribute(chref, "GenQuest.ReputationNotChange"))
        return sti(chref.reputation.(repName));
    float prevVal = COMPLEX_REPUTATION_NEUTRAL;
    if (CheckAttribute(chref, "reputation." + repName))
        prevVal = stf(chref.reputation.(repName));
    float newVal = prevVal + incr;
    if (newVal < COMPLEX_REPUTATION_MIN)
        newVal = COMPLEX_REPUTATION_MIN;
    if (newVal > COMPLEX_REPUTATION_MAX)
        newVal = COMPLEX_REPUTATION_MAX;
...
bool SetMainCharacterToMushketer(string sMushket, bool _ToMushketer) // если _ToMushketer == true, значит делаем мушкетером, иначе - делаем ГГ фехтовальщиком
{
    ...
    if (_ToMushketer && sMushket != "") // Делаем ГГ мушкетером
    {
        ...
        if (!CanEquipMushketOnLocation(PChar.Location))
            return false;
...
float ChangeIndianRelation(float _val) // Jason: репутация у индейцев
{
    ...
    if (stf(pchar.questTemp.Indian.relation) > 100.0)
        pchar.questTemp.Indian.relation = 100.0;
    if (stf(pchar.questTemp.Indian.relation) < 0)
        pchar.questTemp.Indian.relation = 0;
