...
int GetCharacterAddHPValue(ref _refCharacter)
{
    ...
    if (GetCharacterEquipByGroup(_refCharacter, CIRASS_ITEM_TYPE) == "underwater")
        Ers = Ers + 2;
    if (GetCharacterEquipByGroup(_refCharacter, CIRASS_ITEM_TYPE) == "cirass1")
        Ers = Ers + 1;
    if (GetCharacterEquipByGroup(_refCharacter, CIRASS_ITEM_TYPE) == "cirass2")
        Ers = Ers + 1;
    if (GetCharacterEquipByGroup(_refCharacter, CIRASS_ITEM_TYPE) == "cirass4")
        Ers = Ers + 1;
    // <-- убрать влияние кирас
...
int ApplayNavyPenalty(ref _refCharacter, string skillName, int sumSkill)
{
    if (IsCompanion(_refCharacter) && GetRemovable(_refCharacter)) //пусть будет для компаньонов тоже sti(_refCharacter.index) == GetMainCharacterIndex()) // только для главного, чтоб не тормозить всю игру
    {
        ...
        if (CheckAttribute(_refCharacter, "NoNavyPenalty"))
            needSkill = 1;
...
string GetReputationComplexName(int reputation, string repName)
{
    ...
    case "fame":
        if (reputation < 21)
            return "REPUTATION_FAME_1";
        if (reputation < 41)
            return "REPUTATION_FAME_2";
        if (reputation < 61)
            return "REPUTATION_FAME_3";
        if (reputation < 81)
            return "REPUTATION_FAME_4";
        return "REPUTATION_FAME_5";
        break;

    case "nobility":
        if (reputation < 6)
            return "REPUTATION_NOBILITY_1";
        if (reputation < 16)
            return "REPUTATION_NOBILITY_2";
        if (reputation < 31)
            return "REPUTATION_NOBILITY_3";
        if (reputation < 48)
            return "REPUTATION_NOBILITY_4";
        if (reputation < 53)
            return "REPUTATION_NOBILITY_5";
        if (reputation < 71)
            return "REPUTATION_NOBILITY_6";
        if (reputation < 86)
            return "REPUTATION_NOBILITY_7";
        if (reputation < 96)
            return "REPUTATION_NOBILITY_8";
...
int ApplySPECIALCirassPenalty(ref rChar, String sSkillName)
{
    ...
    if (sSuit == INVALID_SUIT)
        return 0;

    switch (sSuit)
    {
    case "cirass1":
        if (sSkillName == SPECIAL_E)
            iValue = -1;
        if (sSkillName == SPECIAL_A)
            iValue = -2;
        break;
...
int SetCharacterSkillBySuit(ref rChar, String sSkillName)
{
    ...
    case "underwater": // Jason: водолаз
        ...
        if (sSkillName == SKILL_F_LIGHT)
            iValue = -15;
        if (sSkillName == SKILL_FENCING)
            iValue = -15;
        if (sSkillName == SKILL_F_HEAVY)
            iValue = -15;
        if (sSkillName == SKILL_PISTOL)
            iValue = -50;
        break;
    }
...
int ApplySPECIALMangarosaPotion(ref rChar, String sSkillName) // 280313
{
    ...
    if (CheckAttribute(rChar, "questTemp.Mangarosa.Potion"))
    {
        ...
        if (CheckAttribute(rChar, "questTemp.Mangarosa.Potion.Total"))
        {
            ...
            if (sSkillName == SPECIAL_P)
                iValue = 1;
            if (sSkillName == SPECIAL_C)
                iValue = 1;
            if (sSkillName == SPECIAL_L)
                iValue = 2;
        }
    }
    return iValue;
...
// _chref - character _skill - name ex -"Fencing"  _skill_exp = "Fencing_exp"   _addValue = 100
{
    ...
    {                                                                                        // if skill = 0 then it is great loser
        ...
        while (makeint(sti(_chref.skill.(_skill)) * GetCharacterExpRate(_chref, _skill)) <= stf(_chref.skill.(_skill_exp)) && sti(_chref.skill.(_skill)) < SKILL_MAX)
        {
...
int GetMaxItemsWeight(ref _chref)
{
    if (CheckAttribute(_chref, "Skill.FencingS"))
    {
        ...
        if (IsEquipCharacterByArtefact(_chref, "obereg_3"))
            iBonus = makeint(iBonus * 1.15);
        if (IsEquipCharacterByArtefact(_chref, "indian_7"))
            iBonus = makeint(iBonus * 0.9);
        return iBonus;
    }
    else
    {
...
int Statistic_AddValue(ref _chref, string _attrName, int _add) // set and get(_add==0) value
{
    ...
    if (!CheckAttribute(_chref, "index"))
        return 0;

    if (sti(_chref.index) != GetMainCharacterIndex())
        return 0; // оптимизация
...
{
    if (sti(_attack.index) != GetMainCharacterIndex())
        return; // оптимизация
...
void CheckAchievments(ref _chref, int achievNum)
{
    ...
    if (sti(_chref.index) != GetMainCharacterIndex())
        return; // оптимизация, выходим если не ГГ
    ...
    case 5: // Легендарный :  уровень >= 40
        if (sti(_chref.rank) >= 40)
            Achievment_SetStat(_chref, 5, 1);
        break;
    case 11: // Баловень судьбы : (удача = 100%)
        if (GetSkillValue(_chref, SKILL_TYPE, SKILL_FORTUNE) >= 100)
            Achievment_SetStat(_chref, 11, 1);
        break;
    case 12: // Фехтовальщик : (фехт в ЛО = 100%)
        if (GetSkillValue(_chref, SKILL_TYPE, SKILL_F_LIGHT) >= 100)
            Achievment_SetStat(_chref, 12, 1);
        break;
    case 13: // Гусар : (фехт в СО = 100%)
        if (GetSkillValue(_chref, SKILL_TYPE, SKILL_FENCING) >= 100)
            Achievment_SetStat(_chref, 13, 1);
        break;
    case 14: // Драгун : (фехт в ТО = 100%)
        if (GetSkillValue(_chref, SKILL_TYPE, SKILL_F_HEAVY) >= 100)
            Achievment_SetStat(_chref, 14, 1);
        break;
    case 15: // Шпион : (скрытность = 100%)
        if (GetSkillValue(_chref, SKILL_TYPE, SKILL_SNEAK) >= 100)
            Achievment_SetStat(_chref, 15, 1);
        break;
    case 16: // Крепкий орешек : (защита = 100%)
        if (GetSkillValue(_chref, SKILL_TYPE, SKILL_DEFENCE) >= 100)
            Achievment_SetStat(_chref, 16, 1);
        break;
    case 17: // Адмирал : (навигация = 100%)
        if (GetSkillValue(_chref, SKILL_TYPE, SKILL_SAILING) >= 100)
            Achievment_SetStat(_chref, 17, 1);
        break;
    case 18: // Негоциант : (торговля = 100%)
        if (GetSkillValue(_chref, SKILL_TYPE, SKILL_COMMERCE) >= 100)
            Achievment_SetStat(_chref, 18, 1);
        break;
    case 19: // Джедай :  достичь максимальной репутации
        if (sti(_chref.reputation.nobility) > 95)
            Achievment_SetStat(_chref, 19, 1);
        break;
    case 20: // Ситх : достичь минимальной репутации
        if (sti(_chref.reputation.nobility) < 5)
            Achievment_SetStat(_chref, 20, 1);
        break;
    case 28: // Скряга : сколотить капитал >= 10 000 000
        if (sti(_chref.money) >= 10000000)
            Achievment_SetStat(_chref, 28, 1);
        break;
    case 30: // Флотоводец : собрать максимальную эскадру
        if (GetRemovableCompanionsNumber(_chref) == COMPANION_MAX)
            Achievment_SetStat(_chref, 30, 1);
        break;
    case 32: // Болезненный : уронить здоровье до минимума
        if (GetHealthNum(_chref) <= 5)
            Achievment_SetStat(_chref, 32, 1);
        break;
    case 33: // Коллекционер : собрать коллекцию из 100 сокровищ
        if (CheckItemMyCabin("icollection") >= 100 || GetCharacterItem(_chref, "icollection") >= 100)
            Achievment_SetStat(_chref, 33, 1);
        break;
    case 42: // Ростовщик :  вклады у ростовщиков на 5000000 песо
        if (CheckTotalDepositsSum(_chref, 5000000))
            Achievment_SetStat(_chref, 42, 1);
        break;
    case 44: // Ветер Перемен -  прохождение за Голландию
        if (CheckAttribute(_chref, "questTemp.HWIC.Detector") && (_chref.questTemp.HWIC.Detector == "holl_win"))
            Achievment_SetStat(_chref, 44, 1);
        break;
    case 45: // На службе Империи -  прохождение за Англию
        if (CheckAttribute(_chref, "questTemp.HWIC.Detector") && (_chref.questTemp.HWIC.Detector == "eng_win"))
            Achievment_SetStat(_chref, 45, 1);
        break;
    case 46: // Не зная страха - прохождение "Против всех"
        if (CheckAttribute(_chref, "questTemp.HWIC.Detector") && (_chref.questTemp.HWIC.Detector == "self_win"))
            Achievment_SetStat(_chref, 46, 1);
        break;
    case 47: // Простодушный -  провальный финад
        if (CheckAttribute(_chref, "questTemp.Tieyasal_FailEnd"))
            Achievment_SetStat(_chref, 47, 1);
        break;
    case 48: // Усердный -  настоящий финал
        if (CheckAttribute(_chref, "questTemp.Tieyasal_WinEnd"))
            Achievment_SetStat(_chref, 48, 1);
        break;
    case 49: // Морской дьявол -  стать капитаном Калеуче
        if (CheckAttribute(_chref, "questTemp.Caleuche.Abordage") && (RealShips[sti(_chref.Ship.Type)].BaseType == SHIP_CURSED_FDM))
            Achievment_SetStat(_chref, 49, 1);
        break;
    case 54: // "Путешественник" - посетить все колонии (таверны)
        if (sti(pchar.questTemp.TavernVisit.counter) == 28)
            Achievment_SetStat(_chref, 54, 1);
        break;
    case 55: // "Команда профессионалов" - нанять офицеров на все должности
        if (CheckForAllOfficers())
            Achievment_SetStat(_chref, 55, 1);
        break;

    case 62: // за три приготовленных зелья из Мангаросы
        if (CheckAttribute(_chref, "Mangarosa.Alchemy.MP") && CheckAttribute(_chref, "Mangarosa.Alchemy.MF") && CheckAttribute(_chref, "Mangarosa.Alchemy.MT"))
            Achievment_SetStat(_chref, 62, 1);
        break;
    case 63: // за ремонт на всех верфях архипелага
        if (sti(pchar.questTemp.ShipyardVisit.counter) == 23)
            Achievment_SetStat(_chref, 63, 1);
        break;
    case 66: // за полный апгрейд корабля
        if (CheckAttribute(_chref, "achievment.Tuning.stage1") && CheckAttribute(_chref, "achievment.Tuning.stage2") && CheckAttribute(_chref, "achievment.Tuning.stage3") && CheckAttribute(_chref, "achievment.Tuning.stage4"))
            Achievment_SetStat(_chref, 66, 1);
        break;
    case 69: // "найм" монаха-капеллана
        if (CheckAttribute(_chref, "questTemp.ShipCapellan.Yes") && (_chref.questTemp.ShipCapellan.Yes == "true"))
            Achievment_SetStat(_chref, 69, 1);
        break;
        /*        
...
string GetCharType(aref _enemy) //TO_DO переделать на тип в НПС
{
    ...
    case LAI_TYPE_PATROL:
        name = "Solder";
        break case LAI_TYPE_GUARDIAN : name = "Solder";
        break case LAI_TYPE_OFFICER : name = "Solder";
        break;
    // Citizen
    case LAI_TYPE_ACTOR:
        name = "Citizen";
...
int ChangeCharacterHunterScore(ref chref, string _huntName, int incr)
{
    ...
    if (_huntName == "pirhunter")
        return 0;
    if (CheckAttribute(chref, "GenQuest.HunterScore2Pause"))
        incr = 0;

    if (CheckAttribute(chref, "reputation." + _huntName))
        prevVal = sti(chref.reputation.(_huntName));
...
{
    if (i <= -10)
        return "Награда за голову " + (-i * 1000);
    if (i < 0)
        return "Враждебная";
    if (i <= 20)
        return "Нейтральная";
...
void initNewMainCharacter() //инициализация главного героя
{
    ...
    if (MOD_EXP_RATE < 10)
        MOD_EXP_RATE = 10; // иначе будет развал целостности данных, порог релиховой версии бля всех сложностей.
