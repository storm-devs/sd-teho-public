...
void ProcessControls()
{
    ...
    if (ControlName == "QuickSave")
    {
        MakeQuickSave();
    }
    if (ControlName == "QuickLoad")
    {
        MakeQuickLoad();
    }
...
 
    {
        ...
        if (CheckAttribute(pchar, "questTemp.NoFast"))
            return; //запрет ускорения
        ...
        { // без боя
            if (bSeaActive && !bAbordageStarted)
            {
                if (TimeScaleCounter > 28)
                    TimeScaleCounter = 28; // море
            }
            else
            {
                if (TimeScaleCounter > 8)
                    TimeScaleCounter = 8; // суша
            }
        }
...
       
    ...
    case "Dolly":
        ...
        {
            bOk = (IsCharacterInLocator(pchar, "item", "dolly1")) || (IsCharacterInLocator(pchar, "item", "dolly2")) || (IsCharacterInLocator(pchar, "item", "dolly3")) if (CheckAttribute(location, "canteleport") && bOk && !CheckAttribute(pchar, "questTemp.LSC.MaryBye"))
            {
                if (CheckAttribute(pchar, "questTemp.NotTeleportation"))
                    DoQuestCheckDelay("TalkSelf_Quest", 0.1);
                else
                {
                    if (IsCharacterInLocator(pchar, "item", "dolly1"))
                        sGlobalTemp = "dolly1";
                    if (IsCharacterInLocator(pchar, "item", "dolly2"))
                        sGlobalTemp = "dolly2";
                    if (IsCharacterInLocator(pchar, "item", "dolly3"))
                        sGlobalTemp = "dolly3";
                    Dolly_TeleportStart();
                }
            }
        ...
        if (CheckAttribute(location, "id") && location.id == "Treasure_Alcove") // калеуче
        {
            ...
            {
                if (IsCharacterInLocator(pchar, "teleport", "teleport0"))
                    sGlobalTemp = "teleport0";
                if (IsCharacterInLocator(pchar, "teleport", "teleport1"))
                    sGlobalTemp = "teleport1";
                if (IsCharacterInLocator(pchar, "teleport", "teleport2"))
                    sGlobalTemp = "teleport2";
                if (IsCharacterInLocator(pchar, "teleport", "teleport3"))
                    sGlobalTemp = "teleport3";
                if (IsCharacterInLocator(pchar, "teleport", "teleport4"))
                    sGlobalTemp = "teleport4";
                if (IsCharacterInLocator(pchar, "teleport", "teleport5"))
                    sGlobalTemp = "teleport5";
                if (IsCharacterInLocator(pchar, "teleport", "teleport6"))
                    sGlobalTemp = "teleport6";
...
void Teleport(int step)
{
    ...
    if (nTeleportLocation >= nLocationsNum)
        nTeleportLocation = 1;
    if (nTeleportLocation <= 0)
        nTeleportLocation = nLocationsNum - 1;
...
{
    if (CharacterIsDead(GetMainCharacter()))
    {
        return;
    }
    if (bSeaActive && !bAbordageStarted)
...
bool CheckSaveGameEnabled()
{
    ...
    if (CharacterIsDead(mchref))
    {
        TmpBool = false;
    }

    if (LAi_IsFightMode(mchref) && MOD_SKILL_ENEMY_RATE > 1)
    {
        TmpBool = false;
    }

    if (bDisableMapEnter && !CheckBattleSeaSaveEnabled() && bSeaActive)
    {
        TmpBool = false;
    }
